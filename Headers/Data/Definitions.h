...
//#if USE_POOL
    ...
    //  code based on book Modern C++ Design: Generic Programming and Design Patterns Applied, by Andrei Alexandrescu, Addison-Wesley Pub Co, 2001
...
//#endif //#if USE_POOL
...
#if USE_EXACT
    template <typename T> class Exact;
    template <typename T> class ExactFraction;
#endif //#if USE_EXACT
    ...
    typedef std::unordered_multiset<FrontElement *, AbstractFront> FrontElementMultiHash;
#endif //#if USE_SET
#endif //#if USE_C__11
...
#else
    struct R2Tuple {Real values[2];};
    struct R3Tuple {Real values[3];};
    struct R4Tuple {Real values[4];};
    struct UI2Tuple {UInt values[2];};
    struct ULI2Tuple {ULInt values[2];};
    struct ULI4Tuple {ULInt values[4];};
    struct ElementTuple {Vertex *v; FrontElementList *fes; Element *e;};
#endif //#if USE_C__11
...
#endif //#if USE_C__11
{
    ...
    if (str.eof())
    {
        ...
        err << "Impossible to convert from string '" << s << "' to value of type " << typeid(Type).name();
