 ...
 * {a0+a4, a1+a5, a2+a6, a3+a7} For packet-size smaller or equal to 4, this
 * boils down to a noop.
 */
template <typename Packet>
 ...
 * \a Offset last elements of \a first and \a Offset first elements of \a
 * second.
 *
 * This function is currently only used to optimize matrix-vector products on
 * unligned matrices. It takes 2 packets that represent a contiguous memory
 * array, and returns a packet starting at the position \a Offset. For instance,
 * for packets of 4 elements, we have: Input:
 *  - first = {f0,f1,f2,f3}
 *  - second = {s0,s1,s2,s3}
 * Output:
