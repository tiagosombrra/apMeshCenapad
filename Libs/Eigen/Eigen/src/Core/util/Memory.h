 ...
 * elements of type TYPE on the stack if SIZE is smaller than
 * EIGEN_STACK_ALLOCATION_LIMIT, and if stack allocation is supported by the
 * platform (currently, this is Linux and Visual Studio only). Otherwise the
 * memory is allocated on the heap. The allocated buffer is automatically
...
class aligned_allocator : public std::allocator<T> {
 public:
  ...
  pointer allocate(size_type num, const void* /*hint*/ = 0) {
    ...
    // It triggered eigen/Eigen/src/Core/util/Memory.h:189:12: warning: argument
    // 1 value '18446744073709551612' exceeds maximum object size
    // 9223372036854775807
