    ...
    while (getline(stream, line)) {
      ...
      switch (type) {
        case type_t::all_pot_sizes: {
          ...
          float gflops;
          int sscanf_result = sscanf(line.c_str(), "%x %x %f", &product_size,
                                     &block_size, &gflops);
          if (3 != sscanf_result || !product_size || product_size > 0xfff ||
              !block_size || block_size > 0xfff || !isfinite(gflops)) {
        ...
        case type_t::default_sizes: {
          ...
          float gflops;
          int bk, bm, bn;
          int sscanf_result = sscanf(line.c_str(), "%x default(%d, %d, %d) %f",
                                     &product_size, &bk, &bm, &bn, &gflops);
          if (5 != sscanf_result || !product_size || product_size > 0xfff ||
              !isfinite(gflops)) {
            cerr << "ill-formed input file: " << filename << endl;
            cerr << "offending line:" << endl << line << endl;
            exit(1);
  ...
  void import_input_file_range_one_product_size(
    ...
    for (auto it = begin; it != end; ++it) {
      if (it->product_size != product_size) {
        ...
        cerr << "(Expected all entries for product size " << hex << product_size
             << dec << " to be grouped)" << endl;
        exit(1);
      }
...
void check_all_files_in_same_exact_order(
  ...
  for (size_t i = 0; i < preprocessed_inputfiles.size(); i++) {
    if (preprocessed_inputfiles[i].entries.size() != num_entries) {
      cerr << "these files have different number of entries: "
           << preprocessed_inputfiles[i].filename << " and "
  ...
  for (size_t entry_index = 0; entry_index < num_entries; entry_index++) {
    const uint16_t entry_product_size =
        first_file.entries[entry_index].product_size;
    const uint16_t entry_block_size =
        first_file.entries[entry_index].block_size;
    for (size_t file_index = 0; file_index < preprocessed_inputfiles.size();
         file_index++) {
      const preprocessed_inputfile_t& cur_file =
          preprocessed_inputfiles[file_index];
      if (cur_file.entries[entry_index].product_size != entry_product_size ||
        ...
        cerr << "entries not in same order between these files: "
             << first_file.filename << " and " << cur_file.filename << endl;
        exit(1);
      }
    }
  }
...
float efficiency_of_subset(
  ...
  const preprocessed_inputfile_t& first_file =
      preprocessed_inputfiles[subset[0]];
  const size_t num_entries = first_file.entries.size();
  ...
  while (entry_index < num_entries) {
    ...
    if (entry_index == num_entries ||
      ...
      for (size_t e = first_entry_index_with_this_product_size; e < entry_index;
           e++) {
...
void dump_table_for_subset(
  ...
  cout << "  static const size_t BaseSize = " << min_product_size.k << ";"
       << endl;
  ...
  if (i != TableSize) {
    cerr << endl
         << "Wrote " << i << " table entries, expected " << TableSize << endl;
    abort();
  }
  cout << endl << "    };" << endl;
...
float efficiency_of_partition(
  ...
  for (auto s = partition.begin(); s != partition.end(); ++s) {
    efficiency =
        min(efficiency, efficiency_of_subset(preprocessed_inputfiles, *s));
  }
  return efficiency;
}

void make_first_subset(size_t subset_size, vector<size_t>& out_subset,
                       size_t set_size) {
...
size_t max_feasible_subset_size(size_t n) {
  assert(n > 0);
  const size_t minresult =
      min<size_t>(n - 1, always_search_subsets_of_size_at_least);
...
void find_subset_with_efficiency_higher_than(
  ...
  while (!inout_remainder.empty()) {
    ...
    size_t candidate_indices_subset_size =
        max_feasible_subset_size(candidate_indices.size());
    while (candidate_indices_subset_size >= 1) {
      vector<size_t> candidate_indices_subset;
      make_first_subset(candidate_indices_subset_size, candidate_indices_subset,
      ...
      while (true) {
        for (size_t i = 0; i < candidate_indices_subset_size; i++) {
          trial_subset[out_subset.size() + i] =
              inout_remainder[candidate_indices_subset[i]];
        }

        float trial_efficiency =
            efficiency_of_subset(preprocessed_inputfiles, trial_subset);
        ...
        if (is_last_subset(candidate_indices_subset,
                           candidate_indices.size())) {
          break;
        }
      ...
      if (best_efficiency > required_efficiency_to_beat) {
        for (size_t i = 0; i < best_candidate_indices_subset.size(); i++) {
          candidate_indices[i] =
              candidate_indices[best_candidate_indices_subset[i]];
        }
...
void find_partition_with_efficiency_higher_than(
    const vector<preprocessed_inputfile_t>& preprocessed_inputfiles,
    float required_efficiency_to_beat, vector<vector<size_t>>& out_partition) {
  ...
  while (!remainder.empty()) {
    ...
    find_subset_with_efficiency_higher_than(preprocessed_inputfiles,
                                            required_efficiency_to_beat,
                                            remainder, new_subset);
...
void print_partition(
    ...
    const vector<vector<size_t>>& partition) {
  float efficiency =
      efficiency_of_partition(preprocessed_inputfiles, partition);
  cout << "Partition into " << partition.size() << " subsets for "
       << efficiency * 100.0f << "% efficiency" << endl;
  for (auto subset = partition.begin(); subset != partition.end(); ++subset) {
    cout << "  Subset " << (subset - partition.begin()) << ", efficiency "
         << efficiency_of_subset(preprocessed_inputfiles, *subset) * 100.0f
         << "%:" << endl;
  ...
  virtual void run(const vector<string>& input_filenames) const override {
    ...
    while (true) {
      ...
      float actual_efficiency =
          efficiency_of_partition(preprocessed_inputfiles, partition);
      cerr << "partition " << preprocessed_inputfiles.size() << " files into "
           << partition.size() << " subsets for " << 100.0f * actual_efficiency
           << " % efficiency"
           << "                  \r" << flush;
      partitions.push_back(partition);
      if (partition.size() == preprocessed_inputfiles.size() ||
          actual_efficiency == 1.0f) {
        break;
      }
...
struct evaluate_defaults_action_t : action_t {
  ...
  friend ostream& operator<<(ostream& s, const results_entry_t& entry) {
    return s << "Product size " << size_triple_t(entry.product_size)
             << ": default block size " << entry.default_block_size << " -> "
             << entry.default_gflops
             << " GFlop/s = " << entry.default_efficiency * 100.0f << " %"
             << " of best POT block size "
             << size_triple_t(entry.best_pot_block_size) << " -> "
             << entry.best_pot_gflops << " GFlop/s" << dec;
  }
  static bool lower_efficiency(const results_entry_t& e1,
  ...
  void show_usage_and_exit() const {
    cerr << "usage: " << invokation_name()
         << " default-sizes-data all-pot-sizes-data" << endl;
    cerr << "checks how well the performance with default sizes compares to "
            "the best "
  ...
  virtual void run(const vector<string>& input_filenames) const override {
    ...
    for (auto it_default_sizes = inputfile_default_sizes.entries.begin();
      ...
      for (auto it = it_all_pot_sizes;
           it != inputfile_all_pot_sizes.entries.end() &&
           it->product_size == product_size;
           ++it) {
...
void show_usage_and_exit(
  ...
  for (auto it = available_actions.begin(); it != available_actions.end();
       ++it) {
    cerr << "  " << (*it)->invokation_name() << endl;
  }
  cerr << "the input files should each contain an output of "
          "benchmark-blocking-sizes"
       << endl;
  exit(1);
}
