...
__attribute__((noinline)) void benchVec(Scalar* a, Scalar* b, Scalar* c,
                                        int size);
...
int main(int argc, char* argv[]) {
  ...
  std::cout << timer.value() << "s  "
            << (double(size2 * REPEAT) / timer.value()) /
                   (1024. * 1024. * 1024.)
            << " GFlops\n";
  return 0;
  for (int innersize = size; innersize > 2; --innersize) {
    if (size2 % innersize == 0) {
      ...
      std::cout << innersize << " x " << outersize << "  " << timer.value()
                << "s   "
                << (double(size2 * REPEAT) / timer.value()) /
                       (1024. * 1024. * 1024.)
                << " GFlops\n";
...
void benchVec(Scalar* a, Scalar* b, Scalar* c, int size) {
  ...
  for (int k = 0; k < REPEAT; ++k)
    for (int i = 0; i < size; i += PacketSize * 8) {
      ...
      //
      //             internal::pstore(&a[i+1*PacketSize], internal::padd(a1,
      //             b1)); a1 = internal::pload(&a[i+5*PacketSize]); b1 =
      //             internal::pload(&b[i+5*PacketSize]);
      //
      //             internal::pstore(&a[i+2*PacketSize], internal::padd(a2,
      //             b2)); a2 = internal::pload(&a[i+6*PacketSize]); b2 =
      //             internal::pload(&b[i+6*PacketSize]);
      //
      //             internal::pstore(&a[i+3*PacketSize], internal::padd(a3,
      //             b3)); a3 = internal::pload(&a[i+7*PacketSize]); b3 =
      //             internal::pload(&b[i+7*PacketSize]);
      //
      //             internal::pstore(&a[i+4*PacketSize], internal::padd(a0,
      //             b0)); internal::pstore(&a[i+5*PacketSize],
      //             internal::padd(a1, b1));
      //             internal::pstore(&a[i+6*PacketSize], internal::padd(a2,
      //             b2)); internal::pstore(&a[i+7*PacketSize],
      //             internal::padd(a3, b3));

      internal::pstore(
          &a[i + 2 * PacketSize],
          internal::padd(internal::ploadu(&a[i + 2 * PacketSize]),
                         internal::ploadu(&b[i + 2 * PacketSize])));
      internal::pstore(
          &a[i + 3 * PacketSize],
          internal::padd(internal::ploadu(&a[i + 3 * PacketSize]),
                         internal::ploadu(&b[i + 3 * PacketSize])));
      internal::pstore(
          &a[i + 4 * PacketSize],
          internal::padd(internal::ploadu(&a[i + 4 * PacketSize]),
                         internal::ploadu(&b[i + 4 * PacketSize])));
      internal::pstore(
          &a[i + 5 * PacketSize],
          internal::padd(internal::ploadu(&a[i + 5 * PacketSize]),
                         internal::ploadu(&b[i + 5 * PacketSize])));
      internal::pstore(
          &a[i + 6 * PacketSize],
          internal::padd(internal::ploadu(&a[i + 6 * PacketSize]),
                         internal::ploadu(&b[i + 6 * PacketSize])));
      internal::pstore(
          &a[i + 7 * PacketSize],
          internal::padd(internal::ploadu(&a[i + 7 * PacketSize]),
