...
static_assert(maxsize < (minsize << 16),
...
void benchmark_t::run() {
  ...
  const size_t combined_three_matrices_sizes =
      sizeof(Scalar) *
      (productsizes.k * productsizes.m + productsizes.k * productsizes.n +
  ...
  gflops =
      2e-9 * productsizes.k * productsizes.m * productsizes.n / time_per_iter;
}
...
void show_usage_and_exit(
  ...
  cerr << "       This is rounded up as needed to a multiple of matrix size."
       << endl;
  cerr << "       A larger working set lowers the chance of a warm cache."
       << endl;
  cerr << "       The default value 0 means use a large enough working" << endl;
  cerr << "       set to likely outsize caches." << endl;
  cerr
      << "       A value of 1 (that is, 1 byte) would mean don't do anything to"
      << endl;
...
float measure_clock_speed() {
  ...
  float stable_estimate =
      all_gflops[2] + all_gflops[3] + all_gflops[4] + all_gflops[5];

  // multiply by an arbitrary constant to discourage trying doing anything with
  // the returned values besides just comparing them with each other.
...
const char session_filename[] =
    "/data/local/tmp/benchmark-blocking-sizes-session.data";
...
void serialize_benchmarks(const char* filename,
  ...
  if (!file) {
    ...
    cerr << "Do you have write permissions on the current working directory?"
         << endl;
    exit(1);
  }
...
bool deserialize_benchmarks(const char* filename,
  ...
  if (1 !=
      fread(&benchmarks_vector_size, sizeof(benchmarks_vector_size), 1, file)) {
    return false;
  }
  if (1 !=
      fread(&first_benchmark_to_run, sizeof(first_benchmark_to_run), 1, file)) {
    return false;
  }
  benchmarks.resize(benchmarks_vector_size);
  if (benchmarks.size() !=
      fread(benchmarks.data(), sizeof(benchmark_t), benchmarks.size(), file)) {
    return false;
  }
  unlink(filename);
  return true;
...
void try_run_some_benchmarks(vector<benchmark_t>& benchmarks, double time_start,
                             size_t& first_benchmark_to_run) {
  ...
  while (true) {
    ...
    if (benchmark_index == benchmarks.size() ||
      ...
      if (current_clock_speed < (1 - clock_speed_tolerance) * max_clock_speed) {
        ...
        while (current_clock_speed <
               (1 - clock_speed_tolerance) * max_clock_speed) {
          if (seconds_to_sleep_if_lower_clock_speed > 32) {
            ...
            serialize_benchmarks(session_filename, benchmarks,
                                 first_benchmark_to_run);
            cerr << "Now restart this benchmark, and it should pick up where "
                    "we left."
                 << endl;
            exit(2);
          }
      ...
      if (rerun_last_tests) {
        cerr << "Redoing the last "
             << 100.0f * float(benchmark_index - first_benchmark_to_run) /
                    benchmarks.size()
             << " % because clock speed had been low.   " << endl;
        return;
      }

      // nothing wrong with the clock speed so far, so there won't be a need to
      // rerun benchmarks run so far in case we later encounter a lower clock
      // speed.
    ...
    if (time_now > time_last_progress_update + 1.0f) {
      ...
      cerr << "Measurements... " << 100.0f * ratio_done << " %, ETA "
           << human_duration_t(float(time_now - time_start) *
                               (1.0f - ratio_done) / ratio_done)
           << "                          \r" << flush;
    }
...
void run_benchmarks(vector<benchmark_t>& benchmarks) {
  ...
  if (deserialize_benchmarks(session_filename, deserialized_benchmarks,
                             first_benchmark_to_run)) {
  ...
  if (use_deserialized_benchmarks) {
    ...
    // Randomly shuffling benchmarks allows us to get accurate enough progress
    // info, as now the cheap/expensive benchmarks are randomly mixed so they
    // average out. It also means that if data is corrupted for some time span,
    // the odds are that not all repetitions of a given benchmark will be
    // corrupted.
  ...
  for (auto it = benchmarks.begin(); it != benchmarks.end(); ++it) {
    if (best_benchmarks.empty() ||
        best_benchmarks.back().compact_product_size !=
            it->compact_product_size ||
...
struct measure_all_pot_sizes_action_t : action_t {
  ...
  virtual void run() const {
    vector<benchmark_t> benchmarks;
    for (int repetition = 0; repetition < measurement_repetitions;
         repetition++) {
            ...
            for (size_t kblock = minsize; kblock <= ksize; kblock *= 2) {
              for (size_t mblock = minsize; mblock <= msize; mblock *= 2) {
                for (size_t nblock = minsize; nblock <= nsize; nblock *= 2) {
                  benchmarks.emplace_back(ksize, msize, nsize, kblock, mblock,
                                          nblock);
                }
              }
            }
...
int main(int argc, char* argv[]) {
  ...
  cout << "packet size: " << internal::packet_traits<MatrixType::Scalar>::size
       << endl;
