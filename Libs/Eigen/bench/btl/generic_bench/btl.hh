...
class BtlString : public std::string {
 public:
  ...
  BtlString(const BtlString& str)
      : std::string(static_cast<const std::string&>(str)) {}
  ...
  void trim(bool left = true, bool right = true) {
    ...
    if (left)
      for (i = 0; i < len && (at(i) == ' ' || at(i) == '\t' || at(i) == '\r' ||
                              at(i) == '\n');
           ++lspaces, ++i)
        ;

    if (right && lspaces < len)
      for (i = len - 1; i >= 0 && (at(i) == ' ' || at(i) == '\t' ||
                                   at(i) == '\r' || at(i) == '\n');
           rspaces++, i--)
        ;
  ...
  /** Decompose the current string as a path and a file.
      For instance: "dir1/dir2/file.ext" leads to path="dir1/dir2/" and
     filename="file.ext"
  */
