...
int main() {
  ...
  bench<Action_axpby<blas_interface<REAL_TYPE> > >(MIN_AXPY, MAX_AXPY,
                                                   NB_POINT);

  bench<Action_matrix_vector_product<blas_interface<REAL_TYPE> > >(
      MIN_MV, MAX_MV, NB_POINT);
  bench<Action_atv_product<blas_interface<REAL_TYPE> > >(MIN_MV, MAX_MV,
                                                         NB_POINT);
  ...
  bench<Action_trisolve_matrix<blas_interface<REAL_TYPE> > >(MIN_MM, MAX_MM,
                                                             NB_POINT);
  ...
  bench<Action_partial_lu<blas_interface<REAL_TYPE> > >(MIN_LU, MAX_LU,
                                                        NB_POINT);

#ifdef HAS_LAPACK
  //   bench<Action_lu_decomp<blas_interface<REAL_TYPE> >
  //   >(MIN_LU,MAX_LU,NB_POINT);
  bench<Action_hessenberg<blas_interface<REAL_TYPE> > >(MIN_LU, MAX_LU,
                                                        NB_POINT);
  bench<Action_tridiagonalization<blas_interface<REAL_TYPE> > >(MIN_LU, MAX_LU,
                                                                NB_POINT);
#endif

  // bench<Action_lu_solve<blas_LU_solve_interface<REAL_TYPE> >
  // >(MIN_LU,MAX_LU,NB_POINT);
