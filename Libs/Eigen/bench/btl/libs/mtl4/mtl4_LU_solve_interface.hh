...
class blitz_LU_solve_interface : public blitz_interface<real> {
 public:
  ...
  static inline real matrix_vector_product_sliced(const gene_matrix &A,
                                                  gene_vector B, int row,
  ...
  static inline real matrix_matrix_product_sliced(gene_matrix &A, int row,
                                                  int col_start, int col_end,
                                                  gene_matrix &B, int row_shift,
                                                  int col) {
    real somme = 0.;
  ...
  inline static void LU_solve(const gene_matrix &LU, const Pivot_Vector pivot,
                              gene_vector &B, gene_vector X, int N) {
    // Pour conserver le meme header, on travaille sur X, copie du second-membre
    // B
    X = B.copy();
    ASSERT(LU.rows() == LU.cols());
    firstIndex indI;
    ...
    for (int i = N - 1; i >= 0; i--) {
      ...
      //      ) ) ; theSum -= sum( LU( i, Range( i+1, toEnd ) )*B( Range( i+1,
      //      toEnd ) ) ) ;
