...
#define DUMP_CPUID(CODE)                                                    \
  {                                                                         \
    ...
    std::cout << "The code " << CODE << " gives " << (int*)(abcd[0]) << " " \
              << (int*)(abcd[1]) << " " << (int*)(abcd[2]) << " "           \
              << (int*)(abcd[3]) << " " << std::endl;                       \
  }
...
int main() {
  ...
  cout << "Eigen's intel codes L1, L2, L3 = " << l1 << " " << l2 << " " << l3
       << endl;
  if (max_funcs >= 4) {
    internal::queryCacheSizes_intel_direct(l1, l2, l3);
    cout << "Eigen's intel direct L1, L2, L3 = " << l1 << " " << l2 << " " << l3
         << endl;
  }
  internal::queryCacheSizes_amd(l1, l2, l3);
  cout << "Eigen's amd L1, L2, L3         = " << l1 << " " << l2 << " " << l3
       << endl;
  cout << endl;
