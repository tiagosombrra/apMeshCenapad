...
void bench(int id, int rows, int size = Size) {
  ...
  BenchTimer t_llt, t_ldlt, t_lu, t_fplu, t_qr, t_cpqr, t_cod, t_fpqr, t_jsvd,
      t_bdcsvd;
...
int main() {
  ...
  for (int i = 0; i < labels.size(); ++i) {
    ...
    for (int k = 0; k < sizes.size(); ++k) {
      if (r(k) >= 1e6)
        ...
        if (i > 0)
          cout << " (x"
               << numext::round(10.f * results[labels[i]](k) /
                                results["LLT"](k)) /
                      10.f
               << ")";
  ...
  //   cout << "LLT                             (ms)  " <<
  //   (results["LLT"]*1000.).format(fmt) << "\n"; cout << "LDLT (%)  " <<
  //   (results["LDLT"]/results["LLT"]).format(fmt) << "\n"; cout <<
  //   "PartialPivLU                     (%)  " <<
  //   (results["PartialPivLU"]/results["LLT"]).format(fmt) << "\n"; cout <<
  //   "FullPivLU                        (%)  " <<
  //   (results["FullPivLU"]/results["LLT"]).format(fmt) << "\n"; cout <<
  //   "HouseholderQR                    (%)  " <<
  //   (results["HouseholderQR"]/results["LLT"]).format(fmt) << "\n"; cout <<
  //   "ColPivHouseholderQR              (%)  " <<
  ...
  //   (results["FullPivHouseholderQR"]/results["LLT"]).format(fmt) << "\n";
  //   cout << "JacobiSVD                        (%)  " <<
  //   (results["JacobiSVD"]/results["LLT"]).format(fmt) << "\n"; cout <<
  //   "BDCSVD                           (%)  " <<
  //   (results["BDCSVD"]/results["LLT"]).format(fmt) << "\n";
}
