...
template <typename Q>
EIGEN_DONT_INLINE Q slerp_legacy_nlerp(const Q& a, const Q& b,
                                       typename Q::Scalar t) {
  typedef typename Q::Scalar Scalar;
...
EIGEN_DONT_INLINE Q slerp_rw(const Q& a, const Q& b, typename Q::Scalar t) {
  ...
  if (d < 0.0)
    theta =
        /*M_PI -*/ Scalar(2) * std::asin((a.coeffs() + b.coeffs()).norm() / 2);
  else
  ...
  Scalar scale0 =
      (Scalar(1) - t) * sin_over_x((Scalar(1) - t) * theta) / sinOverTheta;
...
EIGEN_DONT_INLINE Q slerp_gael(const Q& a, const Q& b, typename Q::Scalar t) {
  ...
  Scalar theta;
  //   theta = Scalar(2) *
  //   atan2((a.coeffs()-b.coeffs()).norm(),(a.coeffs()+b.coeffs()).norm()); if
  //   (d<0.0)
  //     theta = M_PI-theta;
...
int main() {
  ...
  cout << "double=>float=>double       nlerp        eigen        legacy(snap)  "
