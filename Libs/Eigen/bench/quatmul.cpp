...
EIGEN_DONT_INLINE void quatmul_novec(const Quat& a, const Quat& b, Quat& c) {
  c = internal::quat_product<0, Quat, Quat, typename Quat::Scalar,
                             Aligned>::run(a, b);
}
...
void bench(const std::string& label) {
  ...
  std::cout << label << " default " << 1e3 * t.best(CPU_TIMER) << "ms  \t"
            << 1e-6 * double(rep) / (t.best(CPU_TIMER)) << " M mul/s\n";

  BENCH(t, tries, rep, quatmul_novec(a, b, c));
  std::cout << label << " novec   " << 1e3 * t.best(CPU_TIMER) << "ms  \t"
            << 1e-6 * double(rep) / (t.best(CPU_TIMER)) << " M mul/s\n";
}
