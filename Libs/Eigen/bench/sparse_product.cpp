...
// ./a.out g++ -O3 -g0 -DNDEBUG  sparse_product.cpp -I..
...
// dense matrices
    ...
    {
      std::cout << "Eigen sparse\t"
                << sm1.nonZeros() / (float(sm1.rows()) * float(sm1.cols())) *
                       100
                << "% * "
                << sm2.nonZeros() / (float(sm2.rows()) * float(sm2.cols())) *
                       100
                << "%\n";
...
//       std::cout << sm1 << "\n\n" << sm1.transpose() << "\n\n" <<
sm4.transpose() << "\n\n"; BENCH(for (int k=0; k<REPEAT; ++k) m3 =
m1.transpose() * m2;)
//       timer.stop();
  std::cout << "   a' * b:\t" << timer.value() << endl;
...
// GMM++
#ifndef NOGMM
    {
      ...
      //       if (rows<500)
      //       {
      //         BENCH(gmm::mult(gmm::transposed(m1), gmm::transposed(m2),
      //         gmmT3);); std::cout << "   a' * b':\t" << timer.value() <<
      //         endl;
      //
