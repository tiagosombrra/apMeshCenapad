...
int EIGEN_BLAS_FUNC(axpy)(const int *n, const RealScalar *palpha,
                          const RealScalar *px, const int *incx, RealScalar *py,
                          const int *incy) {
...
int EIGEN_BLAS_FUNC(copy)(int *n, RealScalar *px, int *incx, RealScalar *py,
                          int *incy) {
  if (*n <= 0) return 0;
...
int EIGEN_CAT(EIGEN_CAT(i, SCALAR_SUFFIX), amax_)(int *n, RealScalar *px,
                                                  int *incx) {
  if (*n <= 0) return 0;
  Scalar *x = reinterpret_cast<Scalar *>(px);
...
int EIGEN_CAT(EIGEN_CAT(i, SCALAR_SUFFIX), amin_)(int *n, RealScalar *px,
                                                  int *incx) {
  if (*n <= 0) return 0;
  Scalar *x = reinterpret_cast<Scalar *>(px);
...
int EIGEN_BLAS_FUNC(rotg)(RealScalar *pa, RealScalar *pb, RealScalar *pc,
                          RealScalar *ps) {
  using std::abs;
  using std::sqrt;
...
int EIGEN_BLAS_FUNC(scal)(int *n, RealScalar *palpha, RealScalar *px,
                          int *incx) {
  if (*n <= 0) return 0;
...
int EIGEN_BLAS_FUNC(swap)(int *n, RealScalar *px, int *incx, RealScalar *py,
                          int *incy) {
  if (*n <= 0) return 0;
  ...
  if (*incx == 1 && *incy == 1)
    ...
    make_vector(y, *n, -*incy)
        .reverse()
        .swap(make_vector(x, *n, -*incx).reverse());
