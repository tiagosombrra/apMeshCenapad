...
void MandelbrotThread::render(int img_width, int img_height) {
  ...
  typedef Eigen::Array<Real, packetSize, 1>
      Packet;  // wrap a Packet as a vector
  ...
  Vector2 start(widget->center.x() - widget->xradius,
  ...
  for (int y = id; y < img_height; y += threadcount) {
    ...
    // for each pixel, we're going to do the iteration z := z^2 + c where z and
    // c are complex numbers, starting with z = c = complex coord of the pixel.
    // pzi and pzr denote the real and imaginary parts of z. pci and pcr denote
    // the real and imaginary parts of c.

    Packet pzi_start, pci_start;
    for (int i = 0; i < packetSize; i++)
      pzi_start[i] = pci_start[i] = start.y() + y * step.y();

    for (int x = 0; x < alignedWidth; x += packetSize, pix += packetSize) {
      Packet pcr, pci = pci_start, pzr, pzi = pzi_start, pzr_buf;
      for (int i = 0; i < packetSize; i++)
        pzr[i] = pcr[i] = start.x() + (x + i) * step.x();

      // do the iterations. Every iters_before_test iterations we check for
      // divergence, in which case we can stop iterating.
      int j = 0;
      typedef Eigen::Matrix<int, packetSize, 1> Packeti;
      Packeti
          pix_iter =
              Packeti::Zero(),  // number of iteration per pixel in the packet
          pix_dont_diverge;  // whether or not each pixel has already diverged
      do {
        for (int i = 0; i < iters_before_test / 4;
             i++)  // peel the inner loop by 4
        {
#define ITERATE              \
  pzr_buf = pzr;             \
  pzr = pzr.square();        \
  ...
  pzi += pci;
        ...
        pix_dont_diverge =
            ((pzr.square() + pzi.square())
                 .eval()  // temporary fix as what follows is not yet vectorized
                          // by Eigen
             <= Packet::Constant(4))
...
void MandelbrotWidget::paintEvent(QPaintEvent *) {
  ...
  if (draft == 1) {
    ...
    std::cout << threadcount << " threads, " << elapsed << " ms, " << speed
              << " iters/s (max " << max_speed << ")" << std::endl;
    ...
    setWindowTitle(
        QString("resolution ") +
        QString::number(xradius * 2 / width(), 'e', 2) +
        QString(", %1 iterations per pixel, ").arg(threads[0]->max_iter) +
        (threads[0]->single_precision ? QString("single ")
                                      : QString("double ")) +
        QString("precision, ") +
        (packetSize == 1
             ? QString("no vectorization")
