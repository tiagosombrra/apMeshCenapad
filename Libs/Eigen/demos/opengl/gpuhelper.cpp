...
void GpuHelper::drawVector(const Vector3f& position, const Vector3f& vec,
                           const Color& color, float aspect /* = 50.*/) {
...
void GpuHelper::drawVectorBox(const Vector3f& position, const Vector3f& vec,
                              const Color& color, float aspect) {
  static GLUquadricObj* cylindre = gluNewQuadric();
  glColor4fv(color.data());
  float length = vec.norm();
  pushMatrix(GL_MODELVIEW);
...
void GpuHelper::drawUnitCube(void) {
  static float vertices[][3] = {{-0.5, -0.5, -0.5}, {0.5, -0.5, -0.5},
