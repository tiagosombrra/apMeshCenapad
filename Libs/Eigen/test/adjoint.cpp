...
struct adjoint_specific<true> {
  ...
  static void run(const Vec& v1, const Vec& v2, Vec& v3, const Mat& square,
                  Scalar s1, Scalar s2) {
    VERIFY(test_isApproxWithRef(
        (s1 * v1 + s2 * v2).dot(v3),
        numext::conj(s1) * v1.dot(v3) + numext::conj(s2) * v2.dot(v3), 0));
    VERIFY(test_isApproxWithRef(v3.dot(s1 * v1 + s2 * v2),
                                s1 * v3.dot(v1) + s2 * v3.dot(v2), 0));

    // check compatibility of dot and adjoint
    VERIFY(test_isApproxWithRef(v1.dot(square * v2),
                                (square.adjoint() * v1).dot(v2), 0));
  }
};
...
struct adjoint_specific<false> {
  ...
  static void run(const Vec& v1, const Vec& v2, Vec& v3, const Mat& square,
    ...
    RealScalar ref = NumTraits<Scalar>::IsInteger
                         ? RealScalar(0)
                         : (std::max)((s1 * v1 + s2 * v2).norm(), v3.norm());
    VERIFY(test_isApproxWithRef(
        (s1 * v1 + s2 * v2).dot(v3),
        numext::conj(s1) * v1.dot(v3) + numext::conj(s2) * v2.dot(v3), ref));
    VERIFY(test_isApproxWithRef(v3.dot(s1 * v1 + s2 * v2),
                                s1 * v3.dot(v1) + s2 * v3.dot(v2), ref));
    ...
    VERIFY(internal::isMuchSmallerThan(
        abs(v1.dot(square * v2) - (square.adjoint() * v1).dot(v2)), ref,
        test_precision<Scalar>()));

    // check that Random().normalized() works: tricky as the random xpr must be
    // evaluated by normalized() in order to produce a consistent result.
...
void adjoint(const MatrixType& m) {
  ...
  */
  ...
  VectorType v1 = VectorType::Random(rows), v2 = VectorType::Random(rows),
             v3 = VectorType::Random(rows), vzero = VectorType::Zero(rows);
  ...
  adjoint_specific<NumTraits<Scalar>::IsInteger>::run(v1, v2, v3, square, s1,
                                                      s2);
  ...
  if (PacketSize < m3.rows() && PacketSize < m3.cols()) {
    ...
    VERIFY_IS_APPROX(
        (m3.template block<PacketSize, PacketSize>(i, j)),
...
void test_adjoint() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_5(
        adjoint(MatrixXi(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                         internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_6(
        adjoint(MatrixXf(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                         internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
