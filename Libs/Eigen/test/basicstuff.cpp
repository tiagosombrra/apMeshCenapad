...
void basicStuff(const MatrixType& m) {
  ...
  MatrixType m1 = MatrixType::Random(rows, cols),
             ...
             square = Matrix<Scalar, MatrixType::RowsAtCompileTime,
                             MatrixType::RowsAtCompileTime>::Random(rows, rows);
  ...
  // hence has no _write() method, the corresponding MatrixBase method (here
  // zero()) should return a const-qualified object so that it is the
  // const-qualified operator() that gets called, which in turn calls _read().
  VERIFY_IS_MUCH_SMALLER_THAN(MatrixType::Zero(rows, cols)(r, c),
  ...
  m3.real() = m1.real();
  VERIFY_IS_APPROX(static_cast<const MatrixType&>(m3).real(),
                   static_cast<const MatrixType&>(m1).real());
...
void basicStuffComplex(const MatrixType& m) {
  ...
  // extended precision in Intel FPUs means that s1 == s2 in the line above is
  // not guaranteed.
...
void fixedSizeMatrixConstruction() {
  ...
  }
  {
    Matrix<Scalar, 1, 2> m(raw), m2((DenseIndex(raw[0])), (DenseIndex(raw[1]))),
        m3((int(raw[0])), (int(raw[1]))), m4((float(raw[0])), (float(raw[1])));
...
void test_basicstuff() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_3(
        basicStuff(MatrixXcf(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                             internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_4(
        basicStuff(MatrixXi(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                            internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_5(
        basicStuff(MatrixXcd(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                             internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    ...
    CALL_SUBTEST_5(basicStuffComplex(
        MatrixXcd(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                  internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
  }
