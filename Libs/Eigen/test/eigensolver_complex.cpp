...
bool find_pivot(typename MatrixType::Scalar tol, MatrixType& diffs,
                Index col = 0) {
  ...
  if (match || col == diffs.cols()) {
    ...
    for (Index i = col; i < n; ++i) {
      ...
      transpositions.push_back(
          std::pair<Index, Index>(n - (i - col) - 1, best_index));
    }
    // restore
    for (Index k = transpositions.size() - 1; k >= 0; --k)
      diffs.row(transpositions[k].first)
          .swap(diffs.row(transpositions[k].second));
  }
  return false;
}

/* Check that two column vectors are approximately equal upto permutations.
 * Initially, this method checked that the k-th power sums are equal for all k =
 * 1, ..., vec1.rows(), however this strategy is numerically inacurate because
 * of numerical cancellation issues.
 */
...
void verify_is_approx_upto_permutation(const VectorType& vec1,
  ...
  Matrix<RealScalar, Dynamic, Dynamic> diffs =
      (vec1.rowwise().replicate(n) - vec2.rowwise().replicate(n).transpose())
          .cwiseAbs2();
...
void eigensolver(const MatrixType& m) {
  ...
  */
  ...
  VERIFY_IS_APPROX(symmA * ei0.eigenvectors(),
                   ei0.eigenvectors() * ei0.eigenvalues().asDiagonal());

  ComplexEigenSolver<MatrixType> ei1(a);
  VERIFY_IS_EQUAL(ei1.info(), Success);
  VERIFY_IS_APPROX(a * ei1.eigenvectors(),
