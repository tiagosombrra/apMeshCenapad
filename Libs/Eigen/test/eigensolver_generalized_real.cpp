...
void generalized_eigensolver_real(const MatrixType& m) {
  ...
  */
  ...
  // lets compare to GeneralizedSelfAdjointEigenSolver
  {
    ...
    std::sort(realEigenvalues.data(),
              realEigenvalues.data() + realEigenvalues.size());
    VERIFY_IS_APPROX(realEigenvalues, symmEig.eigenvalues());

    // check eigenvectors
    typename GeneralizedEigenSolver<MatrixType>::EigenvectorsType D =
        eig.eigenvalues().asDiagonal();
    typename GeneralizedEigenSolver<MatrixType>::EigenvectorsType V =
        eig.eigenvectors();
  ...
  // non symmetric case:
  {
    ...
    // TODO enable full-prealocation of required memory, this probably requires
    // an in-place mode for HessenbergDecomposition
    // Eigen::internal::set_is_malloc_allowed(false);
    eig.compute(a, b);
    // Eigen::internal::set_is_malloc_allowed(true);
    for (Index k = 0; k < cols; ++k) {
      Matrix<ComplexScalar, Dynamic, Dynamic> tmp =
          (eig.betas()(k) * a).template cast<ComplexScalar>() -
          eig.alphas()(k) * b;
  ...
  {
    GeneralizedSelfAdjointEigenSolver<MatrixType> eig1(a.adjoint() * a,
                                                       b.adjoint() * b);
