...
EIGEN_STRONG_INLINE DstXprType& copy_using_evaluator(
    const EigenBase<DstXprType>& dst, const SrcXprType& src) {
...
template <typename DstXprType, template <typename> class StorageBase,
          typename SrcXprType>
EIGEN_STRONG_INLINE const DstXprType& copy_using_evaluator(
    const NoAlias<DstXprType, StorageBase>& dst, const SrcXprType& src) {
...
template <typename DstXprType, typename SrcXprType>
EIGEN_STRONG_INLINE DstXprType& copy_using_evaluator(
    const PlainObjectBase<DstXprType>& dst, const SrcXprType& src) {
#ifdef EIGEN_NO_AUTOMATIC_RESIZING
  eigen_assert((dst.size() == 0 ||
                (IsVectorAtCompileTime ? (dst.size() == src.size())
                                       : (dst.rows() == src.rows() &&
                                          dst.cols() == src.cols()))) &&
...
void add_assign_using_evaluator(const DstXprType& dst, const SrcXprType& src) {
  ...
  call_assignment(
      const_cast<DstXprType&>(dst), src.derived(),
      internal::add_assign_op<Scalar, typename SrcXprType::Scalar>());
}

template <typename DstXprType, typename SrcXprType>
void subtract_assign_using_evaluator(const DstXprType& dst,
...
template <typename DstXprType, typename SrcXprType>
void multiply_assign_using_evaluator(const DstXprType& dst,
                                     const SrcXprType& src) {
  typedef typename DstXprType::Scalar Scalar;
  call_assignment(
...
template <typename DstXprType, typename SrcXprType>
void divide_assign_using_evaluator(const DstXprType& dst,
                                   const SrcXprType& src) {
  typedef typename DstXprType::Scalar Scalar;
  call_assignment(
      dst.const_cast_derived(), src.derived(),
...
void swap_using_evaluator(const DstXprType& dst, const SrcXprType& src) {
  typedef typename DstXprType::Scalar Scalar;
  call_assignment(dst.const_cast_derived(), src.const_cast_derived(),
...
template <typename Dst, template <typename> class StorageBase, typename Src,
          typename Func>
EIGEN_DEVICE_FUNC void call_assignment(const NoAlias<Dst, StorageBase>& dst,
...
void test_evaluators() {
  ...
  // Testing Array evaluator
  {
    ...
    VERIFY((w.array() >= -1).all() &&
           (w.array() <= 1).all());  // not easy to test ...
...

  {
    ...
    VERIFY_IS_APPROX_EVALUATOR(w, (v + Vector2d::Ones())
                                      .transpose()
                                      .cwiseProduct(RowVector2d::Constant(3)));
    ...
    VERIFY_IS_APPROX_EVALUATOR(m3.transpose(),
                               Matrix3f::Identity().transpose());  // transpose
    VERIFY_IS_APPROX_EVALUATOR(m3, 2 * Matrix3f::Identity());      // unary
    VERIFY_IS_APPROX_EVALUATOR(
        m3, Matrix3f::Identity() + Matrix3f::Zero());  // binary
    VERIFY_IS_APPROX_EVALUATOR(
        m3.block(0, 0, 2, 2), Matrix3f::Identity().block(1, 1, 2, 2));  // block
    ...
    VERIFY_IS_APPROX_EVALUATOR(m3.transpose(),
                               Matrix3f::Zero().transpose());  // transpose
    ...
    VERIFY_IS_APPROX_EVALUATOR(
        aX, ArrayXXf::Constant(10, 10, 3.0).block(2, 3, 6, 6));
...

  {
    ...
    A.setRandom();
    B.setRandom();
    VERIFY_IS_APPROX_EVALUATOR2(B, A.triangularView<Upper>(),
                                MatrixXd(A.triangularView<Upper>()));

    A.setRandom();
    B.setRandom();
    VERIFY_IS_APPROX_EVALUATOR2(B, A.triangularView<UnitLower>(),
                                MatrixXd(A.triangularView<UnitLower>()));

    A.setRandom();
    B.setRandom();
    VERIFY_IS_APPROX_EVALUATOR2(B, A.triangularView<UnitUpper>(),
                                MatrixXd(A.triangularView<UnitUpper>()));
    ...
    VERIFY(B.isApprox(C) &&
           "copy_using_evaluator(B.triangularView<Lower>(), "
    ...
    copy_using_evaluator(B.triangularView<Lower>(),
                         A.triangularView<Upper>().transpose());
    VERIFY(B.isApprox(C) &&
           "copy_using_evaluator(B.triangularView<Lower>(), "
    ...
    VERIFY(B.isApprox(C) &&
           "swap_using_evaluator(B.triangularView<Upper>(), "
           "A.triangularView<Upper>())");

    VERIFY_IS_APPROX_EVALUATOR2(B, prod(A.triangularView<Upper>(), A),
                                MatrixXd(A.triangularView<Upper>() * A));

    VERIFY_IS_APPROX_EVALUATOR2(B, prod(A.selfadjointView<Upper>(), A),
                                MatrixXd(A.selfadjointView<Upper>() * A));
  }

  {
    ...
    VERIFY_IS_APPROX_EVALUATOR2(B, lazyprod(d.asDiagonal(), A),
                                MatrixXd(d.asDiagonal() * A));
    VERIFY_IS_APPROX_EVALUATOR2(B, lazyprod(A, d.asDiagonal()),
                                MatrixXd(A * d.asDiagonal()));
  }
