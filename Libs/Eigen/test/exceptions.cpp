...
#define CHECK_MEMLEAK(OP)                                         \
  {                                                               \
    ...
    try {                                                         \
      ...
      VERIFY(ScalarWithExceptions::instances == before &&         \
             "memory leak detected in " && EIGEN_MAKESTRING(OP)); \
    }                                                             \
    VERIFY(exception_thrown && " no exception thrown in " &&      \
           EIGEN_MAKESTRING(OP));                                 \
  }
...
void memoryleak() {
  ...
  VERIFY(ScalarWithExceptions::instances == 0 &&
         "global memory leak detected in " && EIGEN_MAKESTRING(OP));
}

void test_exceptions() {
  EIGEN_TRY { CALL_SUBTEST(memoryleak()); }
  EIGEN_CATCH(...) {}
}
