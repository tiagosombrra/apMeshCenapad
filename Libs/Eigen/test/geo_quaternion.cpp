...
void quaternion(void) {
  ...
  */
  ...
  if ((q1.coeffs() - q2.coeffs()).norm() > 10 * largeEps) {
    VERIFY_IS_MUCH_SMALLER_THAN(abs(q1.angularDistance(q2) - refangle),
                                Scalar(1));
  }
  ...
  VERIFY((q2 * q1).isApprox(q1 * q2, largeEps) ||
         !(q2 * q1 * v2)
              .isApprox(q1.toRotationMatrix() * q2.toRotationMatrix() * v2));
  ...
  if (abs(aa.angle()) > 5 * test_precision<Scalar>() &&
    ...
    VERIFY_IS_NOT_APPROX(
        q1 * v1, Quaternionx(AngleAxisx(aa.angle() * 2, aa.axis())) * v1);
  }

  // from two vector creation
  VERIFY_IS_APPROX(v2.normalized(),
                   (q2.setFromTwoVectors(v1, v2) * v1).normalized());
  ...
  if (internal::is_same<Scalar, double>::value) {
    ...
    VERIFY_IS_APPROX(v3.normalized(),
                     (q2.setFromTwoVectors(v1, v3) * v1).normalized());
    VERIFY_IS_APPROX(-v3.normalized(),
                     (q2.setFromTwoVectors(v1, -v3) * v1).normalized());
  }
  ...
  VERIFY_IS_APPROX(v2.normalized(),
                   (Quaternionx::FromTwoVectors(v1, v2) * v1).normalized());
  VERIFY_IS_APPROX(v1.normalized(),
  ...
  if (internal::is_same<Scalar, double>::value) {
    v3 = (v1.array() + eps).matrix();
    VERIFY_IS_APPROX(v3.normalized(),
                     (Quaternionx::FromTwoVectors(v1, v3) * v1).normalized());
    VERIFY_IS_APPROX(-v3.normalized(),
                     (Quaternionx::FromTwoVectors(v1, -v3) * v1).normalized());
  }
...
void mapQuaternion(void) {
  ...
  // Bug 1461, compilation issue with Map<const Quat>::w(), and other
  // reference/constness checks:
  VERIFY_IS_APPROX(mcq3.coeffs().x() + mcq3.coeffs().y() + mcq3.coeffs().z() +
                       mcq3.coeffs().w(),
                   mcq3.coeffs().sum());
  VERIFY_IS_APPROX(mcq3.x() + mcq3.y() + mcq3.z() + mcq3.w(),
                   mcq3.coeffs().sum());
  mq3.w() = 1;
  ...
  VERIFY(&cmq3.x() == &mq3.x());
  // FIXME the following should be ok. The problem is that currently the
  // LValueBit flag is used to determine wether we can return a coeff by
  // reference or not, which is not enough for Map<const ...>.
...
void quaternionAlignment(void) {
  ...
  QuaternionUA* q3 =
      ::new (reinterpret_cast<void*>(arrayunaligned)) QuaternionUA;
  ...
  *q3 = *q1;
  ...
  if (internal::packet_traits<Scalar>::Vectorizable &&
      internal::packet_traits<Scalar>::size <= 4)
...
void check_const_correctness(const PlainObjectType&) {
  ...
  // the only possible approach would be to run a script trying to compile stuff
  // and checking that it fails. CMake can help with that.

  // verify that map-to-const don't have LvalueBit
  typedef
      typename internal::add_const<PlainObjectType>::type ConstPlainObjectType;
  VERIFY(!(internal::traits<Map<ConstPlainObjectType> >::Flags & LvalueBit));
  VERIFY(!(internal::traits<Map<ConstPlainObjectType, Aligned> >::Flags &
           LvalueBit));
