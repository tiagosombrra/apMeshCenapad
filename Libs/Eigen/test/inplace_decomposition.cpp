...
// This file test inplace decomposition through Ref<>, as supported by Cholesky,
...
void inplace(bool square = false, bool SPD = false) {
  ...
  Index rows = MatrixType::RowsAtCompileTime == Dynamic
                   ? internal::random<Index>(2, EIGEN_TEST_MAX_SIZE / 2)
                   : Index(MatrixType::RowsAtCompileTime);
  Index cols = MatrixType::ColsAtCompileTime == Dynamic
                   ? (square ? rows : internal::random<Index>(2, rows))
  ...
  if (rows == cols) {
    ...
    VERIFY_IS_APPROX(A0.transpose() * A0 * (x = dec.solve(b)),
                     A0.transpose() * b);
  }
  ...
  if (rows == cols) {
    ...
    VERIFY_IS_NOT_APPROX(A0.transpose() * A0 * (x = dec.solve(b)),
                         A0.transpose() * b);
  }
...
void test_inplace_decomposition() {
  ...
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_6((inplace<ColPivHouseholderQR<Ref<Matrix43d> >, Matrix43d>(
        false, false)));

    CALL_SUBTEST_7((
        inplace<FullPivHouseholderQR<Ref<MatrixXd> >, MatrixXd>(false, false)));
    CALL_SUBTEST_7((inplace<FullPivHouseholderQR<Ref<Matrix43d> >, Matrix43d>(
        false, false)));

    CALL_SUBTEST_8(
        (inplace<CompleteOrthogonalDecomposition<Ref<MatrixXd> >, MatrixXd>(
            false, false)));
    CALL_SUBTEST_8(
        (inplace<CompleteOrthogonalDecomposition<Ref<Matrix43d> >, Matrix43d>(
            false, false)));
  }
}
