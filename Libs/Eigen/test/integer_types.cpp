...
void signed_integer_type_tests(const MatrixType& m) {
  ...
  MatrixType m1(rows, cols), m2 = MatrixType::Random(rows, cols),
...
void integer_type_tests(const MatrixType& m) {
  ...
  VectorType v1(rows), v2 = VectorType::Random(rows),
  ...
  VERIFY_IS_APPROX((m1 + m2).transpose() * square,
                   m1.transpose() * square + m2.transpose() * square);
...
void test_integer_types() {
  ...
  if (sizeof(long) > sizeof(int)) {
    VERIFY(int(internal::scalar_div_cost<long>::value) >
           int(internal::scalar_div_cost<int>::value));
    VERIFY(int(internal::scalar_div_cost<unsigned long>::value) >
           int(internal::scalar_div_cost<int>::value));
  }
#endif
}
