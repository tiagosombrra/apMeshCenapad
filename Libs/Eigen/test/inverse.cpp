...
void inverse(const MatrixType& m) {
  ...
  */
  ...
  // since for the general case we implement separately row-major and col-major,
  // test that
  VERIFY_IS_APPROX(MatrixType(m1.transpose().inverse()),
                   MatrixType(m1.inverse().transpose()));
  ...
  // check with submatrices
  {
    Matrix<Scalar, MatrixType::RowsAtCompileTime + 1,
           MatrixType::RowsAtCompileTime + 1, MatrixType::Options>
        m5;
    m5.setRandom();
    m5.topLeftCorner(rows, rows) = m1;
    m2 = m5.template topLeftCorner<MatrixType::RowsAtCompileTime,
                                   MatrixType::ColsAtCompileTime>()
             .inverse();
    VERIFY_IS_APPROX(
        (m5.template topLeftCorner<MatrixType::RowsAtCompileTime,
                                   MatrixType::ColsAtCompileTime>()),
        m2.inverse());
  }
#endif
  ...
  if (MatrixType::RowsAtCompileTime >= 2 &&
