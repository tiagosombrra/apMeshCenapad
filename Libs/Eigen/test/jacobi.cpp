...
void jacobi(const MatrixType& m = MatrixType()) {
...

  {
    ...
    VERIFY_IS_APPROX(b.col(q),
                     numext::conj(s) * a.col(p) + numext::conj(c) * a.col(q));
  }
}
...
void test_jacobi() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    int r = internal::random<int>(
            2, internal::random<int>(1, EIGEN_TEST_MAX_SIZE) / 2),
        c = internal::random<int>(
            2, internal::random<int>(1, EIGEN_TEST_MAX_SIZE) / 2);
    ...
    // complex<float> is really important to test as it is the only way to cover
    // conjugation issues in certain unaligned paths
