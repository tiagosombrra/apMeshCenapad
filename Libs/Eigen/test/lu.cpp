...
void lu_non_invertible() {
  ...
  */
  ...
  typedef
      typename internal::kernel_retval_base<FullPivLU<MatrixType> >::ReturnType
          KernelMatrixType;
  typedef
      typename internal::image_retval_base<FullPivLU<MatrixType> >::ReturnType
          ImageMatrixType;
  typedef Matrix<typename MatrixType::Scalar, ColsAtCompileTime,
                 ColsAtCompileTime>
      CMatrixType;
  typedef Matrix<typename MatrixType::Scalar, RowsAtCompileTime,
                 RowsAtCompileTime>
      RMatrixType;

  Index rank = internal::random<Index>(1, (std::min)(rows, cols) - 1);

  // The image of the zero matrix should consist of a single (zero) column
  // vector
  VERIFY((MatrixType::Zero(rows, cols)
              .fullPivLu()
              .image(MatrixType::Zero(rows, cols))
              .cols() == 1));

  // The kernel of the zero matrix is the entire space, and thus is an
  // invertible matrix of dimensions cols.
  ...
  // The special value 0.01 below works well in tests. Keep in mind that we're
  // only computing the rank of singular values are either 0 or 1. So it's not
  // clear at all that the epsilon should play any role there.
  ...
  l.block(0, 0, rows, (std::min)(rows, cols))
...
void lu_invertible() {
  ...
  */
  ...
  VERIFY(lu.kernel().cols() ==
         1);  // the kernel() should consist of a single (zero) column vector
  VERIFY(size == lu.rank());
  VERIFY(lu.isInjective());
  VERIFY(lu.isSurjective());
  VERIFY(lu.isInvertible());
  ...
  RealScalar rcond =
      (RealScalar(1) / matrix_l1_norm(m1)) / matrix_l1_norm(m1_inverse);
