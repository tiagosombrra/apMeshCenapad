...
// Unit tests calling Eigen's blas library must preserve the default blocking
// size to avoid troubles.
...
// On windows CE, NDEBUG is automatically defined <assert.h> if NDEBUG is not
// defined.
#ifndef DEBUG
#define DEBUG
#endif
...
#define eigen_assert(a)                                            \
  if ((!(a)) && (!no_more_assert)) {                               \
    ...
    eigen_assert_list.push_back(std::string(EI_PP_MAKE_STRING(     \
        __FILE__) " (" EI_PP_MAKE_STRING(__LINE__) ") : " #a));    \
  }
...
#define VERIFY_RAISES_ASSERT(a)                                            \
  {                                                                        \
    ...
    try {                                                                  \
      a;                                                                   \
      std::cerr                                                            \
          << "One of the following asserts should have been triggered:\n"; \
...
#define VERIFY_RAISES_ASSERT(a)                  \
  std::cout << "Can't VERIFY_RAISES_ASSERT( " #a \
               " ) with exceptions disabled\n";
#endif
#ifndef VERIFY_RAISES_STATIC_ASSERT
#define VERIFY_RAISES_STATIC_ASSERT(a)                  \
  std::cout << "Can't VERIFY_RAISES_STATIC_ASSERT( " #a \
               " ) with exceptions disabled\n";
#endif
...
#define VERIFY(a)                                                   \
  ::verify_impl(a, g_test_stack.back().c_str(), __FILE__, __LINE__, \
                EI_PP_MAKE_STRING(a))

#define VERIFY_GE(a, b)                                                  \
  ::verify_impl(a >= b, g_test_stack.back().c_str(), __FILE__, __LINE__, \
                EI_PP_MAKE_STRING(a >= b))
...
#define VERIFY_IS_NOT_APPROX_OR_LESS_THAN(a, b) \
  VERIFY(!test_isApproxOrLessThan(a, b))
...
inline bool test_isApprox(const std::complex<float>& a,
                          const std::complex<float>& b) {
  return internal::isApprox(a, b, test_precision<std::complex<float> >());
}
inline bool test_isMuchSmallerThan(const std::complex<float>& a,
                                   const std::complex<float>& b) {
  return internal::isMuchSmallerThan(a, b,
                                     test_precision<std::complex<float> >());
}

inline bool test_isApprox(const std::complex<double>& a,
                          const std::complex<double>& b) {
  return internal::isApprox(a, b, test_precision<std::complex<double> >());
}
inline bool test_isMuchSmallerThan(const std::complex<double>& a,
                                   const std::complex<double>& b) {
  return internal::isMuchSmallerThan(a, b,
...
inline bool test_isApprox(const std::complex<long double>& a,
                          const std::complex<long double>& b) {
  return internal::isApprox(a, b, test_precision<std::complex<long double> >());
}
inline bool test_isMuchSmallerThan(const std::complex<long double>& a,
                                   const std::complex<long double>& b) {
  return internal::isMuchSmallerThan(
      a, b, test_precision<std::complex<long double> >());
}
#endif
#endif
...
inline bool test_isApproxOrLessThan(const long double& a,
                                    const long double& b) {
...
// test_relative_error returns the relative difference between a and b as a real
// scalar as used in isApprox.
template <typename T1, typename T2>
typename NumTraits<typename T1::RealScalar>::NonInteger test_relative_error(
  ...
  return sqrt(
      RealScalar((ea - eb).cwiseAbs2().sum()) /
      RealScalar((std::min)(eb.cwiseAbs2().sum(), ea.cwiseAbs2().sum())));
}

template <typename T1, typename T2>
typename T1::RealScalar test_relative_error(
    const T1& a, const T2& b, const typename T1::Coefficients* = 0) {
  return test_relative_error(a.coeffs(), b.coeffs());
}

template <typename T1, typename T2>
typename T1::Scalar test_relative_error(const T1& a, const T2& b,
...
S test_relative_error(const ParametrizedLine<S, D, O>& a,
                      const ParametrizedLine<S, D, O>& b) {
  return (std::max)(test_relative_error(a.origin(), b.origin()),
                    test_relative_error(a.origin(), b.origin()));
}

template <typename S, int D>
S test_relative_error(const AlignedBox<S, D>& a, const AlignedBox<S, D>& b) {
  return (std::max)(test_relative_error((a.min)(), (b.min)()),
                    test_relative_error((a.max)(), (b.max)()));
}

template <typename Derived>
class SparseMatrixBase;
template <typename T1, typename T2>
typename T1::RealScalar test_relative_error(const MatrixBase<T1>& a,
...
class SparseMatrixBase;
template <typename T1, typename T2>
typename T1::RealScalar test_relative_error(const SparseMatrixBase<T1>& a,
...
test_relative_error(
    ...
    typename internal::enable_if<
        internal::is_arithmetic<typename NumTraits<T1>::Real>::value,
        T1>::type* = 0) {
  typedef
      typename NumTraits<typename NumTraits<T1>::Real>::NonInteger RealScalar;
  return numext::sqrt(
      RealScalar(numext::abs2(a - b)) /
      RealScalar((numext::mini)(numext::abs2(a), numext::abs2(b))));
}
...
T test_relative_error(const AngleAxis<T>& a, const AngleAxis<T>& b) {
  return (std::max)(test_relative_error(a.angle(), b.angle()),
                    test_relative_error(a.axis(), b.axis()));
}

template <typename Type1, typename Type2>
inline bool test_isApprox(
...
// get_test_precision is a small wrapper to test_precision allowing to return
// the scalar precision for either scalars or expressions
template <typename T>
typename NumTraits<typename T::Scalar>::Real get_test_precision(
    const T&, const typename T::Scalar* = 0) {
...
typename NumTraits<T>::Real get_test_precision(
    const T&,
    typename internal::enable_if<
        internal::is_arithmetic<typename NumTraits<T>::Real>::value, T>::type* =
        0) {
  return test_precision<typename NumTraits<T>::Real>();
}

// verifyIsApprox is a wrapper to test_isApprox that outputs the relative
// difference magnitude if the test fails.
template <typename Type1, typename Type2>
inline bool verifyIsApprox(const Type1& a, const Type2& b) {
  ...
  if (!ret) {
    std::cerr << "Difference too large wrt tolerance " << get_test_precision(a)
              << ", relative error is: " << test_relative_error(a, b)
              << std::endl;
  }
  return ret;
}
...
// However, it seems that simply comparing a+ref and b+ref is more sensitive to
// true error.
template <typename Scalar, typename ScalarRef>
inline bool test_isApproxWithRef(const Scalar& a, const Scalar& b,
                                 const ScalarRef& ref) {
...
template <typename Derived>
inline bool test_isMuchSmallerThan(
    const MatrixBase<Derived>& m,
    const typename NumTraits<typename internal::traits<Derived>::Scalar>::Real&
        s) {
  return m.isMuchSmallerThan(
...
inline bool test_isUnitary(const MatrixBase<Derived>& m) {
  return m.isUnitary(
      test_precision<typename internal::traits<Derived>::Scalar>());
}
 ...
 *
 * A partial isometry is a matrix all of whose singular values are either 0
 * or 1. This is very useful to test rank-revealing algorithms.
 */
// Forward declaration to avoid ICC warning
template <typename MatrixType>
void createRandomPIMatrixOfRank(Index desired_rank, Index rows, Index cols,
                                MatrixType& m);
template <typename MatrixType>
void createRandomPIMatrixOfRank(Index desired_rank, Index rows, Index cols,
                                MatrixType& m) {
  ...
  if (desired_rank == 1) {
    ...
    m = VectorType::Random(rows).normalized() *
        VectorType::Random(cols).normalized().transpose();
    return;
  }
  ...
  if (diag_size != desired_rank)
    d.diagonal().segment(desired_rank, diag_size - desired_rank) =
...
int main(int argc, char* argv[]) {
  ...
  if (need_help) {
    std::cout << "This test application takes the following optional arguments:"
              << std::endl;
    std::cout << "  rN     Repeat each test N times (default: "
              << DEFAULT_REPEAT << ")" << std::endl;
    std::cout << "  sN     Use N as seed for random numbers (default: based on "
                 "current time)"
              << std::endl;
    std::cout << std::endl;
    std::cout
        << "If defined, the environment variables EIGEN_REPEAT and EIGEN_SEED"
        << std::endl;
    std::cout << "will be used as default values for these parameters."
              << std::endl;
    return 1;
  }
  ...
  Eigen::g_test_stack.push_back(
      std::string(EI_PP_MAKE_STRING(EIGEN_TEST_FUNC)));

  EIGEN_CAT(test_, EIGEN_TEST_FUNC)();
  return 0;
}

// These warning are disabled here such that they are still ON when parsing
// Eigen's header files.
#if defined __INTEL_COMPILER
// remark #383: value copied to temporary, reference to temporary used
//  -> this warning is raised even for legal usage as:
//  g_test_stack.push_back("foo"); where g_test_stack is a
//  std::vector<std::string>
// remark #1418: external function definition with no prior declaration
//  -> this warning is raised for all our test functions. Declaring them static
//  would fix the issue.
