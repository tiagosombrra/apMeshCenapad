...
void map_class_vector(const VectorType& m) {
  ...
  Scalar* array3unaligned =
      (internal::UIntPtr(array3) % EIGEN_MAX_ALIGN_BYTES) == 0 ? array3 + 1
                                                               : array3;
  Scalar array4[EIGEN_TESTMAP_MAX_SIZE];

  Map<VectorType, AlignedMax>(array1, size) = VectorType::Random(size);
  Map<VectorType, AlignedMax>(array2, size) =
...
void map_class_matrix(const MatrixType& m) {
  ...
  Scalar* array3 = new Scalar[size + 1];
  Index sizep1 =
      size + 1;  // <- without this temporary MSVC 2103 generates bad code
...
void check_const_correctness(const PlainObjectType&) {
  ...
  VERIFY(!(internal::traits<Map<ConstPlainObjectType, AlignedMax> >::Flags &
           LvalueBit));
...
void map_not_aligned_on_scalar() {
  ...
  Scalar* array2 =
      reinterpret_cast<Scalar*>(sizeof(Scalar) / 2 + std::size_t(array1));
  Map<MatrixType, 0, OuterStride<> > map2(array2, size, size,
                                          OuterStride<>(size + 1));
...
void test_mapped_matrix() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_4(map_class_matrix(
        MatrixXcf(internal::random<int>(1, 10), internal::random<int>(1, 10))));
    CALL_SUBTEST_5(map_class_matrix(
