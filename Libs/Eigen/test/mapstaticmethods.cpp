...
struct mapstaticmethods_impl<PlainObjectType, false, IsVector> {
  static void run(const PlainObjectType& m) {
    ...
    PlainObjectType::MapAligned(const_ptr, Stride<Dynamic, Dynamic>(i, j))
        .sum();
...
struct mapstaticmethods_impl<PlainObjectType, true, false> {
  static void run(const PlainObjectType& m) {
    ...
    PlainObjectType::Map(ptr, rows, cols, Stride<Dynamic, Dynamic>(i, j))
        .setZero();
    PlainObjectType::MapAligned(ptr, rows, cols, Stride<2, Dynamic>(2, i))
        .setZero();
    PlainObjectType::Map(const_ptr, rows, cols, Stride<Dynamic, 3>(i, 3)).sum();
    PlainObjectType::MapAligned(const_ptr, rows, cols,
                                Stride<Dynamic, Dynamic>(i, j))
        .sum();
