...
// NOTE: we disbale inlining for this function to workaround a GCC issue when
// using -O3 and the i387 FPU.
template <typename Scalar>
EIGEN_DONT_INLINE bool isApproxAbs(
...
template <typename Scalar>
bool areApproxAbs(const Scalar* a, const Scalar* b, int size,
                  const typename NumTraits<Scalar>::Real& refvalue) {
  for (int i = 0; i < size; ++i) {
    if (!isApproxAbs(a[i], b[i], refvalue)) {
      std::cout << "ref: [" << Map<const Matrix<Scalar, 1, Dynamic> >(a, size)
                << "]"
                << " != vec: ["
...
void packetmath() {
  ...
  VERIFY((internal::is_same<Scalar, int>::value) ||
         (!PacketTraits::Vectorizable) || PacketTraits::HasDiv);
  ...
  VERIFY(internal::isApprox(data1[0],
                            internal::pfirst(internal::pload<Packet>(data1))) &&
         "internal::pfirst");
  ...
  if (PacketSize > 2) {
    for (int offset = 0; offset < 4; ++offset) {
      for (int i = 0; i < PacketSize / 4; ++i)
        ref[4 * i + 0] = ref[4 * i + 1] = ref[4 * i + 2] = ref[4 * i + 3] =
            data1[offset + i];
...

  {
    ...
    for (int i = 0; i < newsize; ++i) ref[i] = 0;
    for (int i = 0; i < PacketSize; ++i) ref[i % newsize] += data1[i];
    internal::pstore(data2,
                     internal::predux_downto4(internal::pload<Packet>(data1)));
  ...
  VERIFY(internal::isApprox(
             ref[0], internal::predux_mul(internal::pload<Packet>(data1))) &&
         "internal::predux_mul");
  ...
  if (PacketTraits::HasBlend) {
    ...
    for (int i = 0; i < PacketSize; ++i) {
      VERIFY(isApproxAbs(result[i], (selector.select[i] ? data1[i] : data2[i]),
                         refvalue));
    }
  }
  ...
  if (PacketTraits::HasBlend) {
    ...
    ref[0] = s;
    internal::pstore(data2,
                     internal::pinsertfirst(internal::pload<Packet>(data1), s));
  ...
  if (PacketTraits::HasBlend) {
    ...
    ref[PacketSize - 1] = s;
    internal::pstore(data2,
                     internal::pinsertlast(internal::pload<Packet>(data1), s));
...
void packetmath_real() {
  ...
  for (int i = 0; i < size; ++i) {
    data1[i] = internal::random<Scalar>(-1, 1) *
               std::pow(Scalar(10), internal::random<Scalar>(-3, 3));
  ...
  for (int i = 0; i < size; ++i) {
    data1[i] = internal::random<Scalar>(-1, 1) *
               std::pow(Scalar(10), internal::random<Scalar>(-6, 6));
    data2[i] = internal::random<Scalar>(-1, 1) *
               std::pow(Scalar(10), internal::random<Scalar>(-6, 6));
  }
  ...
  if (PacketTraits::HasExp && PacketTraits::size >= 2) {
    ...
    data1[1] = 0;
    h.store(data2, internal::pexp(h.load(data1)));
    VERIFY_IS_EQUAL(std::exp(-std::numeric_limits<Scalar>::epsilon()),
                    data2[0]);
    ...
    h.store(data2, internal::pexp(h.load(data1)));
    VERIFY_IS_EQUAL(std::exp(std::numeric_limits<Scalar>::denorm_min()),
                    data2[0]);
    VERIFY_IS_EQUAL(std::exp(-std::numeric_limits<Scalar>::denorm_min()),
                    data2[1]);
  }
  ...
  if (PacketTraits::HasTanh) {
    // NOTE this test migh fail with GCC prior to 6.3, see MathFunctionsImpl.h
    // for details.
    data1[0] = std::numeric_limits<Scalar>::quiet_NaN();
  ...
  for (int i = 0; i < size; ++i) {
    data1[i] = internal::random<Scalar>(0, 1) *
               std::pow(Scalar(10), internal::random<Scalar>(-6, 6));
    data2[i] = internal::random<Scalar>(0, 1) *
               std::pow(Scalar(10), internal::random<Scalar>(-6, 6));
  }
  ...
  if (PacketTraits::HasLog && PacketTraits::size >= 2) {
    ...
    data1[1] = -std::numeric_limits<Scalar>::denorm_min();
    h.store(data2, internal::plog(h.load(data1)));
    // VERIFY_IS_EQUAL(std::log(std::numeric_limits<Scalar>::denorm_min()),
    // data2[0]);
    VERIFY((numext::isnan)(data2[1]));
...
void packetmath_notcomplex() {
  ...
  VERIFY(internal::isApprox(
             ref[0], internal::predux_min(internal::pload<Packet>(data1))) &&
         "internal::predux_min");
  ...
  VERIFY(internal::isApprox(
             ref[0], internal::predux_max(internal::pload<Packet>(data1))) &&
         "internal::predux_max");
...
void test_conj_helper(Scalar* data1, Scalar* data2, Scalar* ref, Scalar* pval) {
  ...
  for (int i = 0; i < PacketSize; ++i) {
    ...
    VERIFY(internal::isApprox(ref[i], cj.pmul(data1[i], data2[i])) &&
           "conj_helper pmul");
  }
  internal::pstore(pval, pcj.pmul(internal::pload<Packet>(data1),
                                  internal::pload<Packet>(data2)));
  VERIFY(areApprox(ref, pval, PacketSize) && "conj_helper pmul");

  for (int i = 0; i < PacketSize; ++i) {
    Scalar tmp = ref[i];
    ref[i] += cj0(data1[i]) * cj1(data2[i]);
    VERIFY(internal::isApprox(ref[i], cj.pmadd(data1[i], data2[i], tmp)) &&
           "conj_helper pmadd");
  }
  internal::pstore(pval, pcj.pmadd(internal::pload<Packet>(data1),
                                   internal::pload<Packet>(data2),
