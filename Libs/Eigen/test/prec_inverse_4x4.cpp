...
void inverse_general_4x4(int repeat) {
  ...
  std::cerr << "inverse_general_4x4, Scalar = " << type_name<Scalar>()
            << std::endl;
  double error_avg = error_sum / repeat;
  EIGEN_DEBUG_VAR(error_avg);
  EIGEN_DEBUG_VAR(error_max);
  // FIXME that 1.25 used to be a 1.0 until the NumTraits changes on 28 April
  // 2010, what's going wrong??
  // FIXME that 1.25 used to be 1.2 until we tested gcc 4.1 on 30 June 2010 and
  // got 1.21.
