...
void product_extra(const MatrixType& m) {
  ...
  typedef Matrix<Scalar, Dynamic, Dynamic, MatrixType::Flags & RowMajorBit>
  ...
  VERIFY_IS_APPROX(m3.noalias() = m1.adjoint() * square.adjoint(),
                   m1.adjoint().eval() * square.adjoint().eval());
  VERIFY_IS_APPROX(m3.noalias() = m1.adjoint() * m2, m1.adjoint().eval() * m2);
  VERIFY_IS_APPROX(m3.noalias() = (s1 * m1.adjoint()) * m2,
                   (s1 * m1.adjoint()).eval() * m2);
  VERIFY_IS_APPROX(m3.noalias() = ((s1 * m1).adjoint()) * m2,
                   (numext::conj(s1) * m1.adjoint()).eval() * m2);
  VERIFY_IS_APPROX(m3.noalias() = (-m1.adjoint() * s1) * (s3 * m2),
                   (-m1.adjoint() * s1).eval() * (s3 * m2).eval());
  VERIFY_IS_APPROX(m3.noalias() = (s2 * m1.adjoint() * s1) * m2,
                   (s2 * m1.adjoint() * s1).eval() * m2);
  VERIFY_IS_APPROX(m3.noalias() = (-m1 * s2) * s1 * m2.adjoint(),
                   (-m1 * s2).eval() * (s1 * m2.adjoint()).eval());

  // a very tricky case where a scale factor has to be automatically conjugated:
  VERIFY_IS_APPROX(m1.adjoint() * (s1 * m2).conjugate(),
                   (m1.adjoint()).eval() * ((s1 * m2).conjugate()).eval());

  // test all possible conjugate combinations for the four matrix-vector product
  // cases:
  ...
  VERIFY_IS_APPROX(
      (-m1.conjugate() * s2) * (s1 * vc2.conjugate()),
  ...
  VERIFY_IS_APPROX(
      (s1 * v1.conjugate()) * (-m1.conjugate() * s2),
  ...
  VERIFY_IS_APPROX(
      m1.col(j2).adjoint() * m1.block(0, j, m1.rows(), c),
      m1.col(j2).adjoint().eval() * m1.block(0, j, m1.rows(), c).eval());
  VERIFY_IS_APPROX(
      m1.block(i, 0, r, m1.cols()) * m1.row(i2).adjoint(),
...
// Regression test for bug reported at
// http://forum.kde.org/viewtopic.php?f=74&t=96947
void mat_mat_scalar_scalar_product() {
  Eigen::Matrix2Xd dNdxy(2, 3);
  dNdxy << -0.5, 0.5, 0, -0.3, 0, 0.3;
  double det = 6.0, wt = 0.5;
  VERIFY_IS_APPROX(dNdxy.transpose() * dNdxy * det * wt,
                   det * wt * dNdxy.transpose() * dNdxy);
}
...
void zero_sized_objects(const MatrixType& m) {
...

  {
    ...
    VERIFY_IS_APPROX((res = b.transpose() * a.transpose()),
                     MatrixType::Zero(cols, rows));
  }
...

  {
    ...
    VERIFY_IS_APPROX((res = a.lazyProduct(b)),
                     MatrixType::Zero(PacketSize1, 1));
  }
...
void bug_127() {
  ...
  //
  // was failing on a runtime assertion, because it had been mis-compiled as a
  // dot product because Product.h was using the max-sizes to detect size 1
  // indicating vectors, and that didn't account for 0-sized object with
  // max-size 1.
...
void unaligned_objects() {
  ...
  // Recall the matrix*vector kernel avoid unaligned loads by loading two
  // packets and then reassemble then. There was a mistake in the computation of
  // the valid range for fully unaligned objects: in some rare cases, memory was
  // read outside the allocated matrix memory. Though the values were not used,
  // this might raise segfault.
...
void bug_1308() {
  ...
  VERIFY_IS_APPROX(m44.noalias() = ones44 * Matrix4d::Ones(),
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(m44.noalias() = ones44.transpose() * Matrix4d::Ones(),
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(m44.noalias() = Matrix4d::Ones() * ones44,
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(m44.noalias() = Matrix4d::Ones() * ones44.transpose(),
                   Matrix4d::Constant(4));
  ...
  VERIFY_IS_APPROX(r44.noalias() = ones44 * Matrix4d::Ones(),
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(r44.noalias() = ones44.transpose() * Matrix4d::Ones(),
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(r44.noalias() = Matrix4d::Ones() * ones44,
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(r44.noalias() = Matrix4d::Ones() * ones44.transpose(),
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(r44.noalias() = ones44 * RMatrix4d::Ones(),
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(r44.noalias() = ones44.transpose() * RMatrix4d::Ones(),
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(r44.noalias() = RMatrix4d::Ones() * ones44,
                   Matrix4d::Constant(4));
  VERIFY_IS_APPROX(r44.noalias() = RMatrix4d::Ones() * ones44.transpose(),
                   Matrix4d::Constant(4));
  ...
  r44.setZero();
  VERIFY_IS_APPROX(
      r44.noalias() += m44.row(0).transpose() * RowVector4d::Ones(), ones44);
  r44.setZero();
  ...
  r44.setZero();
  VERIFY_IS_APPROX(r44.noalias() += Vector4d::Ones() * m44.col(0).transpose(),
                   ones44);
}
...
void test_product_extra() {
  for (int i = 0; i < g_repeat; i++) {
    CALL_SUBTEST_1(
        product_extra(MatrixXf(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                               internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_2(
        product_extra(MatrixXd(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                               internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_2(mat_mat_scalar_scalar_product());
    CALL_SUBTEST_3(product_extra(
        MatrixXcf(internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2),
                  internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2))));
    CALL_SUBTEST_4(product_extra(
        MatrixXcd(internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2),
                  internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2))));
    CALL_SUBTEST_1(zero_sized_objects(
        MatrixXf(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                 internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
  }
