...
void test_aliasing() {
  ...
  VERIFY_IS_APPROX(
      x = y + A * x,
      A * z);  // OK because "y + A*x" is marked as "assume-aliasing"
  x = z;
  // CwiseUnaryOp
  VERIFY_IS_APPROX(x = T(1.) * (A * x),
                   A * z);  // OK because 1*(A*x) is replaced by (1*A*x) which
                            // is a Product<> expression
  x = z;
  // VERIFY_IS_APPROX(x = y-A*x, -A*z);   // Not OK in 3.3 because x is resized
  // before A*x gets evaluated
  x = z;
}
...
void test_product_large() {
...

  {
    ...
    internal::computeProductBlockingSizes<float, float, std::ptrdiff_t>(k1, m1,
                                                                        n1, 1);
  }
