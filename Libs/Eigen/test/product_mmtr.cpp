...
void mmtr(int size) {
  ...
  // see bug 1741
  {
    ...
    Map<MatrixColMaj, 0, Stride<Dynamic, Dynamic> > map1(
        buffer.data(), size, size, Stride<Dynamic, Dynamic>(2 * size, 2));
    buffer.setZero();
...
void test_product_mmtr() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_3((mmtr<std::complex<float> >(
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2))));
    CALL_SUBTEST_4((mmtr<std::complex<double> >(
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2))));
  }
}
