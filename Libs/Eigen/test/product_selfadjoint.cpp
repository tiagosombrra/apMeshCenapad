...
void product_selfadjoint(const MatrixType& m) {
  ...
  typedef Matrix<Scalar, MatrixType::RowsAtCompileTime, Dynamic, RowMajor>
      RhsMatrixType;
  ...
  VERIFY_IS_APPROX(m2, (m1 + v1 * v2.adjoint() + v2 * v1.adjoint())
                           .template triangularView<Lower>()
                           .toDenseMatrix());

  m2 = m1.template triangularView<Upper>();
  m2.template selfadjointView<Upper>().rankUpdate(-v1, s2 * v2, s3);
  VERIFY_IS_APPROX(m2, (m1 + (s3 * (-v1) * (s2 * v2).adjoint() +
                              numext::conj(s3) * (s2 * v2) * (-v1).adjoint()))
                           .template triangularView<Upper>()
                           .toDenseMatrix());

  m2 = m1.template triangularView<Upper>();
  m2.template selfadjointView<Upper>().rankUpdate(-s2 * r1.adjoint(),
                                                  r2.adjoint() * s3, s1);
  VERIFY_IS_APPROX(
      m2,
  ...
  if (rows > 1) {
    ...
    m3.block(1, 1, rows - 1, cols - 1) +=
        v1.tail(rows - 1) * v2.head(cols - 1).adjoint() +
        v2.head(cols - 1) * v1.tail(rows - 1).adjoint();
