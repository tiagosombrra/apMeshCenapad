...
#define VERIFY_TRSM_ONTHERIGHT(TRI, XB)                                    \
  {                                                                        \
    ...
    ref = (XB);                                                            \
    (XB).transpose() =                                                     \
        (TRI).transpose().template solve<OnTheRight>(XB.transpose());      \
    VERIFY_IS_APPROX((XB).transpose() * (TRI).transpose().toDenseMatrix(), \
                     ref.transpose());                                     \
  }
...
void trsolve(int size = Size, int cols = Cols) {
  ...
  VERIFY_TRSM_ONTHERIGHT(cmLhs.conjugate().template triangularView<Lower>(),
                         cmRhs);
  ...
  VERIFY_TRSM_ONTHERIGHT(cmLhs.conjugate().template triangularView<Upper>(),
                         rmRhs);

  VERIFY_TRSM_ONTHERIGHT(cmLhs.conjugate().template triangularView<UnitLower>(),
                         cmRhs);
  ...
  VERIFY_TRSM_ONTHERIGHT(rmLhs.conjugate().template triangularView<UnitUpper>(),
                         rmRhs);
  ...
  if (Size == Dynamic) {
    ...
    Matrix<Scalar, Size, Cols, colmajor> res =
        cmLhs.template triangularView<Lower>().solve(cmRhs);
...
void test_product_trsolve() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    // vectors
    CALL_SUBTEST_5((trsolve<float, Dynamic, 1>(
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_6((trsolve<double, Dynamic, 1>(
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_7((trsolve<std::complex<float>, Dynamic, 1>(
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_8((trsolve<std::complex<double>, Dynamic, 1>(
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
