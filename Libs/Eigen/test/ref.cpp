...
void ref_matrix(const MatrixType &m) {
  ...
  typedef Matrix<RealScalar, Dynamic, Dynamic, MatrixType::Options>
      RealDynMatrixType;

  typedef Ref<MatrixType> RefMat;
  typedef Ref<DynMatrixType> RefDynMat;
  typedef Ref<const DynMatrixType> ConstRefDynMat;
  typedef Ref<RealDynMatrixType, 0, Stride<Dynamic, Dynamic> >
...
void ref_vector(const VectorType &m) {
  ...
  MatrixType mat1 = MatrixType::Random(size, size), mat2 = mat1,
  ...
  {
    VERIFY_RAISES_ASSERT(RefMat rm0 = v1.block(0, 0, size, 0);
                         EIGEN_UNUSED_VARIABLE(rm0););
  }
  if (VectorType::SizeAtCompileTime != 1) {
    VERIFY_RAISES_ASSERT(RefDynMat rv1 = v1.block(0, 0, size, 0);
                         EIGEN_UNUSED_VARIABLE(rv1););
  }
...
template <typename B>
EIGEN_DONT_INLINE void call_ref_4(
    const Ref<const VectorXf, 0, InnerStride<> > &a, const B &b) {
  VERIFY_IS_EQUAL(a, b);
}
template <typename B>
...
template <typename B>
EIGEN_DONT_INLINE void call_ref_6(
    const Ref<const MatrixXf, 0, OuterStride<> > &a, const B &b) {
  VERIFY_IS_EQUAL(a, b);
}
template <typename B>
...
void call_ref() {
  ...
  //   call_ref_1(A.row(3),A.row(3));    // does not compile because
  //   innerstride!=1
  ...
  VERIFY_EVALUATION_COUNT(call_ref_2(A * A.col(1), tmp),
                          1);  // evaluated into a temp
  ...
  VERIFY_EVALUATION_COUNT(call_ref_2(ca.imag(), ca.imag()),
                          1);  // evaluated into a temp
  ...
  VERIFY_EVALUATION_COUNT(call_ref_5(A, A), 0);
  //   call_ref_5(A.transpose(),A.transpose());   // does not compile because
  //   storage order does not match
  VERIFY_EVALUATION_COUNT(call_ref_5(A.block(1, 1, 2, 2), A.block(1, 1, 2, 2)),
                          0);
  VERIFY_EVALUATION_COUNT(call_ref_5(b, b),
                          0);  // storage order do not match, but this is a
  ...
  VERIFY_EVALUATION_COUNT(call_ref_6(A.row(3), A.row(3)),
                          1);  // evaluated into a temp thouth it could be
                               // avoided by viewing it as a 1xn matrix
  tmp = A + A;
  VERIFY_EVALUATION_COUNT(call_ref_6(A + A, tmp), 1);  // evaluated into a temp
  VERIFY_EVALUATION_COUNT(call_ref_6(A, A), 0);
  VERIFY_EVALUATION_COUNT(
      call_ref_6(A.transpose(), A.transpose()),
      1);  // evaluated into a temp because the storage orders do not match
  VERIFY_EVALUATION_COUNT(call_ref_6(A.block(1, 1, 2, 2), A.block(1, 1, 2, 2)),
                          0);
...
void test_ref() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_4(ref_matrix(
        MatrixXcf(internal::random<int>(1, 10), internal::random<int>(1, 10))));
    CALL_SUBTEST_4(ref_matrix(Matrix<std::complex<double>, 10, 15>()));
    CALL_SUBTEST_5(ref_matrix(
        MatrixXi(internal::random<int>(1, 10), internal::random<int>(1, 10))));
