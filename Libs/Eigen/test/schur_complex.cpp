...
void schur(int size = MatrixType::ColsAtCompileTime) {
  ...
  typedef
      typename ComplexSchur<MatrixType>::ComplexMatrixType ComplexMatrixType;
  ...
  cs3.setMaxIterations(ComplexSchur<MatrixType>::m_maxIterationsPerRow * size)
      .compute(A);
  ...
  cs3.setMaxIterations(1).compute(
      Atriangular);  // triangular matrices do not need any iterations
  VERIFY_IS_EQUAL(cs3.info(), Success);
