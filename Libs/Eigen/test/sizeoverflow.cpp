...
void test_sizeoverflow() {
  // there are 2 levels of overflow checking. first in PlainObjectBase.h we
  // check for overflow in rows*cols computations. this is tested in tests of
  // the form times_itself_gives_0 * times_itself_gives_0 Then in Memory.h we
  // check for overflow in size * sizeof(T) computations. this is tested in
  // tests of the form times_4_gives_0 * sizeof(float)
  ...
  triggerMatrixBadAlloc<MatrixXf>(times_itself_gives_0 / 4,
                                  times_itself_gives_0);
  triggerMatrixBadAlloc<MatrixXf>(times_4_gives_0, 1);

  triggerMatrixBadAlloc<MatrixXd>(times_itself_gives_0, times_itself_gives_0);
  triggerMatrixBadAlloc<MatrixXd>(times_itself_gives_0 / 8,
                                  times_itself_gives_0);
