...
void solve_with_guess(IterativeSolverBase<Solver>& solver,
                      const MatrixBase<Rhs>& b, const Guess& g, Result& x) {
  if (internal::random<bool>()) {
    // With a temporary through evaluator<SolveWithGuess>
    x = solver.derived().solveWithGuess(b, g) +
        Result::Zero(x.rows(), x.cols());
  } else {
...
void solve_with_guess(SparseSolverBase<Solver>& solver,
                      const MatrixBase<Rhs>& b, const Guess&, Result& x) {
  if (internal::random<bool>())
...
template <typename Solver, typename Rhs, typename Guess, typename Result>
void solve_with_guess(SparseSolverBase<Solver>& solver,
                      const SparseMatrixBase<Rhs>& b, const Guess&, Result& x) {
  x = solver.derived().solve(b);
}

template <typename Solver, typename Rhs, typename DenseMat, typename DenseRhs>
void check_sparse_solving(Solver& solver, const typename Solver::MatrixType& A,
                          const Rhs& b, const DenseMat& dA,
                          const DenseRhs& db) {
  ...
  DenseRhs refX = dA.householderQr().solve(db);
  {
    ...
    if (solver.info() != Success) {
      std::cerr << "ERROR | sparse solver testing, factorization failed ("
                << typeid(Solver).name() << ")\n";
      VERIFY(solver.info() == Success);
    }
    x = solver.solve(b);
    if (solver.info() != Success) {
      std::cerr << "WARNING | sparse solver testing: solving failed ("
                << typeid(Solver).name() << ")\n";
      return;
    }
    VERIFY(oldb.isApprox(b) &&
    ...
    x.setZero();
    // test with Map
    MappedSparseMatrix<Scalar, Mat::Options, StorageIndex> Am(
  ...
  if (A.rows() < 2000) {
    ...
    // test dense Block as the result and rhs:
    {
      ...
      DenseRhs oldb(db);
      x.setZero();
      x.block(0, 0, x.rows(), x.cols()) =
          solver.solve(db.block(0, 0, db.rows(), db.cols()));
      VERIFY(oldb.isApprox(db) &&
             "sparse solver testing: the rhs should not be modified!");
      VERIFY(x.isApprox(refX, test_precision<Scalar>()));
    }
    ...
    {
      Mat A2 = A;
      A2.reserve((ArrayXf::Random(A.outerSize()) + 2)
                     .template cast<typename Mat::StorageIndex>()
                     .eval());
      solver.compute(A2);
...
void check_sparse_solving_real_cases(Solver& solver,
  ...
  VERIFY((res_error <= test_precision<Scalar>()) &&
         "sparse solver failed without noticing it");

  if (refX.size() != 0 &&
      (refX - x).norm() / refX.norm() > test_precision<Scalar>()) {
    std::cerr << "WARNING | found solution is different from the provided "
                 "reference one\n";
  }
}
...
void check_sparse_determinant(Solver& solver,
  ...
  solver.compute(A);
  if (solver.info() != Success) {
    std::cerr << "WARNING | sparse solver testing: factorization failed "
...
template <typename Solver, typename DenseMat>
int generate_sparse_spd_problem(Solver&, typename Solver::MatrixType& A,
...
void check_sparse_spd_solving(Solver& solver, int maxSize = 300,
    ...
    for (; it; ++it) {
      if (it.sym() == SPD) {
        ...
        if (A.diagonal().size() <= maxRealWorldSize) {
          ...
          std::cout << "INFO | Testing " << sym_to_string(it.sym())
                    << "sparse problem " << it.matname() << " (" << A.rows()
                    << "x" << A.cols() << ") using " << typeid(Solver).name()
                    << "..." << std::endl;
          ...
          std::cout << "INFO | Skip sparse problem \"" << it.matname()
                    << "\" (too large)" << std::endl;
        }
      }
    }
...
template <typename Solver, typename DenseMat>
Index generate_sparse_square_problem(Solver&, typename Solver::MatrixType& A,
                                     DenseMat& dA, int maxSize = 300,
...
template <typename Solver>
void check_sparse_square_solving(Solver& solver, int maxSize = 300,
                                 int maxRealWorldSize = 100000,
