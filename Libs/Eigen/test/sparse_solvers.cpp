...
void initSPD(double density, Matrix<Scalar, Dynamic, Dynamic>& refMat,
...
void sparse_solvers(int rows, int cols) {
  ...
  // test triangular solver
  {
    ...
    initSparse<Scalar>(density, refMat2, m2,
                       ForceNonZeroDiag | MakeLowerTriangular, &zeroCoords,
                       &nonzeroCoords);
    ...
    // upper - dense
    initSparse<Scalar>(density, refMat2, m2,
                       ForceNonZeroDiag | MakeUpperTriangular, &zeroCoords,
                       &nonzeroCoords);
    VERIFY_IS_APPROX(refMat2.template triangularView<Upper>().solve(vec2),
                     m2.template triangularView<Upper>().solve(vec3));
    VERIFY_IS_APPROX(
        refMat2.conjugate().template triangularView<Upper>().solve(vec2),
        m2.conjugate().template triangularView<Upper>().solve(vec3));
    {
      SparseMatrix<Scalar> cm2(m2);
      // Index rows, Index cols, Index nnz, Index* outerIndexPtr, Index*
      // innerIndexPtr, Scalar* valuePtr
      MappedSparseMatrix<Scalar> mm2(rows, cols, cm2.nonZeros(),
                                     cm2.outerIndexPtr(), cm2.innerIndexPtr(),
                                     cm2.valuePtr());
      VERIFY_IS_APPROX(
          refMat2.conjugate().template triangularView<Upper>().solve(vec2),
    ...
    initSparse<Scalar>(density, refMat2, m2,
                       ...
                       &nonzeroCoords);
    VERIFY_IS_APPROX(
        refMat2.transpose().template triangularView<Upper>().solve(vec2),
    ...
    initSparse<Scalar>(density, refMat2, m2,
                       ...
                       &nonzeroCoords);
    VERIFY_IS_APPROX(
        refMat2.transpose().template triangularView<Lower>().solve(vec2),
    ...
    // lower - sparse
    initSparse<Scalar>(density, refMat2, m2,
                       ForceNonZeroDiag | MakeLowerTriangular);
