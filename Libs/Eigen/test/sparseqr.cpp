...
int generate_sparse_rectangular_problem(MatrixType& A, DenseMat& dA,
                                        int maxRows = 300, int maxCols = 150) {
  ...
  A.makeCompressed();
  int nop = internal::random<int>(
      0, internal::random<double>(0, 1) > 0.5 ? cols / 2 : 0);
  for (int k = 0; k < nop; ++k) {
...
void test_sparseqr_scalar() {
  ...
  if (internal::random<float>(0, 1) > 0.5f)
    solver.factorize(A);  // this checks that calling analyzePattern is not
                          // needed if the pattern do not change.
