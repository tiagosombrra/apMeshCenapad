...
void stable_norm(const MatrixType& m) {
  ...
  */
  ...
  // Check the basic machine-dependent constants.
  {
    ...
    VERIFY((!(iemin > 1 - 2 * it || 1 + it > iemax || (it == 2 && ibeta < 5) ||
              (it <= 4 && ibeta <= 3) || it < 2)) &&
    ...
    if (NumTraits<Scalar>::IsComplex && (numext::isnan)(inf * RealScalar(1))) {
      ...
      if (first)
        std::cerr << "WARNING: compiler mess up complex*real product, " << inf
                  << " * " << 1.0 << " = " << inf * RealScalar(1) << std::endl;
      first = false;
    }
  }
  ...
  Scalar big =
      factor * ((std::numeric_limits<RealScalar>::max)() * RealScalar(1e-4));

  factor = internal::random<Scalar>();
  while (numext::abs2(factor) < RealScalar(1e-4))
    factor = internal::random<Scalar>();
  Scalar small =
      factor * ((std::numeric_limits<RealScalar>::min)() * RealScalar(1e4));

  Scalar one(1);

  MatrixType vzero = MatrixType::Zero(rows, cols),
             vrand = MatrixType::Random(rows, cols), vbig(rows, cols),
             vsmall(rows, cols);
  ...
  VERIFY_IS_APPROX((one * vrand + one * vrand - one * vrand).stableNorm(),
                   vrand.norm());
  VERIFY_IS_APPROX((one * vrand + one * vrand - one * vrand).blueNorm(),
                   vrand.norm());
  VERIFY_IS_APPROX((one * vrand + one * vrand - one * vrand).hypotNorm(),
                   vrand.norm());
  ...
  VERIFY_IS_NOT_APPROX(
      sqrt(copy(vbig.squaredNorm())),
      abs(sqrt(size) * big));  // here the default norm must fail
  ...
  VERIFY_IS_NOT_APPROX(
      sqrt(copy(vsmall.squaredNorm())),
      abs(sqrt(size) * small));  // here the default norm must fail
  ...
  // stableNormalize[d]
  {
    ...
    RealScalar big_scaling =
        ((std::numeric_limits<RealScalar>::max)() * RealScalar(1e-4));
    VERIFY_IS_APPROX(
        vbig / big_scaling,
        (vbig.stableNorm() * vbig.stableNormalized()).eval() / big_scaling);
