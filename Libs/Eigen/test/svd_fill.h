...
void svd_fill_random(MatrixType &m, int Option = 0) {
  ...
  Matrix<RealScalar, Dynamic, 1> d =
      Matrix<RealScalar, Dynamic, 1>::Random(diagSize);
  ...
  bool unit_uv =
      ...
      (dup ? 7
           : 3);  // if we duplicate some diagonal entries, then increase the
                  // chance to preserve them using unitary U and V factors
        ...
        if (!(dup && unit_uv)) {
          ...
          for (Index k = 0; k < n; ++k) {
            ...
            if (NumTraits<Scalar>::IsComplex)
              *(&numext::real_ref(m(j, i)) + 1) =
                  *(&numext::real_ref(m(i, j)) + 1) = samples.real()(
