...
void triangular_square(const MatrixType& m) {
  ...
  MatrixType m1 = MatrixType::Random(rows, cols),
             ...
             m4(rows, cols), r1(rows, cols), r2(rows, cols);
  ...
  //   VERIFY_IS_APPROX(m1up.transpose() * m2, m1.upper().transpose().lower() *
  //   m2);
  ...
  m3 = m2.transpose() + m2;
  VERIFY_IS_APPROX(
      m3.template triangularView<Lower>().transpose().toDenseMatrix(), m1);
  ...
  VERIFY_IS_APPROX(
      m3.template triangularView<Lower>().conjugate().toDenseMatrix(),
  ...
  VERIFY(v2.isApprox(
      m3.adjoint() * (m1.adjoint().template triangularView<Lower>().solve(v2)),
      largerEps));
  m3 = m1.template triangularView<Lower>();
  VERIFY(v2.isApprox(
      m3.transpose() *
          (m1.transpose().template triangularView<Upper>().solve(v2)),
      largerEps));
  m3 = m1.template triangularView<Upper>();
  VERIFY(v2.isApprox(m3 * (m1.template triangularView<Upper>().solve(v2)),
                     largerEps));
  m3 = m1.template triangularView<Lower>();
  VERIFY(v2.isApprox(
      m3.conjugate() *
          (m1.conjugate().template triangularView<Lower>().solve(v2)),
      largerEps));

  // test back and forward substitution with a matrix as the rhs
  m3 = m1.template triangularView<Upper>();
  VERIFY(m2.isApprox(
      m3.adjoint() * (m1.adjoint().template triangularView<Lower>().solve(m2)),
      largerEps));
  m3 = m1.template triangularView<Lower>();
  VERIFY(m2.isApprox(
      m3.transpose() *
          (m1.transpose().template triangularView<Upper>().solve(m2)),
      largerEps));
  m3 = m1.template triangularView<Upper>();
  VERIFY(m2.isApprox(m3 * (m1.template triangularView<Upper>().solve(m2)),
                     largerEps));
  m3 = m1.template triangularView<Lower>();
  VERIFY(m2.isApprox(
      m3.conjugate() *
          (m1.conjugate().template triangularView<Lower>().solve(m2)),
      largerEps));
  ...
  VERIFY(m2.isApprox(m3 * (m1.template triangularView<UnitUpper>().solve(m2)),
                     largerEps));
  ...
  m1.setRandom();
  m3 = m1.template triangularView<Upper>();
  Matrix<Scalar, MatrixType::ColsAtCompileTime, Dynamic> m5(
      cols, internal::random<int>(1, 20));
  m5.setRandom();
  Matrix<Scalar, Dynamic, MatrixType::RowsAtCompileTime> m6(
      internal::random<int>(1, 20), rows);
  m6.setRandom();
  ...
  VERIFY_IS_APPROX(m1.template selfadjointView<Upper>().diagonal(),
                   m1.diagonal());
}
...
void triangular_rect(const MatrixType& m) {
  ...
  VERIFY_IS_APPROX(m3.template triangularView<StrictlyUpper>().toDenseMatrix(),
                   m1);

  m1.setZero();
  m1.template triangularView<StrictlyLower>() = 3 * m2;
  VERIFY_IS_APPROX(m3.template triangularView<StrictlyLower>().toDenseMatrix(),
                   m1);
  m1.setRandom();
