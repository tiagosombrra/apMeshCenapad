...
Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> randMatrixUnitary(int size) {
  ...
  while (!is_unitary && max_tries > 0) {
    ...
    // this additional orthogonalization is not necessary in theory but should
    // enhance the numerical orthogonality of the matrix
...
template <typename T>
Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> randMatrixSpecialUnitary(
    int size) {
  typedef T Scalar;
...
void run_test(int dim, int num_elements) {
  ...
  // MUST be positive because in any other case det(cR_t) may become negative
  // for odd dimensions!
  ...
  MatrixX cR_t_umeyama = umeyama(src.block(0, 0, dim, num_elements),
                                 dst.block(0, 0, dim, num_elements));
...
void run_fixed_size_test(int num_elements) {
  ...
  // MUST be positive because in any other case det(cR_t) may become negative
  // for odd dimensions! Also if c is to small compared to t.norm(), problem is
  // ill-posed (cf. Bug 744)
