...
#define VERIFY_ALIGNED_UNALIGNED_COUNT(XPR, AL, UL, AS, US)                  \
  {                                                                          \
    ...
    XPR;                                                                     \
    if (!(nb_load == AL && nb_loadu == UL && nb_store == AS &&               \
          nb_storeu == US))                                                  \
      std::cerr << " >> " << nb_load << ", " << nb_loadu << ", " << nb_store \
                << ", " << nb_storeu << "\n";                                \
    VERIFY((#XPR) && nb_load == AL && nb_loadu == UL && nb_store == AS &&    \
           nb_storeu == US);                                                 \
  }
...
void test_unalignedcount() {
  ...
  VERIFY_ALIGNED_UNALIGNED_COUNT(a.segment(0, 40) += b.segment(0, 40), 5, 5, 5,
                                 0);
  VERIFY_ALIGNED_UNALIGNED_COUNT(a.segment(0, 40) -= b.segment(0, 40), 5, 5, 5,
                                 0);
  ...
  VERIFY_ALIGNED_UNALIGNED_COUNT(a.segment(0, 40) += b.segment(0, 40), 10, 10,
                                 10, 0);
  VERIFY_ALIGNED_UNALIGNED_COUNT(a.segment(0, 40) -= b.segment(0, 40), 10, 10,
                                 10, 0);
