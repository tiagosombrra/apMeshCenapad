...
void upperbidiag(const MatrixType& m) {
  ...
  typedef Matrix<typename MatrixType::RealScalar, MatrixType::RowsAtCompileTime,
  ...
  TransposeMatrixType d =
      ubd.householderV() * b.adjoint() * ubd.householderU().adjoint();
...
void test_upperbidiagonalization() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_4(upperbidiag(
        Matrix<std::complex<double>, Dynamic, Dynamic, RowMajor>(16, 15)));
