...
struct vectorization_logic {
  ...
  static void run() {
    ...
    typedef Matrix<Scalar,
                   ...
                   (PacketSize == 8   ? 2
                    ...
                    : PacketSize == 2 ? 2
                                      : /*PacketSize==1 ?*/ 1),
                   DontAlign |
                       ((Matrix1::Flags & RowMajorBit) ? RowMajor : ColMajor)>
        Matrix1u;
    ...
    if (PacketSize > 1) {
      ...
      VERIFY(test_assign(Matrix33c().col(0),
                         ...
                         EIGEN_UNALIGNED_VECTORIZE
                             ? (HalfPacketSize == 1 ? InnerVectorizedTraversal
                                                    : LinearVectorizedTraversal)
                             : (HalfPacketSize == 1 ? SliceVectorizedTraversal
                                                    : LinearTraversal),
                         ((!EIGEN_UNALIGNED_VECTORIZE) && HalfPacketSize == 1)
                             ? NoUnrolling
                             : CompleteUnrolling));
    ...
    VERIFY(test_redux(Matrix44().template block < (Matrix1::Flags & RowMajorBit)
                          ? 4
                          : PacketSize,
...
struct vectorization_logic_half {
  ...
  static void run() {
    ...
    //     Matrix44u; typedef Matrix<Scalar,4*PacketSize,4*PacketSize,ColMajor>
    //     Matrix44c; typedef Matrix<Scalar,4*PacketSize,4*PacketSize,RowMajor>
    //     Matrix44r;
    ...
    VERIFY(
        test_assign(Vector1(),
                    (Scalar(2.1) * Vector1().template segment<PacketSize>(0) -
    ...
    if (PacketSize > 1) {
      ...
      VERIFY(test_assign(Matrix33c().col(0),
                         Matrix33c().col(1) + Matrix33c().col(1),
                         EIGEN_UNALIGNED_VECTORIZE
                             ? (PacketSize == 1 ? InnerVectorizedTraversal
                                                : LinearVectorizedTraversal)
                             : LinearTraversal,
                         CompleteUnrolling));
