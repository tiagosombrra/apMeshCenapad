...
void zeroSizedMatrix() {
  ...
  if (MatrixType::SizeAtCompileTime == Dynamic ||
      MatrixType::SizeAtCompileTime == 0) {
    zeroReduction(t1);
  ...
  if (MatrixType::MaxColsAtCompileTime != 0 &&
      MatrixType::MaxRowsAtCompileTime != 0) {
    ...
    zeroReduction(
        m.template block<MatrixType::RowsAtCompileTime, 0>(0, 0, rows, 0));
    zeroReduction(m.template block<0, 1>(0, 0));
    zeroReduction(m.template block<1, 0>(0, 0));
    Matrix<Scalar, Dynamic, Dynamic> prod =
        m.template block<MatrixType::RowsAtCompileTime, 0>(0, 0, rows, 0) *
        m.template block<0, MatrixType::ColsAtCompileTime>(0, 0, 0, cols);
...
void zeroSizedVector() {
  ...
  if (VectorType::SizeAtCompileTime == Dynamic ||
      VectorType::SizeAtCompileTime == 0) {
    zeroReduction(t1);
    VERIFY(t1.size() == 0);
    VectorType t2(
        DenseIndex(0));  // DenseIndex disambiguates with 0-the-null-pointer
                         // (error with gcc 4.4 and MSVC8)
    VERIFY(t2.size() == 0);
    zeroReduction(t2);
