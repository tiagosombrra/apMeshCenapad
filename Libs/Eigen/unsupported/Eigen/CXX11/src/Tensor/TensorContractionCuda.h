...
__device__ EIGEN_STRONG_INLINE void EigenContractionKernelInternal(
  ...
  // contracting index is shared by both sides, then the contracting index
  // should be in threadIdx.x.

  // On the LHS, we pad each row inside of each block with an extra element.
  // This makes each block 8 rows of 9 elements, which is 72 elements. This
  // gives no bank conflicts on writes and very few 2-way conflicts on reads.
  // There is an 8x8 grid of these blocks.

  // On the RHS we just add 8 padding elements to the end of each block. This
  // gives no bank conflicts on writes and also none on reads.
...
#undef writeRegToShmem
    ...
    // compute 8x8 matrix product by outer product. This involves packing one
    // column of LHS and one row of RHS into registers (takes 16 registers).
...
#undef computePass
  ...
  // the sum across all big k blocks of the product of little k block of index
  // (x, y) with block of index (y, z). To compute the final output, we need to
  // reduce the 8 threads over y by summation.
...
__device__ EIGEN_STRONG_INLINE void EigenFloatContractionKernelInternal16x16(
    const LhsMapper lhs, const RhsMapper rhs, const OutputMapper output,
...
#define prefetch_lhs(reg, row, col)                \
  ...
  for (Index k = 0; k < k_size; k += 16) {
    ...
    // 61) Row 1 (time 1) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), ..
    // (60, 61)
    // ...
    // Row 15 (time 15) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), ..
    // (60, 61) Row 16 (time 0) -> features (2, 3), (6, 7), .. (30, 31), (34,
    // 35), ..  (62, 63)
    // ...
...
template <typename Index, typename LhsMapper, typename RhsMapper,
          typename OutputMapper, bool CHECK_LHS_BOUNDARY,
          bool CHECK_RHS_BOUNDARY>
__device__ EIGEN_STRONG_INLINE void EigenFloatContractionKernelInternal(
    const LhsMapper lhs, const RhsMapper rhs, const OutputMapper output,
  ...
  for (Index k = 0; k < k_size; k += 32) {
    ...
    if (!CHECK_LHS_BOUNDARY) {
      if ((threadIdx.y / 4 + k + 24) < k_size) {
        ...
        lhs_pf1 =
            lhs.loadPacket<Unaligned>(lhs_vert, (threadIdx.y / 4 + k + 8));
        lhs_pf2 =
            lhs.loadPacket<Unaligned>(lhs_vert, (threadIdx.y / 4 + k + 16));
        lhs_pf3 =
            lhs.loadPacket<Unaligned>(lhs_vert, (threadIdx.y / 4 + k + 24));
    ...
    // LHS.
    // Row 0 (time 0) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), .. (60,
    // 61) .. (124, 125) Row 1 (time 1) -> features (0, 1), (4, 5), .. (28, 29),
    // (32, 33), ..  (60, 61) .. (124, 125)
    // ...
    // Row 8 (time 0) -> features (2, 3), (6, 7), .. (30, 31), (34, 35), .. (62,
    // 63) .. (126, 127) Row 15 (time 7) -> features (2, 3), (6, 7), .. (30,
    // 31), (34, 35), ..  (62, 63) .. (126, 127)
...
struct TensorEvaluator<
    const TensorContractionOp<Indices, LeftArgType, RightArgType>, GpuDevice>
  ...
  struct LaunchKernels {
    static void Run(const LhsMapper& lhs, const RhsMapper& rhs,
      ...
      LAUNCH_CUDA_KERNEL((EigenContractionKernel<Scalar, Index, LhsMapper,
