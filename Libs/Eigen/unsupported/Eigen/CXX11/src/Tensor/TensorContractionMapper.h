...
struct CoeffLoader {
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void offsetBuffer(
      typename Tensor::Index) {
    eigen_assert(false && "unsupported");
  }

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE typename Tensor::Scalar coeff(
      typename Tensor::Index index) const {
    return m_tensor.coeff(index);
  }

  template <int LoadMode>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename Tensor::PacketReturnType
...
struct CoeffLoader<Tensor, true> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename Tensor::PacketReturnType
  packet(typename Tensor::Index index) const {
    return internal::ploadt_ro<typename Tensor::PacketReturnType, LoadMode>(
        m_data + index);
  }
...
template <typename Scalar, typename Index, int side, typename Tensor,
          ...
          bool inner_dim_contiguous, int Alignment>
 ...
 public:
  ...
  EIGEN_STRONG_INLINE Index computeIndex(Index row, Index col) const {
    ...
    EIGEN_UNUSED_VARIABLE(
        left);  // annoying bug in g++8.1:
                // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=85963
    Index nocontract_val = left ? row : col;
    Index linidx = 0;
    for (int i = static_cast<int>(array_size<nocontract_t>::value) - 1; i > 0;
         i--) {
      const Index idx = nocontract_val / m_ij_strides[i];
      linidx += idx * m_nocontract_strides[i];
      nocontract_val -= idx * m_ij_strides[i];
    }
    if (array_size<typename Tensor::Dimensions>::value >
        array_size<contract_t>::value) {
    ...
    if (array_size<contract_t>::value > 0) {
      for (int i = static_cast<int>(array_size<contract_t>::value) - 1; i > 0;
           i--) {
  ...
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE IndexPair<Index> computeIndexPair(
      Index row, Index col, const Index distance) const {
    const bool left = (side == Lhs);
    EIGEN_UNUSED_VARIABLE(
        left);  // annoying bug in g++8.1:
                // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=85963
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Index firstAligned(Index size) const {
    ...
    // matrix-vector product relies on the stride when dealing with aligned
    // inputs.
    return (Alignment == Aligned) && (side == Lhs) && inner_dim_contiguous
               ? 0
               : size;
  }
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Index stride() const {
    return ((side == Lhs) && inner_dim_contiguous &&
            array_size<contract_t>::value > 0)
               ? m_contract_strides[0]
               : 1;
  }
...
class BaseTensorContractionMapper
    : public SimpleTensorContractionMapper<
          Scalar, Index, side, Tensor, nocontract_t, contract_t, packet_size,
          inner_dim_contiguous, Alignment> {
 public:
  typedef SimpleTensorContractionMapper<Scalar, Index, side, Tensor,
                                        nocontract_t, contract_t, packet_size,
                                        inner_dim_contiguous, Alignment>
      ParentMapper;
  ...
  BaseTensorContractionMapper(const Tensor& tensor,
                              ...
                              const contract_t& contract_strides,
                              const contract_t& k_strides)
      : ParentMapper(tensor, nocontract_strides, ij_strides, contract_strides,
                     k_strides) {}
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet loadPacket(Index i,
                                                          Index j) const {
    ...
    if (Tensor::PacketAccess && inner_dim_contiguous && !inner_dim_reordered) {
      ...
      eigen_assert(this->computeIndex(i + packet_size - 1, j) ==
                   index + packet_size - 1);
      return this->m_tensor.template packet<AlignmentType>(index);
    }

    const IndexPair<Index> indexPair =
        this->computeIndexPair(i, j, packet_size - 1);
    const Index first = indexPair.first;
    const Index last = indexPair.second;

    // We can always do optimized packet reads from left hand side right now,
    // because the vertical matrix dimension on the left hand side is never
    // contracting. On the right hand side we need to check if the contracting
    // dimensions may have been shuffled first.
    if (Tensor::PacketAccess &&
        (side == Lhs || internal::array_size<contract_t>::value <= 1 ||
         !inner_dim_reordered) &&
    ...
    for (Index k = 1; k < packet_size - 1; k += 2) {
      const IndexPair<Index> internal_pair =
          this->computeIndexPair(i + k, j, 1);
...
class BaseTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t,
                                  contract_t, 1, inner_dim_contiguous,
                                  inner_dim_reordered, Alignment>
    : public SimpleTensorContractionMapper<Scalar, Index, side, Tensor,
                                           nocontract_t, contract_t, 1,
                                           inner_dim_contiguous, Alignment> {
 public:
  typedef SimpleTensorContractionMapper<Scalar, Index, side, Tensor,
                                        nocontract_t, contract_t, 1,
  ...
  template <int>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet loadHalfPacket(Index i,
                                                              Index j) const {
...
class TensorContractionSubMapper {
 public:
  ...
  typedef BaseTensorContractionMapper<
      Scalar, Index, side, Tensor, nocontract_t, contract_t, packet_size,
  ...
  enum {
    // We can use direct offsets iff the parent mapper supports then and we can
    // compute the strides.
    // TODO: we should also enable direct offsets for the Rhs case.
    UseDirectOffsets = ParentMapper::DirectOffsets && (side == Lhs) &&
                       inner_dim_contiguous &&
                       (array_size<contract_t>::value > 0)
  };

  EIGEN_DEVICE_FUNC TensorContractionSubMapper(const ParentMapper& base_mapper,
                                               Index vert_offset,
                                               Index horiz_offset)
      : m_base_mapper(base_mapper),
        m_vert_offset(vert_offset),
        m_horiz_offset(horiz_offset) {
    // Bake the offsets into the buffer used by the base mapper whenever
    // possible. This avoids the need to recompute this offset every time we
    // attempt to access a coefficient.
    if (UseDirectOffsets) {
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Packet loadPacket(Index i) const {
    ...
    return m_base_mapper.template loadPacket<Alignment>(i + m_vert_offset,
                                                        m_horiz_offset);
  }
  ...
  loadHalfPacket(Index i) const {
    ...
    return m_base_mapper.template loadHalfPacket<Alignment>(i + m_vert_offset,
                                                            m_horiz_offset);
  }

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void storePacket(Index i,
                                                         Packet p) const {
    if (UseDirectOffsets) {
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketT load(Index i) const {
    EIGEN_STATIC_ASSERT((internal::is_same<PacketT, Packet>::value),
                        YOU_MADE_A_PROGRAMMING_MISTAKE);
    const int ActualAlignment =
        (AlignmentType == Aligned) && (Alignment == Aligned) ? Aligned
                                                             : Unaligned;
    if (UseDirectOffsets) {
      return m_base_mapper.template loadPacket<ActualAlignment>(i, 0);
    }
    return m_base_mapper.template loadPacket<ActualAlignment>(i + m_vert_offset,
                                                              m_horiz_offset);
  }
...
template <typename Scalar_, typename Index, int side, typename Tensor,
          typename nocontract_t, typename contract_t, int packet_size,
          bool inner_dim_contiguous, bool inner_dim_reordered, int Alignment>
class TensorContractionInputMapper
    : public BaseTensorContractionMapper<
 ...
 public:
  ...
  EIGEN_DEVICE_FUNC TensorContractionInputMapper(
      const Tensor& tensor, const nocontract_t& nocontract_strides,
      const nocontract_t& ij_strides, const contract_t& contract_strides,
      const contract_t& k_strides)
      : Base(tensor, nocontract_strides, ij_strides, contract_strides,
             k_strides) {}
