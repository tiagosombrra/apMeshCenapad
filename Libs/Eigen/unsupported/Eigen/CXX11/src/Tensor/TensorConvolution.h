...
class IndexMapper {
 public:
  ...
  mapCudaInputPlaneToTensorInputOffset(Index p) const {
    Index inputIndex = 0;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapCudaOutputPlaneToTensorOutputOffset(Index p) const {
    Index outputIndex = 0;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      for (int d = NumDims - 1; d > NumKernelDims; --d) {
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapCudaInputKernelToTensorInputOffset(Index i) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapCudaOutputKernelToTensorOutputOffset(Index i) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapCudaInputKernelToTensorInputOffset(Index i, Index j) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapCudaOutputKernelToTensorOutputOffset(Index i, Index j) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapCudaInputKernelToTensorInputOffset(Index i, Index j, Index k) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
    return i * m_inputStrides[offset] + j * m_inputStrides[offset + 1] +
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapCudaOutputKernelToTensorOutputOffset(Index i, Index j, Index k) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
    return i * m_outputStrides[offset] + j * m_outputStrides[offset + 1] +
 ...
 private:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void preloadKernel() {
    ...
    if (in_place) {
      ...
      internal::TensorExecutor<const EvalTo, Device, PacketAccess>::run(
          evalToTmp, m_device);
...
__global__ void EigenConvolutionKernel1D(
  ...
  for (int p = first_plane + threadIdx.y; p < numPlanes; p += plane_stride) {
    ...
    const int plane_input_offset =
        indexMapper.mapCudaInputPlaneToTensorInputOffset(p);
    const int plane_kernel_offset = threadIdx.y * num_x_input;
#pragma unroll
    for (int i = threadIdx.x; i < num_x_input; i += blockDim.x) {
      const int tensor_index =
          plane_input_offset +
          indexMapper.mapCudaInputKernelToTensorInputOffset(i + first_x);
    ...
    const int plane_output_offset =
        indexMapper.mapCudaOutputPlaneToTensorOutputOffset(p);
...
#pragma unroll
      ...
      const int tensor_index =
          plane_output_offset +
          indexMapper.mapCudaOutputKernelToTensorOutputOffset(i + first_x);
...
struct TensorEvaluator<
  ...
  void executeEval(Scalar* data) const {
    ...
    const int maxBlocksPerProcessor =
        m_device.maxCudaThreadsPerMultiProcessor() / maxThreadsPerBlock;
    ...
    switch (NumKernelDims) {
      case 1: {
        ...
        switch (kernel_size) {
          case 4: {
            LAUNCH_CUDA_KERNEL((EigenConvolutionKernel1D<
        ...
        switch (kernel_size_x) {
          case 4: {
            switch (kernel_size_y) {
              case 7: {
                LAUNCH_CUDA_KERNEL((EigenConvolutionKernel2D<
                                       TensorEvaluator<InputArgType, GpuDevice>,
                                       Index, InputDims, 4, 7>),
                                   num_blocks, block_size, shared_mem, m_device,
