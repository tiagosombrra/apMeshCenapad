...
struct TensorEvaluator {
  ...
  static const int NumCoords = internal::traits<Derived>::NumDimensions > 0
                                   ? internal::traits<Derived>::NumDimensions
                                   : 0;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const Derived& m,
                                                        const Device& device)
      : m_data(const_cast<typename internal::traits<
                   Derived>::template MakePointer<Scalar>::Type>(m.data())),
        m_dims(m.dimensions()),
        m_device(device),
        m_impl(m) {}
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType
  coeff(const array<DenseIndex, NumCoords>& coords) const {
    eigen_assert(m_data);
  ...
  EIGEN_DEVICE_FUNC
      typename internal::traits<Derived>::template MakePointer<Scalar>::Type
      data() const {
    return m_data;
  }
...
template <>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Eigen::half loadConstant(
...
struct TensorEvaluator<const Derived, Device> {
  ...
  EIGEN_DEVICE_FUNC typename internal::traits<Derived>::template MakePointer<
      const Scalar>::Type
  data() const {
    return m_data;
  }
 ...
 protected:
  typename internal::traits<Derived>::template MakePointer<const Scalar>::Type
      m_data;
  Dimensions m_dims;
  const Device& m_device;
  const Derived& m_impl;
...
struct TensorEvaluator<const TensorCwiseNullaryOp<NullaryOp, ArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType
  packet(Index index) const {
    return m_wrapper.template packetOp<PacketReturnType, Index>(m_functor,
                                                                index);
  }
...
template <typename TernaryOp, typename Arg1Type, typename Arg2Type,
          typename Arg3Type, typename Device>
struct TensorEvaluator<
    const TensorCwiseTernaryOp<TernaryOp, Arg1Type, Arg2Type, Arg3Type>,
    Device> {
  typedef TensorCwiseTernaryOp<TernaryOp, Arg1Type, Arg2Type, Arg3Type> XprType;

  enum {
    IsAligned = TensorEvaluator<Arg1Type, Device>::IsAligned &
                TensorEvaluator<Arg2Type, Device>::IsAligned &
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType& op, const Device& device)
    ...
    EIGEN_STATIC_ASSERT(
        ...
        STORAGE_KIND_MUST_MATCH)
    EIGEN_STATIC_ASSERT(
        (internal::is_same<typename internal::traits<Arg1Type>::Index,
    ...
    eigen_assert(
        dimensions_match(m_arg1Impl.dimensions(), m_arg2Impl.dimensions()) &&
  ...
  EIGEN_DEVICE_FUNC CoeffReturnType coeff(Index index) const {
    return m_functor(m_arg1Impl.coeff(index), m_arg2Impl.coeff(index),
