...
struct MakeComplex<false> {
  template <typename T>
  EIGEN_DEVICE_FUNC std::complex<T> operator()(
      const std::complex<T>& val) const {
    return val;
  }
};
...
struct traits<TensorFFTOp<FFT, XprType, FFTResultType, FFTDir> >
  ...
  typedef typename conditional<FFTResultType == RealPart ||
                                   FFTResultType == ImagPart,
...
struct eval<TensorFFTOp<FFT, XprType, FFTResultType, FFTDirection>,
            Eigen::Dense> {
...
struct nested<TensorFFTOp<FFT, XprType, FFTResultType, FFTDirection>, 1,
              typename eval<TensorFFTOp<FFT, XprType, FFTResultType,
                                        FFTDirection> >::type> {
...
template <typename FFT, typename XprType, int FFTResultType, int FFTDir>
class TensorFFTOp
    : public TensorBase<TensorFFTOp<FFT, XprType, FFTResultType, FFTDir>,
                        ReadOnlyAccessors> {
 public:
  ...
  typedef typename internal::conditional<
      FFTResultType == RealPart || FFTResultType == ImagPart, RealScalar,
      ComplexScalar>::type OutputScalar;
  typedef OutputScalar CoeffReturnType;
  typedef typename Eigen::internal::nested<TensorFFTOp>::type Nested;
  typedef
      typename Eigen::internal::traits<TensorFFTOp>::StorageKind StorageKind;
  typedef typename Eigen::internal::traits<TensorFFTOp>::Index Index;

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorFFTOp(const XprType& expr,
                                                    const FFT& fft)
...
// Eval as rvalue
template <typename FFT, typename ArgType, typename Device, int FFTResultType,
          int FFTDir>
struct TensorEvaluator<const TensorFFTOp<FFT, ArgType, FFTResultType, FFTDir>,
                       Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(
    ...
    if (data) {
      ...
      m_data =
          (CoeffReturnType*)m_device.allocate(sizeof(CoeffReturnType) * m_size);
      evalToBuf(m_data);
      return true;
    }
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE CoeffReturnType
 ...
 private:
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void evalToBuf(OutputScalar* data) {
    const bool write_to_out =
        internal::is_same<OutputScalar, ComplexScalar>::value;
    ComplexScalar* buf =
        write_to_out
            ? (ComplexScalar*)data
            : (ComplexScalar*)m_device.allocate(sizeof(ComplexScalar) * m_size);

    for (Index i = 0; i < m_size; ++i) {
      buf[i] = MakeComplex<internal::is_same<InputScalar, RealScalar>::value>()(
          m_impl.coeff(i));
    }
    ...
    for (size_t i = 0; i < m_fft.size(); ++i) {
      ...
      ComplexScalar* line_buf =
          (ComplexScalar*)m_device.allocate(sizeof(ComplexScalar) * line_len);
      const bool is_power_of_two = isPowerOfTwo(line_len);
      const Index good_composite =
          is_power_of_two ? 0 : findGoodComposite(line_len);
      const Index log_len =
          is_power_of_two ? getLog2(line_len) : getLog2(good_composite);

      ComplexScalar* a = is_power_of_two
                             ? NULL
                             : (ComplexScalar*)m_device.allocate(
                                   sizeof(ComplexScalar) * good_composite);
      ComplexScalar* b = is_power_of_two
                             ? NULL
                             : (ComplexScalar*)m_device.allocate(
                                   sizeof(ComplexScalar) * good_composite);
      ...
      if (!is_power_of_two) {
        ...
        if (line_len > 1) {
          ...
          const ComplexScalar pos_j_base =
              ComplexScalar(std::cos(pi_over_len), std::sin(pi_over_len));
      ...
      for (Index partial_index = 0; partial_index < m_size / line_len;
           ++partial_index) {
        ...
        if (is_power_of_two) {
          ...
          processDataLineBluestein(line_buf, line_len, good_composite, log_len,
                                   a, b, pos_j_base_powered);
        }
        ...
        if (FFTDir == FFT_FORWARD && stride == 1) {
          ...
          for (int j = 0; j < line_len; ++j, offset += stride) {
            buf[offset] = (FFTDir == FFT_FORWARD) ? line_buf[j]
                                                  : line_buf[j] * div_factor;
          }
        }
      }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static Index findGoodComposite(
      Index n) {
    Index i = 2;
    while (i < 2 * n - 1) i *= 2;
    return i;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void processDataLineCooleyTukey(
      ComplexScalar* line_buf, Index line_len, Index log_len) {
    eigen_assert(isPowerOfTwo(line_len));
    scramble_FFT(line_buf, line_len);
    compute_1D_Butterfly<FFTDir>(line_buf, line_len, log_len);
  }

  // Call Bluestein's FFT algorithm, m is a good composite number greater than
  // (2 * n - 1), used as the padding length
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void processDataLineBluestein(
      ComplexScalar* line_buf, Index line_len, Index good_composite,
      Index log_len, ComplexScalar* a, ComplexScalar* b,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static void scramble_FFT(
      ComplexScalar* data, Index n) {
...
#define SQRT2DIV2 0.7071067811865476
  ...
  getBaseOffsetFromIndex(Index index, Index omitted_dim) const {
    Index result = 0;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index
  getIndexFromOffset(Index base, Index omitted_dim, Index offset) const {
