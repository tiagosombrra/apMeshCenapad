...
class TensorFixedSize
    : public TensorBase<
          TensorFixedSize<Scalar_, Dimensions_, Options_, IndexType> > {
 public:
 ...
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const Scalar& coeff(
      Index firstIndex, IndexTypes... otherIndices) const {
    // The number of indices used to access a tensor coefficient must be equal
    // to the rank of the tensor.
    EIGEN_STATIC_ASSERT(sizeof...(otherIndices) + 1 == NumIndices,
  ...
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE const Scalar& coeff(
  ...
  EIGEN_STRONG_INLINE const Scalar& operator()(Index i0, Index i1,
                                               Index i2) const {
    if (Options & RowMajor) {
      const Index index = i2 + m_storage.dimensions()[2] *
                                   (i1 + m_storage.dimensions()[1] * i0);
      return m_storage.data()[index];
    } else {
      const Index index = i0 + m_storage.dimensions()[0] *
                                   (i1 + m_storage.dimensions()[1] * i2);
      return m_storage.data()[index];
    }
  }
  EIGEN_DEVICE_FUNC
    ...
    if (Options & RowMajor) {
      const Index index =
          i3 + m_storage.dimensions()[3] *
                   (i2 + m_storage.dimensions()[2] *
                             (i1 + m_storage.dimensions()[1] * i0));
      return m_storage.data()[index];
    } else {
      const Index index =
          i0 + m_storage.dimensions()[0] *
                   (i1 + m_storage.dimensions()[1] *
                             (i2 + m_storage.dimensions()[2] * i3));
      return m_storage.data()[index];
    }
  }
  EIGEN_DEVICE_FUNC
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorFixedSize& operator=(
 ...
 protected:
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE bool checkIndexRange(
      const array<Index, NumIndices>& /*indices*/) const {
    ...
    /*       array_apply_and_reduce<logical_and_op,
  greater_equal_zero_op>(indices) &&
