...
template <typename XprType, template <class> class MakePointer_>
struct nested<TensorForcedEvalOp<XprType, MakePointer_>, 1,
...
template <typename XprType, template <class> class MakePointer_>
class TensorForcedEvalOp
    : public TensorBase<TensorForcedEvalOp<XprType, MakePointer_>,
                        ReadOnlyAccessors> {
 public:
...
struct TensorEvaluator<const TensorForcedEvalOp<ArgType, MakePointer_>,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(
    ...
    m_buffer = (CoeffReturnType*)m_device.allocate(numValues *
    ...
    const bool PacketAccess =
        internal::IsVectorizable<Device, const ArgType>::value;
    internal::TensorExecutor<const EvalTo,
                             typename internal::remove_const<Device>::type,
                             PacketAccess>::run(evalToTmp, m_device);
    return true;
  }
