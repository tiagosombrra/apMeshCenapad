...
class TensorImagePatchOp
    : public TensorBase<TensorImagePatchOp<Rows, Cols, XprType>,
                        ReadOnlyAccessors> {
 public:
  ...
  typedef typename Eigen::internal::traits<TensorImagePatchOp>::StorageKind
      StorageKind;
  typedef typename Eigen::internal::traits<TensorImagePatchOp>::Index Index;

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorImagePatchOp(
      const XprType& expr, DenseIndex patch_rows, DenseIndex patch_cols,
      DenseIndex row_strides, DenseIndex col_strides, DenseIndex in_row_strides,
...
struct TensorEvaluator<const TensorImagePatchOp<Rows, Cols, ArgType>, Device> {
  ...
  typedef
      typename internal::remove_const<typename XprType::Scalar>::type Scalar;
  typedef TensorEvaluator<const TensorImagePatchOp<Rows, Cols, ArgType>, Device>
      Self;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType& op,
    ...
    if (op.padding_explicit()) {
      ...
      switch (op.padding_type()) {
        case PADDING_VALID:
          ...
          m_rowPaddingTop =
              numext::maxi<Index>(0, ((m_outputRows - 1) * m_row_strides +
                                      m_patch_rows_eff - m_input_rows_eff) /
                                         2);
          m_colPaddingLeft =
              numext::maxi<Index>(0, ((m_outputCols - 1) * m_col_strides +
                                      m_patch_cols_eff - m_input_cols_eff) /
                                         2);
          break;
        case PADDING_SAME:
          ...
          m_outputCols = numext::ceil(m_input_cols_eff /
                                      static_cast<float>(m_col_strides));
          // Calculate the padding
          m_rowPaddingTop = ((m_outputRows - 1) * m_row_strides +
                             m_patch_rows_eff - m_input_rows_eff) /
                            2;
          m_colPaddingLeft = ((m_outputCols - 1) * m_col_strides +
                              m_patch_cols_eff - m_input_cols_eff) /
                             2;
          break;
        default:
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      m_patchStride =
          m_colStride * m_dimensions[NumDims - 3] * m_dimensions[NumDims - 1];
  ...
  coeff(Index index) const {
    ...
    const Index origInputRow =
        (m_row_inflate_strides == 1)
            ? inputRow
            : ((inputRow >= 0) ? (inputRow / m_fastInflateRowStride) : 0);
  ...
  packet(Index index) const {
    ...
    const Index otherIndex =
        (NumDims == 4) ? 0 : indices[0] / m_fastOtherStride;
