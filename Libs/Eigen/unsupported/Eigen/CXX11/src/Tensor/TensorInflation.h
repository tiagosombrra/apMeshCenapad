...
template <typename Strides, typename XprType>
struct nested<TensorInflationOp<Strides, XprType>, 1,
              typename eval<TensorInflationOp<Strides, XprType> >::type> {
...
template <typename Strides, typename XprType>
class TensorInflationOp : public TensorBase<TensorInflationOp<Strides, XprType>,
                                            ReadOnlyAccessors> {
 public:
  ...
  typedef typename Eigen::internal::traits<TensorInflationOp>::StorageKind
      StorageKind;
...
struct TensorEvaluator<const TensorInflationOp<Strides, ArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool getInputIndex(
      Index index, Index* inputIndex) const {
    ...
    *inputIndex = 0;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      *inputIndex += index / m_strides[0];
      ...
      if (index !=
          index / m_fastStrides[NumDims - 1] * m_strides[NumDims - 1]) {
        return false;
      }
