...
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE
  ...
  return 31 - index;
#else
  EIGEN_STATIC_ASSERT(sizeof(unsigned long long) == 8,
...
template <typename T>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint32_t muluh(const uint32_t a,
                                                     const T b) {
...
template <typename T>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint64_t muluh(const uint64_t a,
                                                     const T b) {
#if defined(__CUDA_ARCH__)
...
#else
  return (TensorUInt128<static_val<0>, uint64_t>(a) *
          TensorUInt128<static_val<0>, uint64_t>(b))
      .upper();
#endif
}
...
struct DividerHelper {
  static EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint32_t
  computeMultiplier(const int log_div, const T divider) {
    EIGEN_STATIC_ASSERT(N == 32, YOU_MADE_A_PROGRAMMING_MISTAKE);
    return static_cast<uint32_t>((static_cast<uint64_t>(1) << (N + log_div)) /
                                     divider -
...
struct DividerHelper<64, T> {
  static EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint64_t
  computeMultiplier(const int log_div, const T divider) {
#if defined(__SIZEOF_INT128__) && !defined(__CUDA_ARCH__)
    return static_cast<uint64_t>(
        (static_cast<__uint128_t>(1) << (64 + log_div)) /
            static_cast<__uint128_t>(divider) -
        (static_cast<__uint128_t>(1) << 64) + 1);
#else
    const uint64_t shift = 1ULL << log_div;
    TensorUInt128<uint64_t, uint64_t> result =
        TensorUInt128<uint64_t, static_val<0> >(shift, 0) /
            TensorUInt128<static_val<0>, uint64_t>(divider) -
 ...
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorIntDivisor(const T divider) {
    ...
    const int leading_zeros =
        count_leading_zeros(static_cast<UnsignedType>(divider));
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T divide(const T numerator) const {
    eigen_assert(static_cast<typename UnsignedTraits<T>::type>(numerator) <
                 NumTraits<UnsignedType>::highest() / 2);
    // eigen_assert(numerator >= 0); // this is implicitly asserted by the line
    // above
