...
template <typename PatchDim, typename XprType>
struct nested<TensorPatchOp<PatchDim, XprType>, 1,
              typename eval<TensorPatchOp<PatchDim, XprType> >::type> {
...
class TensorPatchOp
 ...
 public:
  ...
  typedef
      typename Eigen::internal::traits<TensorPatchOp>::StorageKind StorageKind;
...
struct TensorEvaluator<const TensorPatchOp<PatchDim, ArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType& op,
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      for (int i = 1; i < NumDims - 1; ++i) {
        ...
        m_patchStrides[i] =
            m_patchStrides[i - 1] * (input_dims[i - 1] - patch_dims[i - 1] + 1);
      }
      m_outputStrides[0] = 1;
  ...
  coeff(Index index) const {
    Index output_stride_index =
        (static_cast<int>(Layout) == static_cast<int>(ColMajor)) ? NumDims - 1
                                                                 : 0;
    ...
    Index patchOffset =
        index - patchIndex * m_outputStrides[output_stride_index];
    Index inputIndex = 0;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      for (int i = NumDims - 2; i > 0; --i) {
