...
struct InnerMostDimReducer<Self, Op, true> {
  ...
  reduce(const Self& self, typename Self::Index firstIndex,
    ...
    typename Self::PacketReturnType p =
...
template <int NPT, typename S, typename R, typename I>
__global__ void InnerReductionKernel(R, const S, I, I,
                                     typename S::CoeffReturnType*);

template <int NPT, typename S, typename R, typename I>
__global__ void OuterReductionKernel(R, const S, I, I,
                                     typename S::CoeffReturnType*);
#endif
...
struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>,
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC bool evalSubExprsIfNeeded(
    ...
    if ((RunningFullReduction && RunningOnSycl) ||
      ...
      Op reducer(m_reducer);
      internal::InnerReducer<Self, Op, Device>::run(*this, reducer, m_device,
                                                    data, num_values_to_reduce,
  ...
  /// added for sycl in order to re-construct the reduction eval on the device
  /// for the sub-kernel
...
#if defined(EIGEN_USE_GPU) && defined(__CUDACC__)
  template <int B, int N, typename S, typename R, typename I>
  friend void internal::FullReductionKernel(R, const S, I,
                                            typename S::CoeffReturnType*,
                                            unsigned int*);
#ifdef EIGEN_HAS_CUDA_FP16
  template <typename S, typename R, typename I>
  friend void internal::ReductionInitFullReduxKernelHalfFloat(R, const S, I,
                                                              half2*);
  template <int B, int N, typename S, typename R, typename I>
  friend void internal::FullReductionKernelHalfFloat(R, const S, I, half*,
                                                     half2*);
  template <int NPT, typename S, typename R, typename I>
  friend void internal::InnerReductionKernelHalfFloat(R, const S, I, I, half*);
#endif
  template <int NPT, typename S, typename R, typename I>
  friend void internal::InnerReductionKernel(R, const S, I, I,
                                             typename S::CoeffReturnType*);

  template <int NPT, typename S, typename R, typename I>
  friend void internal::OuterReductionKernel(R, const S, I, I,
                                             typename S::CoeffReturnType*);
#endif
