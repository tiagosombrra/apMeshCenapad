...
class TensorLazyEvaluatorReadOnly
    : public TensorLazyBaseEvaluator<
...
class TensorLazyEvaluator
    : public internal::conditional<
          bool(internal::is_lvalue<Expr>::value),
...
class TensorRef : public TensorBase<TensorRef<PlainObjectType> > {
 public:
  ...
  EIGEN_STRONG_INLINE TensorRef(const Expression& expr)
      : m_evaluator(new internal::TensorLazyEvaluator<Dimensions, Expression,
                                                      DefaultDevice>(
            expr, DefaultDevice())) {
  ...
  EIGEN_STRONG_INLINE TensorRef& operator=(const Expression& expr) {
    unrefEvaluator();
    m_evaluator =
        new internal::TensorLazyEvaluator<Dimensions, Expression,
                                          DefaultDevice>(expr, DefaultDevice());
    m_evaluator->incrRefCount();
    return *this;
  }
  ...
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE Scalar& coeffRef(Index i0, Index i1, Index i2, Index i3,
                                       Index i4) {
    array<Index, 5> indices;
    indices[0] = i0;
    indices[1] = i1;
...
struct TensorEvaluator<TensorRef<Derived>, Device>
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(TensorRef<Derived>& m,
                                                        const Device& d)
      : Base(m, d) {}
