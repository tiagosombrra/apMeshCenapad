...
template <typename Strides, typename XprType>
struct nested<TensorStridingOp<Strides, XprType>, 1,
              typename eval<TensorStridingOp<Strides, XprType> >::type> {
...
class TensorStridingOp
 ...
 public:
  ...
  typedef typename Eigen::internal::traits<TensorStridingOp>::StorageKind
      StorageKind;
  typedef typename Eigen::internal::traits<TensorStridingOp>::Index Index;

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorStridingOp(const XprType& expr,
                                                         const Strides& dims)
  ...
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE TensorStridingOp& operator=(
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorStridingOp& operator=(
...
struct TensorEvaluator<const TensorStridingOp<Strides, ArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType& op,
    ...
    for (int i = 0; i < NumDims; ++i) {
      m_dimensions[i] =
          ceilf(static_cast<float>(m_dimensions[i]) / op.strides()[i]);
    }
  ...
  costPerCoeff(bool vectorized) const {
    ...
    return m_impl.costPerCoeff(vectorized && m_inputStrides[innerDim] == 1) +
           // Computation is not vectorized per se, but it is done once per
           // packet.
