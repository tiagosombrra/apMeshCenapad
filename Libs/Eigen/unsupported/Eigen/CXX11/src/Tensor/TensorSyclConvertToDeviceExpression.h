...
template <template <class...> class NonOpCategory, bool IsConst,
          typename... Args>
struct NonOpConversion {
  typedef typename GetType<
      IsConst,
      NonOpCategory<typename ConvertToDeviceExpression<Args>::Type...> >::Type
      Type;
};

template <template <class, template <class> class> class NonOpCategory,
          bool IsConst, typename Args>
struct DeviceConvertor {
  typedef typename GetType<
      IsConst, NonOpCategory<typename ConvertToDeviceExpression<Args>::Type,
...
/// specialisation of the \ref ConvertToDeviceExpression struct when the node
/// type is TensorCwiseNullaryOp, TensorCwiseUnaryOp, TensorCwiseBinaryOp,
/// TensorCwiseTernaryOp, TensorBroadcastingOp
#define CATEGORYCONVERT(CVQual)                                             \
  ...
  struct ConvertToDeviceExpression<CVQual Category<OP, subExprs...> > {     \
    typedef CVQual                                                          \
        Category<OP, typename ConvertToDeviceExpression<subExprs>::Type...> \
            Type;                                                           \
  };
...
#define KERNELBROKERCONVERTREDUCTION(CVQual)                        \
  ...
  struct ConvertToDeviceExpression<                                 \
      CVQual TensorReductionOp<OP, Dim, subExpr, MakePointer_> > {  \
    typedef CVQual TensorReductionOp<                               \
        OP, Dim, typename ConvertToDeviceExpression<subExpr>::Type, \
        MakeGlobalPointer>                                          \
        Type;                                                       \
  };
