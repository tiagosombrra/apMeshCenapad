...
/// and recreate the expression with MakeGlobalPointer containing the device
/// address space for the TensorMap pointers used in eval function. It receives
/// the original expression type, the functor of the node, the tuple of
/// accessors, and the device expression type to re-instantiate the expression
/// tree for the device
...
#define TENSORMAP(CVQual)                                                      \
  ...
  struct ExprConstructor<                                                      \
      CVQual TensorMap<Tensor<Scalar_, NumIndices_, Options_, IndexType_>,     \
      ...
      Params...> {                                                             \
    typedef CVQual                                                             \
        TensorMap<Tensor<Scalar_, NumIndices_, Options_, IndexType_>,          \
                  Options2_, MakeGlobalPointer>                                \
            Type;                                                              \
    Type expr;                                                                 \
    template <typename FuncDetector>                                           \
    ExprConstructor(FuncDetector &fd,                                          \
...
#define UNARYCATEGORY(CVQual)                                            \
  template <template <class, class> class UnaryCategory, typename OP,    \
            typename OrigRHSExpr, typename RHSExpr, typename... Params>  \
  struct ExprConstructor<CVQual UnaryCategory<OP, OrigRHSExpr>,          \
                         CVQual UnaryCategory<OP, RHSExpr>, Params...> { \
...
#define BINARYCATEGORY(CVQual)                                                \
  template <template <class, class, class> class BinaryCategory, typename OP, \
            typename OrigLHSExpr, typename OrigRHSExpr, typename LHSExpr,     \
  ...
  struct ExprConstructor<CVQual BinaryCategory<OP, OrigLHSExpr, OrigRHSExpr>, \
    ...
    typedef CVQual BinaryCategory<OP, typename my_left_type::Type,            \
...
#define TERNARYCATEGORY(CVQual)                                              \
  template <template <class, class, class, class> class TernaryCategory,     \
            typename OP, typename OrigArg1Expr, typename OrigArg2Expr,       \
            typename OrigArg3Expr, typename Arg1Expr, typename Arg2Expr,     \
            typename Arg3Expr, typename... Params>                           \
  struct ExprConstructor<                                                    \
      CVQual TernaryCategory<OP, OrigArg1Expr, OrigArg2Expr, OrigArg3Expr>,  \
      CVQual TernaryCategory<OP, Arg1Expr, Arg2Expr, Arg3Expr>, Params...> { \
    ...
    typedef CVQual TernaryCategory<OP, typename my_arg1_type::Type,          \
...
#define SELECTOP(CVQual)                                                       \
  template <typename OrigIfExpr, typename OrigThenExpr, typename OrigElseExpr, \
            ...
            typename... Params>                                                \
  struct ExprConstructor<                                                      \
      CVQual TensorSelectOp<OrigIfExpr, OrigThenExpr, OrigElseExpr>,           \
      CVQual TensorSelectOp<IfExpr, ThenExpr, ElseExpr>, Params...> {          \
...
#define ASSIGN(CVQual)                                                         \
  ...
  struct ExprConstructor<CVQual TensorAssignOp<OrigLHSExpr, OrigRHSExpr>,      \
                         CVQual TensorAssignOp<LHSExpr, RHSExpr>, Params...> { \
    typedef ExprConstructor<OrigLHSExpr, LHSExpr, Params...> my_left_type;     \
    typedef ExprConstructor<OrigRHSExpr, RHSExpr, Params...> my_right_type;    \
    typedef CVQual TensorAssignOp<typename my_left_type::Type,                 \
                                  typename my_right_type::Type>                \
        Type;                                                                  \
    my_left_type lhsExpr;                                                      \
    my_right_type rhsExpr;                                                     \
...
#define EVALTO(CVQual)                                                        \
  ...
  struct ExprConstructor<CVQual TensorEvalToOp<OrigExpr, MakeGlobalPointer>,  \
                         CVQual TensorEvalToOp<Expr>, Params...> {            \
    typedef ExprConstructor<OrigExpr, Expr, Params...> my_expr_type;          \
    typedef typename TensorEvalToOp<OrigExpr, MakeGlobalPointer>::PointerType \
        my_buffer_type;                                                       \
    typedef CVQual                                                            \
        TensorEvalToOp<typename my_expr_type::Type, MakeGlobalPointer>        \
            Type;                                                             \
...
#define FORCEDEVAL(CVQual)                                                     \
  ...
  struct ExprConstructor<                                                      \
      CVQual TensorForcedEvalOp<OrigExpr, MakeGlobalPointer>,                  \
      CVQual PlaceHolder<CVQual TensorForcedEvalOp<DevExpr>, N>, Params...> {  \
    typedef CVQual TensorMap<                                                  \
        Tensor<                                                                \
            typename TensorForcedEvalOp<DevExpr, MakeGlobalPointer>::Scalar,   \
...
#define SYCLREDUCTIONEXPR(CVQual)                                              \
  template <typename OP, typename Dim, typename OrigExpr, typename DevExpr,    \
            size_t N, typename... Params>                                      \
  struct ExprConstructor<                                                      \
      CVQual TensorReductionOp<OP, Dim, OrigExpr, MakeGlobalPointer>,          \
      CVQual PlaceHolder<CVQual TensorReductionOp<OP, Dim, DevExpr>, N>,       \
      Params...> {                                                             \
    static const size_t NumIndices = ValueCondition<                           \
        TensorReductionOp<OP, Dim, DevExpr,                                    \
                          MakeGlobalPointer>::NumDimensions == 0,              \
        1,                                                                     \
        TensorReductionOp<OP, Dim, DevExpr,                                    \
                          MakeGlobalPointer>::NumDimensions>::Res;             \
    typedef CVQual                                                             \
        TensorMap<Tensor<typename TensorReductionOp<                           \
                             OP, Dim, DevExpr, MakeGlobalPointer>::Scalar,     \
                         NumIndices, 0,                                        \
