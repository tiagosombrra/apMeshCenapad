...
struct AccessorConstructor {
  ...
  static inline auto getTuple(cl::sycl::handler& cgh, Arg1 eval1, Arg2 eval2)
      -> decltype(utility::tuple::append(
          ExtractAccessor<Arg1>::getTuple(cgh, eval1),
  ...
  static inline auto getAccessor(cl::sycl::handler& cgh, Arg eval)
      -> decltype(utility::tuple::make_tuple(
          eval.device()
              .template get_sycl_accessor<
                  AcM, typename Eigen::internal::remove_all<
                           typename Arg::CoeffReturnType>::type>(
...
/// const TensorCwiseNullaryOp, const TensorCwiseUnaryOp and const
/// TensorBroadcastingOp
template <template <class, class> class UnaryCategory, typename OP,
          typename RHSExpr, typename Dev>
struct ExtractAccessor<
    TensorEvaluator<const UnaryCategory<OP, RHSExpr>, Dev> > {
...
template <template <class, class, class> class BinaryCategory, typename OP,
          typename LHSExpr, typename RHSExpr, typename Dev>
struct ExtractAccessor<
    TensorEvaluator<const BinaryCategory<OP, LHSExpr, RHSExpr>, Dev> > {
  static inline auto getTuple(
      cl::sycl::handler& cgh,
      const TensorEvaluator<const BinaryCategory<OP, LHSExpr, RHSExpr>, Dev>
          eval) -> decltype(AccessorConstructor::getTuple(cgh, eval.left_impl(),
                                                          eval.right_impl())) {
    return AccessorConstructor::getTuple(cgh, eval.left_impl(),
                                         eval.right_impl());
  }
};
/// specialisation of the \ref ExtractAccessor struct when the node type is
...
template <template <class, class, class> class BinaryCategory, typename OP,
          typename LHSExpr, typename RHSExpr, typename Dev>
struct ExtractAccessor<
    TensorEvaluator<BinaryCategory<OP, LHSExpr, RHSExpr>, Dev> >
    : ExtractAccessor<
          TensorEvaluator<const BinaryCategory<OP, LHSExpr, RHSExpr>, Dev> > {};
...
template <template <class, class, class, class> class TernaryCategory,
          typename OP, typename Arg1Expr, typename Arg2Expr, typename Arg3Expr,
          typename Dev>
struct ExtractAccessor<TensorEvaluator<
    const TernaryCategory<OP, Arg1Expr, Arg2Expr, Arg3Expr>, Dev> > {
  static inline auto getTuple(
      cl::sycl::handler& cgh,
      const TensorEvaluator<
          const TernaryCategory<OP, Arg1Expr, Arg2Expr, Arg3Expr>, Dev>
          eval) -> decltype(AccessorConstructor::getTuple(cgh, eval.arg1Impl(),
                                                          eval.arg2Impl(),
                                                          eval.arg3Impl())) {
    return AccessorConstructor::getTuple(cgh, eval.arg1Impl(), eval.arg2Impl(),
                                         eval.arg3Impl());
  }
};
...
template <template <class, class, class, class> class TernaryCategory,
          ...
          typename Dev>
struct ExtractAccessor<
    TensorEvaluator<TernaryCategory<OP, Arg1Expr, Arg2Expr, Arg3Expr>, Dev> >
    : ExtractAccessor<TensorEvaluator<
          const TernaryCategory<OP, Arg1Expr, Arg2Expr, Arg3Expr>, Dev> > {};
...
struct ExtractAccessor<
  ...
  static inline auto getTuple(
      ...
      const TensorEvaluator<const TensorSelectOp<IfExpr, ThenExpr, ElseExpr>,
                            Dev>
          eval) -> decltype(AccessorConstructor::getTuple(cgh, eval.cond_impl(),
                                                          eval.then_impl(),
                                                          eval.else_impl())) {
    return AccessorConstructor::getTuple(cgh, eval.cond_impl(),
                                         eval.then_impl(), eval.else_impl());
  }
};
...
struct ExtractAccessor<
    ...
    : ExtractAccessor<TensorEvaluator<
          const TensorSelectOp<IfExpr, ThenExpr, ElseExpr>, Dev> > {};
...
struct ExtractAccessor<
  ...
  static inline auto getTuple(
      ...
      -> decltype(AccessorConstructor::getTuple(cgh, eval.left_impl(),
                                                eval.right_impl())) {
    return AccessorConstructor::getTuple(cgh, eval.left_impl(),
                                         eval.right_impl());
  }
...
#define TENSORMAPEXPR(CVQual, ACCType)                                      \
  ...
  struct ExtractAccessor<                                                   \
      TensorEvaluator<CVQual TensorMap<PlainObjectType, Options_>, Dev> > { \
    static inline auto getTuple(                                            \
        cl::sycl::handler& cgh,                                             \
        const TensorEvaluator<CVQual TensorMap<PlainObjectType, Options_>,  \
                              Dev>                                          \
            eval)                                                           \
        -> decltype(AccessorConstructor::template getAccessor<ACCType>(     \
            cgh, eval)) {                                                   \
...
struct ExtractAccessor<TensorEvaluator<const TensorForcedEvalOp<Expr>, Dev> > {
  static inline auto getTuple(
      ...
      -> decltype(AccessorConstructor::template getAccessor<
                  cl::sycl::access::mode::read>(cgh, eval)) {
    return AccessorConstructor::template getAccessor<
        cl::sycl::access::mode::read>(cgh, eval);
  }
};
...
struct ExtractAccessor<TensorEvaluator<const TensorEvalToOp<Expr>, Dev> > {
  static inline auto getTuple(
      ...
      -> decltype(utility::tuple::append(
          AccessorConstructor::template getAccessor<
              cl::sycl::access::mode::write>(cgh, eval),
...
struct ExtractAccessor<
    TensorEvaluator<const TensorReductionOp<OP, Dim, Expr>, Dev> > {
  static inline auto getTuple(
      cl::sycl::handler& cgh,
