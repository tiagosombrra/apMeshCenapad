...
// This struct is used for leafNode (TensorMap) and nodes behaving like leafNode
// (TensorForcedEval).
...
struct FunctorExtractor<
  ...
  OP func;
  FunctorExtractor(
      const TensorEvaluator<const UnaryCategory<OP, RHSExpr>, Dev>& expr)
...
/// specialisation of the \ref FunctorExtractor struct when the node type is
/// const TensorCwiseSelectOp. This is an specialisation without OP so it has to
/// be separated.
...
/// specialisation of the \ref FunctorExtractor struct when the node type is
/// TensorCwiseSelectOp. This is an specialisation without OP so it has to be
/// separated
template <typename IfExpr, typename ThenExpr, typename ElseExpr, typename Dev>
struct FunctorExtractor<
...
/// specialisation of the \ref FunctorExtractor struct when the node type is
/// const TensorAssignOp. This is an specialisation without OP so it has to be
/// separated.
...
/// specialisation of the \ref FunctorExtractor struct when the node type is
/// TensorAssignOp. This is an specialisation without OP so it has to be
/// separated.
template <typename LHSExpr, typename RHSExpr, typename Dev>
...
/// specialisation of the \ref FunctorExtractor struct when the node type is
/// const TensorEvalToOp, This is an specialisation without OP so it has to be
/// separated.
...
/// specialisation of the \ref FunctorExtractor struct when the node type is
/// TensorEvalToOp. This is a specialisation without OP so it has to be
/// separated.
template <typename RHSExpr, typename Dev>
...
struct DimConstr {
  ...
  static inline Dim getDim(InDim dims) {
    return dims;
  }
};
...
struct FunctorExtractor<TensorEvaluator<
  ...
  typedef typename Eigen::internal::conditional<
      Evaluator::NumOutputDims == 0, DSizes<typename Evaluator::Index, 1>,
  ...
  FunctorExtractor(const TensorEvaluator<
                   ...
                   Device>& expr)
      : m_dimensions(DimConstr<Dimensions, Evaluator::NumOutputDims>::getDim(
            expr.dimensions())) {}
};
...
template <typename Op, typename Dims, typename ArgType,
          template <class> class MakePointer_, typename Device>
struct FunctorExtractor<
    TensorEvaluator<TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>>
    : FunctorExtractor<TensorEvaluator<
          const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>> {};
/// template deduction function for FunctorExtractor
template <typename Evaluator>
auto inline extractFunctors(const Evaluator& evaluator)
