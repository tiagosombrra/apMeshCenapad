...
template <DenseIndex Planes, DenseIndex Rows, DenseIndex Cols, typename XprType>
struct nested<
    TensorVolumePatchOp<Planes, Rows, Cols, XprType>, 1,
...
class TensorVolumePatchOp
    : public TensorBase<TensorVolumePatchOp<Planes, Rows, Cols, XprType>,
                        ReadOnlyAccessors> {
 public:
  ...
  typedef typename Eigen::internal::traits<TensorVolumePatchOp>::StorageKind
      StorageKind;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorVolumePatchOp(
      const XprType& expr, DenseIndex patch_planes, DenseIndex patch_rows,
      DenseIndex patch_cols, DenseIndex plane_strides, DenseIndex row_strides,
      DenseIndex col_strides, DenseIndex in_plane_strides,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorVolumePatchOp(
      ...
      DenseIndex in_row_strides, DenseIndex in_col_strides,
      DenseIndex plane_inflate_strides, DenseIndex row_inflate_strides,
      DenseIndex col_inflate_strides, DenseIndex padding_top_z,
      DenseIndex padding_bottom_z, DenseIndex padding_top,
...
template <DenseIndex Planes, DenseIndex Rows, DenseIndex Cols, typename ArgType,
          typename Device>
struct TensorEvaluator<const TensorVolumePatchOp<Planes, Rows, Cols, ArgType>,
                       Device> {
    ...
    if (op.padding_explicit()) {
      ...
      switch (op.padding_type()) {
        case PADDING_VALID:
          ...
          break;
        case PADDING_SAME: {
          m_outputPlanes = numext::ceil(m_input_planes_eff /
                                        static_cast<float>(m_plane_strides));
          m_outputRows = numext::ceil(m_input_rows_eff /
                                      static_cast<float>(m_row_strides));
          m_outputCols = numext::ceil(m_input_cols_eff /
                                      static_cast<float>(m_col_strides));
          const Index dz = m_outputPlanes * m_plane_strides +
                           m_patch_planes_eff - 1 - m_input_planes_eff;
          const Index dy = m_outputRows * m_row_strides + m_patch_rows_eff - 1 -
                           m_input_rows_eff;
          const Index dx = m_outputCols * m_col_strides + m_patch_cols_eff - 1 -
                           m_input_cols_eff;
