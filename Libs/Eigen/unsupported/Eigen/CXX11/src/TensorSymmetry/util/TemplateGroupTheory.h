...
template <template <typename, typename> class Equality, typename id, typename t,
          typename... ts>
struct strip_identities<Equality, id, type_list<t, ts...>> {
  typedef typename conditional<
      Equality<id, t>::value,
      typename strip_identities<Equality, id, type_list<ts...>>::type,
      typename concat<type_list<t>,
                      typename strip_identities<
                          Equality, id, type_list<ts...>>::type>::type>::type
      type;
  constexpr static int global_flags =
      Equality<id, t>::global_flags |
      strip_identities<Equality, id, type_list<ts...>>::global_flags;
};

template <template <typename, typename> class Equality,
          typename id EIGEN_TPL_PP_SPEC_HACK_DEFC(typename, ts)>
struct strip_identities<Equality, id,
                        type_list<EIGEN_TPL_PP_SPEC_HACK_USE(ts)>> {
 ...
 * \ingroup CXX11_TensorSymmetry_Module
 *
 * \brief Recursive template that adds powers of the first generator to the list
 * of group elements
 *
...
template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id, typename g,
          typename current_element, typename elements,
...
template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id, typename g,
          typename current_element, typename elements>
struct dimino_first_step_elements_helper<Multiply, Equality, id, g,
                                         current_element, elements, true>
#endif  // EIGEN_PARSED_BY_DOXYGEN
{
  typedef elements type;
  constexpr static int global_flags =
      Equality<current_element, id>::global_flags;
};
 ...
 *
 * This template takes the first non-identity generator and generates the
 * initial list of elements which consists of all powers of that generator. For
 * a group with just one generated, it would be enumerated after this.
 *
 * \sa enumerate_group_elements
 */
template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id,
          typename generators>
struct dimino_first_step_elements {
  ...
  typedef dimino_first_step_elements_helper<Multiply, Equality, id,
 ...
 */
template <template <typename, typename> class Multiply,
          typename sub_group_elements, typename new_coset_rep,
          bool generate_coset  // = true
          >
struct dimino_get_coset_elements {
  typedef typename apply_op_from_right<Multiply, new_coset_rep,
                                       sub_group_elements>::type type;
};

template <template <typename, typename> class Multiply,
          typename sub_group_elements, typename new_coset_rep>
struct dimino_get_coset_elements<Multiply, sub_group_elements, new_coset_rep,
                                 false> {
  typedef type_list<> type;
};
...
template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id,
          typename sub_group_elements, typename elements, typename generators,
          typename rep_element, int sub_group_size>
struct dimino_add_cosets_for_rep;

template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id,
          typename sub_group_elements, typename elements, typename g,
          typename... gs, typename rep_element, int sub_group_size>
struct dimino_add_cosets_for_rep<Multiply, Equality, id, sub_group_elements,
                                 elements, type_list<g, gs...>, rep_element,
                                 sub_group_size> {
  ...
  typedef typename dimino_get_coset_elements<Multiply, sub_group_elements,
                                             new_coset_rep, add_coset>::type
      coset_elements;

  typedef dimino_add_cosets_for_rep<
      Multiply, Equality, id, sub_group_elements,
      typename concat<elements, coset_elements>::type, type_list<gs...>,
      rep_element, sub_group_size>
      _helper;

  typedef typename _helper::type type;
  constexpr static int global_flags =
      _cil::global_flags | _helper::global_flags;
...
template <template <typename, typename> class Multiply,
          ...
          typename rep_element, int sub_group_size>
struct dimino_add_cosets_for_rep<
    Multiply, Equality, id, sub_group_elements, elements,
...
template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id,
          typename sub_group_elements, typename elements, typename generators,
          int sub_group_size, int rep_pos,
...
struct dimino_add_all_coset_spaces {
  ...
  typedef dimino_add_cosets_for_rep<Multiply, Equality, id, sub_group_elements,
                                    elements, generators, rep_element,
  ...
  typedef dimino_add_all_coset_spaces<
      Multiply, Equality, id, sub_group_elements, new_elements, generators,
      sub_group_size, new_rep_pos, new_stop_condition>
      _helper;

  typedef typename _helper::type type;
  constexpr static int global_flags =
      _helper::global_flags | _ac4r::global_flags;
};
...
template <template <typename, typename> class Multiply,
          ...
          typename sub_group_elements, typename elements, typename generators,
          int sub_group_size, int rep_pos>
struct dimino_add_all_coset_spaces<Multiply, Equality, id, sub_group_elements,
                                   elements, generators, sub_group_size,
                                   rep_pos, true> {
  typedef elements type;
  constexpr static int global_flags = 0;
};
...
template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id,
          typename elements, typename generators_done,
...
struct dimino_add_generator {
   ...
   */
  typedef
      typename apply_op_from_right<Multiply, current_generator, elements>::type
          multiplied_elements;
  ...
  typedef dimino_add_all_coset_spaces<
      ...
      rep_pos,
      false  // don't stop (because rep_pos >= new_elements::count is always
             // false at this point)
      >
      _helper;
  typedef typename _helper::type type;
...
template <template <typename, typename> class Multiply,
          ...
          typename elements, typename generators_done,
          typename current_generator>
struct dimino_add_generator<Multiply, Equality, id, elements, generators_done,
                            current_generator, true> {
  // redundant case
  typedef elements type;
  constexpr static int global_flags = 0;
};
...
template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id,
          typename generators_done, typename remaining_generators,
          typename elements>
struct dimino_add_remaining_generators {
  ...
  typedef dimino_add_generator<Multiply, Equality, id, elements,
                               generators_done, first_generator, _cil::value>
      _helper;
  ...
  constexpr static int global_flags =
      _cil::global_flags | _helper::global_flags | _next_iter::global_flags;
};

template <template <typename, typename> class Multiply,
          template <typename, typename> class Equality, typename id,
          typename generators_done, typename elements>
struct dimino_add_remaining_generators<Multiply, Equality, id, generators_done,
                                       type_list<>, elements> {
  typedef elements type;
  constexpr static int global_flags = 0;
};
...
struct enumerate_group_elements_noid {
  typedef dimino_first_step_elements<Multiply, Equality, id, generators>
      first_step;
  typedef typename first_step::type first_step_elements;

  typedef dimino_add_remaining_generators<
      Multiply, Equality, id, typename first_step::generators_done,
  ...
  typedef typename _helper::type type;
  constexpr static int global_flags =
      initial_global_flags | first_step::global_flags | _helper::global_flags;
};
...
struct enumerate_group_elements_noid<Multiply, Equality, id, type_list<>,
 ...
 *
 * \tparam Multiply      The multiplication operation that multiplies two group
 * elements with each other. \tparam Equality      The equality check operation
 * that checks if two group elements are equal to another. \tparam id The
 * identity element \tparam _generators   A list of (possibly redundant)
