...
// Emulate the cxx11 functionality that we need if the compiler doesn't support
// it. Visual studio 2015 doesn't advertise itself as cxx11 compliant, although
// it supports enough of the standard for our needs
 ...
 * related to Eigen. This file expands upon Core/util/Meta.h and adds support
 * for C++11 specific features.
 */
 ...
 *     typename gen_numeric_list_reversed<int, 5>::type numeric_list<int,
 * 4,3,2,1,0> typename gen_numeric_list_swapped_pair<int, 5,1,2>::type
 * numeric_list<int, 0,2,1,3,4> typename gen_numeric_list_repeated<int, 0,
 * 5>::type      numeric_list<int, 0,0,0,0,0>
 */

template <typename T, std::size_t n, T start = 0, T... ii>
struct gen_numeric_list
    : gen_numeric_list<T, n - 1, start, start + n - 1, ii...> {};
...
struct gen_numeric_list_reversed
    : gen_numeric_list_reversed<T, n - 1, start, ii..., start + n - 1> {};
template <typename T, T start, T... ii>
...
struct gen_numeric_list_swapped_pair
    : gen_numeric_list_swapped_pair<
          T, n - 1, a, b, start,
          (start + n - 1) == a ? b
                               : ((start + n - 1) == b ? a : (start + n - 1)),
          ii...> {};
...
struct gen_numeric_list_repeated
    : gen_numeric_list_repeated<T, n - 1, V, V, nn...> {};
...
struct take<n, numeric_list<T, a, as...>>
    : concat<numeric_list<T, a>,
             typename take<n - 1, numeric_list<T, as...>>::type> {};
...
struct h_skip_helper_numeric<T, n, i, ii...>
    : h_skip_helper_numeric<T, n - 1, ii...> {};
...
struct h_skip {
  ...
  helper(numeric_list<T, ii...>) {
    return typename h_skip_helper_numeric<T, n, ii...>::type();
  }
  template <typename... tt>
  constexpr static inline typename h_skip_helper_type<n, tt...>::type helper(
      type_list<tt...>) {
...
template <bool from_left,  // false
          template <typename, typename> class op, typename additional_param,
          typename... values>
struct h_apply_op_helper {
  typedef type_list<typename op<values, additional_param>::type...> type;
};
template <template <typename, typename> class op, typename additional_param,
          typename... values>
struct h_apply_op_helper<true, op, additional_param, values...> {
  typedef type_list<typename op<additional_param, values>::type...> type;
};

template <bool from_left, template <typename, typename> class op,
          typename additional_param>
struct h_apply_op {
  template <typename... values>
  constexpr static typename h_apply_op_helper<from_left, op, additional_param,
                                              values...>::type
  helper(type_list<values...>) {
    return typename h_apply_op_helper<from_left, op, additional_param,
                                      values...>::type();
  }
};
...
template <template <typename, typename> class test, typename check_against,
          typename h_list, bool last_check_positive = false>
...
struct contained_in_list<test, check_against, type_list<a, as...>, false>
    : contained_in_list<test, check_against, type_list<as...>,
...
struct contained_in_list<test, check_against,
                         type_list<EIGEN_TPL_PP_SPEC_HACK_USE(empty)>, false> {
...
template <template <typename, typename> class test, typename check_against,
          typename h_list, int default_flags = 0,
          bool last_check_positive = false,
          int last_check_flags = default_flags>
struct contained_in_list_gf;

template <template <typename, typename> class test, typename check_against,
          typename h_list, int default_flags, int last_check_flags>
struct contained_in_list_gf<test, check_against, h_list, default_flags, true,
                            last_check_flags> {
  constexpr static bool value = true;
...
template <template <typename, typename> class test, typename check_against,
          typename a, typename... as, int default_flags, int last_check_flags>
struct contained_in_list_gf<test, check_against, type_list<a, as...>,
                            default_flags, false, last_check_flags>
    : contained_in_list_gf<test, check_against, type_list<as...>, default_flags,
                           test<check_against, a>::value,
...
struct contained_in_list_gf<test, check_against,
...
struct reduce<Reducer, A, Ts...> {
  constexpr static inline auto run(A a, Ts... ts)
...
struct logical_and_op {
  template <typename A, typename B>
  constexpr static inline auto run(A a, B b) -> decltype(a && b) {
    return a && b;
  }
};
...
struct not_op {
  template <typename A>
  constexpr static inline auto run(A a) -> decltype(!a) {
    return !a;
  }
};
struct negation_op {
...
struct greater_equal_zero_op {
  template <typename A>
  constexpr static inline auto run(A a) -> decltype(a >= 0) {
    return a >= 0;
  }
};
...
// using auto -> return value spec makes ICC 13.0 and 13.1 crash here, so we
// have to hack it together in front... (13.0 doesn't work with
// array_prod/array_reduce/... anyway, but 13.1 does...
template <typename... Ts>
constexpr inline decltype(reduce<product_op, Ts...>::run((*((Ts*)0))...))
arg_prod(Ts... ts) {
  return reduce<product_op, Ts...>::run(ts...);
}

template <typename... Ts>
constexpr inline decltype(reduce<sum_op, Ts...>::run((*((Ts*)0))...)) arg_sum(
    Ts... ts) {
...
struct h_array_reduce {
  EIGEN_DEVICE_FUNC constexpr static inline auto run(array<T, N> arr,
                                                     T identity)
      -> decltype(Reducer::run(
          h_array_reduce<Reducer, T, N, n - 1>::run(arr, identity),
          array_get<n>(arr))) {
    return Reducer::run(
        h_array_reduce<Reducer, T, N, n - 1>::run(arr, identity),
        array_get<n>(arr));
...
struct h_array_reduce<Reducer, T, 0> {
  EIGEN_DEVICE_FUNC constexpr static inline T run(const array<T, 0>&,
                                                  T identity) {
    return identity;
  }
};
...
template <typename Reducer, typename T, std::size_t N>
EIGEN_DEVICE_FUNC constexpr inline auto array_reduce(const array<T, N>& arr,
                                                     T identity)
...
constexpr inline array<decltype(Op::run(A(), B())), N> h_array_zip(
    array<A, N> a, array<B, N> b, numeric_list<int, n...>) {
  return array<decltype(Op::run(A(), B())), N>{
      {Op::run(array_get<n>(a), array_get<n>(b))...}};
}

template <typename Op, typename A, typename B, std::size_t N>
constexpr inline array<decltype(Op::run(A(), B())), N> array_zip(
    array<A, N> a, array<B, N> b) {
...
constexpr inline auto h_array_zip_and_reduce(array<A, N> a, array<B, N> b,
    ...
    -> decltype(reduce<Reducer,
                       typename id_numeric<
                           int, n, decltype(Op::run(A(), B()))>::type...>::
                    run(Op::run(array_get<n>(a), array_get<n>(b))...)) {
  return reduce<Reducer,
                typename id_numeric<int, n, decltype(Op::run(A(), B()))>::
                    type...>::run(Op::run(array_get<n>(a), array_get<n>(b))...);
}

template <typename Reducer, typename Op, typename A, typename B, std::size_t N>
constexpr inline auto array_zip_and_reduce(array<A, N> a, array<B, N> b)
    -> decltype(h_array_zip_and_reduce<Reducer, Op, A, B, N>(
        a, b, typename gen_numeric_list<int, N>::type())) {
  return h_array_zip_and_reduce<Reducer, Op, A, B, N>(
      a, b, typename gen_numeric_list<int, N>::type());
}
...
constexpr inline array<decltype(Op::run(A())), N> h_array_apply(
    array<A, N> a, numeric_list<int, n...>) {
...
constexpr inline auto h_array_apply_and_reduce(array<A, N> arr,
                                               numeric_list<int, n...>)
    -> decltype(reduce<Reducer,
                       typename id_numeric<int, n, decltype(Op::run(A()))>::
                           type...>::run(Op::run(array_get<n>(arr))...)) {
  return reduce<Reducer, typename id_numeric<int, n, decltype(Op::run(A()))>::
                             type...>::run(Op::run(array_get<n>(arr))...);
}

template <typename Reducer, typename Op, typename A, std::size_t N>
constexpr inline auto array_apply_and_reduce(array<A, N> a)
    -> decltype(h_array_apply_and_reduce<Reducer, Op, A, N>(
        a, typename gen_numeric_list<int, N>::type())) {
  return h_array_apply_and_reduce<Reducer, Op, A, N>(
      a, typename gen_numeric_list<int, N>::type());
}
...
template <class InstType, typename ArrType, std::size_t N, bool Reverse,
          typename... Ps>
...
struct h_instantiate_by_c_array<InstType, ArrType, N, false, Ps...> {
  static InstType run(ArrType* arr, Ps... args) {
    return h_instantiate_by_c_array<InstType, ArrType, N - 1, false, Ps...,
                                    ArrType>::run(arr + 1, args..., arr[0]);
  }
};
...
struct h_instantiate_by_c_array<InstType, ArrType, N, true, Ps...> {
  static InstType run(ArrType* arr, Ps... args) {
    return h_instantiate_by_c_array<InstType, ArrType, N - 1, false, ArrType,
                                    Ps...>::run(arr + 1, arr[0], args...);
  }
};
