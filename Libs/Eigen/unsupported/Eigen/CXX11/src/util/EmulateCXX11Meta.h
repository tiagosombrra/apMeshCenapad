...
struct gen_numeric_list_repeated<T, 3, V> {
  typedef typename make_type_list<type2val<T, V>, type2val<T, V>,
...
struct gen_numeric_list_repeated<T, 5, V> {
  typedef
      typename make_type_list<type2val<T, V>, type2val<T, V>, type2val<T, V>,
                              type2val<T, V>, type2val<T, V> >::type type;
};
...
template <std::size_t I, class Head, class Tail>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename Head::type array_get(
    type_list<Head, Tail>&) {
...
template <class NList>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename NList::HeadType::type array_prod(
    const NList&) {
...
template <std::size_t I, class T>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const T& array_get(
