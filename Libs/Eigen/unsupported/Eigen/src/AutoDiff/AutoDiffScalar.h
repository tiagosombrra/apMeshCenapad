 ...
 * determined from this type. Typical choices include, e.g., \c Vector4f for 4
 * derivatives, or \c VectorXf if the number of derivatives is not known at
 * compile time, and/or, the number of derivatives is large. Note that _DerType
 * can also be a reference (e.g., \c VectorXf&) to wrap a existing vector into
 * an AutoDiffScalar. Finally, _DerType can also be any Eigen compatible
 * expression.
 *
...
void make_coherent_expression(CwiseBinaryOp<BinOp, A, B> xpr,
                              const RefType& ref) {
...
void make_coherent_expression(const CwiseUnaryOp<UnaryOp, A>& xpr,
                              const RefType& ref) {
...
template <typename UnaryOp, typename A, typename RefType>
void make_coherent_expression(const CwiseNullaryOp<UnaryOp, A>&,
                              const RefType&) {}
...
struct make_coherent_impl<
  ...
  static void run(A& a, B& b) {
    if ((A_Rows == Dynamic || A_Cols == Dynamic) && (a.size() == 0)) {
      a.resize(b.size());
      a.setZero();
    } else if (B::SizeAtCompileTime == Dynamic && a.size() != 0 &&
               b.size() == 0) {
...
struct make_coherent_impl<
  ...
  static void run(A& a, B& b) {
    if ((B_Rows == Dynamic || B_Cols == Dynamic) && (b.size() == 0)) {
      b.resize(a.size());
      b.setZero();
    } else if (A::SizeAtCompileTime == Dynamic && b.size() != 0 &&
               a.size() == 0) {
