...
// these templates help the tree initializer get the bounding boxes either from
// a provided iterator range or using bounding_box in a unified way
template <typename ObjectList, typename VolumeList, typename BoxIter>
struct get_boxes_helper {
  void operator()(const ObjectList &objects, BoxIter boxBegin, BoxIter boxEnd,
                  VolumeList &outBoxes) {
 ...
 *  \param _Object The object type that lives in the hierarchy.  It must have
 * value semantics.  Either bounding_box(_Object) must be defined and return an
 * AlignedBox<_Scalar, _Dim> or bounding boxes must be provided to the tree
 * initializer.
 *
 *  This class provides a simple (as opposed to optimized) implementation of a
 * bounding volume hierarchy analogous to a Kd-tree. Given a sequence of
 * objects, it computes their bounding boxes, constructs a Kd-tree of their
 * centers and builds a BVH with the structure of that Kd-tree.  When the
 * elements of the tree are too expensive to be copied around, it is useful for
 * _Object to be a pointer.
 */
...
class KdBVH {
 public:
   ...
   * over their bounding boxes, constructs the BVH */
  ...
  /** Given an iterator range over \a Object references, constructs the BVH,
   * overwriting whatever is in there currently. Requires that
  ...
  /** Given an iterator range over \a Object references and an iterator range
   * over their bounding boxes, constructs the BVH, overwriting whatever is in
   * there currently. */
  template <typename OIter, typename BIter>
  void init(OIter begin, OIter end, BIter boxBegin, BIter boxEnd) {
    ...
    if (n < 2)
      return;  // if we have at most one object, we don't need any internal
               // nodes
    ...
    build(objCenters, 0, n, objBoxes, 0);  // the recursive part of the
                                           // algorithm
   ...
   * over the indices of the volume children of the node and \a outOBegin and \a
   * outOEnd range over the object children of the node */
  EIGEN_STRONG_INLINE void getChildren(Index index, VolumeIterator &outVBegin,
                                       VolumeIterator &outVEnd,
                                       ObjectIterator &outOBegin,
                                       ObjectIterator &outOEnd)
      const {  // inlining this function should open lots of optimization
               // opportunities to the compiler
    if (index < 0) {
      outVBegin = outVEnd;
      if (!objects.empty()) outOBegin = &(objects[0]);
      outOEnd =
          outOBegin + objects.size();  // output all objects--necessary when the
                                       // tree has only one object
      return;
    }
 ...
 private:
  ...
  typedef Matrix<Scalar, Dim, 1> VectorType;
  struct VectorComparator  // compares vectors, or, more specificall, VIPairs
  ...
  // Build the part of the tree between objects[from] and objects[to] (not
  // including objects[to]). This routine partitions the objCenters in [from,
  // to) along the dimension dim, recursively constructs the two halves, and
  // adds their parent node.  TODO: a cache-friendlier layout
  void build(VIPairList &objCenters, int from, int to,
             const VolumeList &objBoxes, int dim) {
    eigen_assert(to - from > 1);
    if (to - from == 2) {
      boxes.push_back(objBoxes[objCenters[from].second].merged(
          objBoxes[objCenters[from + 1].second]));
      children.push_back(from + (int)objects.size() -
                         1);  // there are objects.size() - 1 tree nodes
      children.push_back(from + (int)objects.size());
    } else if (to - from == 3) {
      int mid = from + 2;
  ...
  std::vector<int>
      children;  // children of x are children[2x] and children[2x+1], indices
                 // bigger than boxes.size() index into objects.
  VolumeList boxes;
  ObjectList objects;
};
