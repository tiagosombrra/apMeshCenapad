...
class ArpackGeneralizedSelfAdjointEigenSolver {
 public:
  ...
  /** \brief Constructor; computes generalized eigenvalues of given matrix with
   * respect to another matrix.
   *
   * \param[in] A Self-adjoint matrix whose eigenvalues / eigenvectors will
   *    computed. By default, the upper triangular part is used, but can be
   * changed through the template parameter. \param[in] B Self-adjoint matrix
   * for the generalized eigenvalue problem. \param[in] nbrEigenvalues The
   * number of eigenvalues / eigenvectors to compute. Must be less than the size
   * of the input matrix, or an error is returned. \param[in] eigs_sigma String
   * containing either "LM", "SM", "LA", or "SA", with respective meanings to
   * find the largest magnitude , smallest magnitude, largest algebraic, or
   * smallest algebraic eigenvalues. Alternatively, this value can contain
   * floating point value in string form, in which case the eigenvalues closest
   * to this value will be found. \param[in]  options Can be
   * #ComputeEigenvectors (default) or #EigenvaluesOnly. \param[in] tol What
   * tolerance to find the eigenvalues to. Default is 0, which means machine
   * precision.
   *
   * This constructor calls compute(const MatrixType&, const MatrixType&, Index,
   * string, int, RealScalar) to compute the eigenvalues of the matrix \p A with
   * respect to \p B. The eigenvectors are computed if \p options equals
   * #ComputeEigenvectors.
   *
   */
   ...
   * \param[in] A Self-adjoint matrix whose eigenvalues / eigenvectors will
   *    computed. By default, the upper triangular part is used, but can be
   * changed through the template parameter. \param[in] nbrEigenvalues The
   * number of eigenvalues / eigenvectors to compute. Must be less than the size
   * of the input matrix, or an error is returned. \param[in] eigs_sigma String
   * containing either "LM", "SM", "LA", or "SA", with respective meanings to
   * find the largest magnitude , smallest magnitude, largest algebraic, or
   ...
   * precision.
   *
   * This constructor calls compute(const MatrixType&, Index, string, int,
   * RealScalar) to compute the eigenvalues of the matrix \p A. The eigenvectors
   * are computed if \p options equals #ComputeEigenvectors.
   *
   */
  ...
  /** \brief Computes generalized eigenvalues / eigenvectors of given matrix
   * using the external ARPACK library.
   *
   * \param[in]  A  Selfadjoint matrix whose eigendecomposition is to be
   * computed. \param[in]  B  Selfadjoint matrix for generalized eigenvalues.
   * \param[in] nbrEigenvalues The number of eigenvalues / eigenvectors to
   * compute. Must be less than the size of the input matrix, or an error is
   * returned. \param[in] eigs_sigma String containing either "LM", "SM", "LA",
   * or "SA", with respective meanings to find the largest magnitude , smallest
   * magnitude, largest algebraic, or smallest algebraic eigenvalues.
   * Alternatively, this value can contain floating point value in string form,
   * in which case the eigenvalues closest to this value will be found.
   * \param[in]  options Can be #ComputeEigenvectors (default) or
   * #EigenvaluesOnly. \param[in] tol What tolerance to find the eigenvalues to.
   * Default is 0, which means machine precision.
   *
   * \returns    Reference to \c *this
   *
   * This function computes the generalized eigenvalues of \p A with respect to
   * \p B using ARPACK.  The eigenvalues() function can be used to retrieve
   * them.  If \p options equals #ComputeEigenvectors, then the eigenvectors are
   * also computed and can be retrieved by calling eigenvectors().
   *
   */
  ArpackGeneralizedSelfAdjointEigenSolver &compute(
      const MatrixType &A, const MatrixType &B, Index nbrEigenvalues,
      std::string eigs_sigma = "LM", int options = ComputeEigenvectors,
      RealScalar tol = 0.0);

  /** \brief Computes eigenvalues / eigenvectors of given matrix using the
   * external ARPACK library.
   *
   * \param[in]  A  Selfadjoint matrix whose eigendecomposition is to be
   * computed. \param[in] nbrEigenvalues The number of eigenvalues /
   * eigenvectors to compute. Must be less than the size of the input matrix, or
   * an error is returned. \param[in] eigs_sigma String containing either "LM",
   * "SM", "LA", or "SA", with respective meanings to find the largest magnitude
   * , smallest magnitude, largest algebraic, or smallest algebraic eigenvalues.
   * Alternatively, this value can contain floating point value in string form,
   * in which case the eigenvalues closest to this value will be found.
   * \param[in]  options Can be #ComputeEigenvectors (default) or
   * #EigenvaluesOnly. \param[in] tol What tolerance to find the eigenvalues to.
   ...
   * \returns    Reference to \c *this
   *
   * This function computes the eigenvalues of \p A using ARPACK.  The
   * eigenvalues() function can be used to retrieve them.  If \p options equals
   * #ComputeEigenvectors, then the eigenvectors are also computed and can be
   * retrieved by calling eigenvectors().
   *
   */
  ArpackGeneralizedSelfAdjointEigenSolver &compute(
      const MatrixType &A, Index nbrEigenvalues, std::string eigs_sigma = "LM",
   ...
   * For the generalized eigenproblem, the matrix returned is the solution \f$ A
   * V = D B V \f$
   *
...
ArpackGeneralizedSelfAdjointEigenSolver<MatrixType, MatrixSolver,
...
ArpackGeneralizedSelfAdjointEigenSolver<MatrixType, MatrixSolver,
  ...
  // Specifies the shift if iparam[6] = { 3, 4, 5 }, not used if iparam[6] = {
  // 1, 2 }
  //
  RealScalar sigma = 0.0;

  if (eigs_sigma.length() >= 2 && isalpha(eigs_sigma[0]) &&
      isalpha(eigs_sigma[1])) {
    eigs_sigma[0] = toupper(eigs_sigma[0]);
    eigs_sigma[1] = toupper(eigs_sigma[1]);

    // In the following special case we're going to invert the problem, since
    // solving for larger magnitude is much much faster i.e., if 'SM' is
    // specified, we're going to really use 'LM', the default
    //
  ...
  char bmat[2] = "I";
  if (eigs_sigma.substr(0, 2) == "SM" ||
      !(isalpha(eigs_sigma[0]) && isalpha(eigs_sigma[1])) ||
      (!isBempty && !BisSPD))
    bmat[0] = 'G';
  ...
  iparam[0] = 1;  // 1 means we let ARPACK perform the shifts, 0 means we'd have
                  // to do it
  iparam[2] = std::max(300, (int)std::ceil(2 * n / std::max(ncv, 1)));
  iparam[6] = mode;  // The mode, 1 is standard ev problem, 2 for generalized
                     // ev, 3 for shift-and-invert
  ...
  // initial residual vector is used, any other values means resid contains the
  // initial residual vector, possibly from a previous run
  //
  int info = 0;
  ...
  do {
    ...
    if (ido == -1 || ido == 1) {
      ...
      if (ido == 1 && mode != 2) {
        ...
        if (isBempty || mode == 1)
          Matrix<Scalar, Dynamic, 1>::Map(out2, n) =
      ...
      if (mode == 1) {
        if (isBempty) {
          ...
          Matrix<Scalar, Dynamic, 1>::Map(out, n) =
              A * Matrix<Scalar, Dynamic, 1>::Map(in, n);
        } else {
          // OP = L^{-1}AL^{-T}
          //
          internal::OP<MatrixSolver, MatrixType, Scalar, BisSPD>::applyOP(
              OP, A, n, in, out);
        }
      } else if (mode == 2) {
        if (ido == 1)
        ...
        //
        Matrix<Scalar, Dynamic, 1>::Map(out, n) =
            OP.solve(Matrix<Scalar, Dynamic, 1>::Map(in, n));
      } else if (mode == 3) {
        ...
        if (ido == 1 || isBempty)
          ...
          Matrix<Scalar, Dynamic, 1>::Map(out, n) =
              OP.solve(B * Matrix<Scalar, Dynamic, 1>::Map(in, n));
      }
    } else if (ido == 2) {
      Scalar *in = workd + ipntr[0] - 1;
      Scalar *out = workd + ipntr[1] - 1;
  ...
  if (info == 1)
    ...
    // if howmny == "S", specifies the eigenvalues to compute (not implemented
    // in ARPACK)
    //
    ...
    internal::arpack_wrapper<Scalar, RealScalar>::seupd(
        &rvec, howmny, select, m_eivalues.data(), v, &ldv, &sigma, bmat, &n,
        whch, &nev, &tol, resid, &ncv, v, &ldv, iparam, ipntr, workd, workl,
        &lworkl, &info);
    ...
    if (info == -14)
      ...
      if (rvec) {
        ...
        if (mode == 1 && !isBempty && BisSPD)
          internal::OP<MatrixSolver, MatrixType, Scalar, BisSPD>::project(
              OP, n, nev, m_eivec.data());
...
// Single precision
//
extern "C" void ssaupd_(int *ido, char *bmat, int *n, char *which, int *nev,
                        float *tol, float *resid, int *ncv, float *v, int *ldv,
                        int *iparam, int *ipntr, float *workd, float *workl,
                        int *lworkl, int *info);

extern "C" void sseupd_(int *rvec, char *All, int *select, float *d, float *z,
...
template <typename Scalar, typename RealScalar>
struct arpack_wrapper {
  static inline void saupd(int *ido, char *bmat, int *n, char *which, int *nev,
                           RealScalar *tol, Scalar *resid, int *ncv, Scalar *v,
                           int *ldv, int *iparam, int *ipntr, Scalar *workd,
                           Scalar *workl, int *lworkl, int *info) {
    EIGEN_STATIC_ASSERT(!NumTraits<Scalar>::IsComplex,
                        NUMERIC_TYPE_MUST_BE_REAL)
  }

  static inline void seupd(int *rvec, char *All, int *select, Scalar *d,
                           Scalar *z, int *ldz, RealScalar *sigma, char *bmat,
                           int *n, char *which, int *nev, RealScalar *tol,
                           Scalar *resid, int *ncv, Scalar *v, int *ldv,
                           int *iparam, int *ipntr, Scalar *workd,
...
struct arpack_wrapper<float, float> {
  ...
  static inline void seupd(int *rvec, char *All, int *select, float *d,
                           ...
                           int *ipntr, float *workd, float *workl, int *lworkl,
                           int *ierr) {
    sseupd_(rvec, All, select, d, z, ldz, sigma, bmat, n, which, nev, tol,
...
template <typename MatrixSolver, typename MatrixType, typename Scalar,
          bool BisSPD>
struct OP {
  static inline void applyOP(MatrixSolver &OP, const MatrixType &A, int n,
...
struct OP<MatrixSolver, MatrixType, Scalar, true> {
  static inline void applyOP(MatrixSolver &OP, const MatrixType &A, int n,
    ...
    //
    Matrix<Scalar, Dynamic, 1>::Map(out, n) =
        OP.matrixU().solve(Matrix<Scalar, Dynamic, 1>::Map(in, n));
    Matrix<Scalar, Dynamic, 1>::Map(out, n) =
    ...
    //
    Matrix<Scalar, Dynamic, 1>::Map(out, n) =
        A * Matrix<Scalar, Dynamic, 1>::Map(out, n);

    // Then solve L out = out
    //
    Matrix<Scalar, Dynamic, 1>::Map(out, n) =
        OP.permutationP() * Matrix<Scalar, Dynamic, 1>::Map(out, n);
    Matrix<Scalar, Dynamic, 1>::Map(out, n) =
        OP.matrixL().solve(Matrix<Scalar, Dynamic, 1>::Map(out, n));
  }
