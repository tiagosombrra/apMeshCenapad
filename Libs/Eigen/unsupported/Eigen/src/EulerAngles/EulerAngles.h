...
#ifndef EIGEN_EULERANGLESCLASS_H  // TODO: Fix previous "EIGEN_EULERANGLES_H"
                                  // definition?
 ...
 *
 * Euler rotation is a set of three rotation of three angles over three fixed
 * axes, defined by the EulerSystem given as a template parameter.
 *
 * Here is how intrinsic Euler angles works:
 *  - first, rotate the axes system over the alpha axis in angle alpha
 *  - then, rotate the axes system over the beta axis(which was rotated in the
 * first stage) in angle beta
 *  - then, rotate the axes system over the gamma axis(which was rotated in the
 * two stages above) in angle gamma
 *
 ...
 *
 * It has been proved(see Wikipedia link below) that every rotation can be
 * represented by Euler angles, but there is no singular representation (e.g.
 * unlike rotation matrices). Therefore, you can convert from Eigen rotation and
 * to them (including rotation matrices, which is not called "rotations" by
 * Eigen design).
 *
 * Euler angles usually used for:
 *  - convenient human representation of rotation, especially in interactive
 * GUI.
 ...
 * To overcome this, this class provide easy movement from the math friendly
 * representation to the human friendly representation, and vise-versa.
 *
 ...
 *
 * When converting some rotation to Euler angles, there are some ways you can
 * guarantee the Euler angles ranges.
 *
 * #### implicit ranges ####
 * When using implicit ranges, all angles are guarantee to be in the range [-PI,
 * +PI], unless you convert from some other Euler angles. In this case, the
 * range is __undefined__ (might be even less than -PI or greater than +2*PI).
 * \sa EulerAngles(const MatrixBase<Derived>&)
 * \sa EulerAngles(const RotationBase<Derived, 3>&)
 *
 * #### explicit ranges ####
 * When using explicit ranges, all angles are guarantee to be in the range you
 * choose. In the range Boolean parameter, you're been ask whether you prefer
 * the positive range or not:
 ...
 * If you need negative axes{-x,-y,-z}, it is recommended to create you own
 * typedef with a word that represent what you need.
 *
 * ### Example ###
 *
 ...
 * ### Additional reading ###
 *
 * If you're want to get more idea about how Euler system work in Eigen see
 * EulerSystem.
 *
 * More information about Euler angles:
 * https://en.wikipedia.org/wiki/Euler_angles
 *
 * \tparam _Scalar the scalar type, i.e., the type of the angles.
 *
 * \tparam _System the EulerSystem to use, which represents the axes of
 * rotation.
 */
...
class EulerAngles : public RotationBase<EulerAngles<_Scalar, _System>, 3> {
 public:
  ...
  typedef Matrix<Scalar, 3, 3>
      Matrix3; /*!< the equivalent rotation matrix type */
  typedef Matrix<Scalar, 3, 1>
      Vector3; /*!< the equivalent 3 dimension vector type */
  typedef Quaternion<Scalar>
      QuaternionType; /*!< the equivalent quaternion type */
  typedef AngleAxis<Scalar>
      AngleAxisType; /*!< the equivalent angle-axis type */
 ...
 public:
   ...
   *
   * If positive range is true, then the specified angle will be in the range
   * [0, +2*PI]. Otherwise, the specified angle will be in the range [-PI, +PI].
   *
   * \param m The 3x3 rotation matrix to convert
   * \param positiveRangeAlpha If true, alpha will be in [0, 2*PI]. Otherwise,
   * in [-PI, +PI]. \param positiveRangeBeta If true, beta will be in [0, 2*PI].
   * Otherwise, in [-PI, +PI]. \param positiveRangeGamma If true, gamma will be
   * in [0, 2*PI]. Otherwise, in [-PI, +PI].
   */
  template <typename Derived>
  EulerAngles(const MatrixBase<Derived>& m, bool positiveRangeAlpha,
              bool positiveRangeBeta, bool positiveRangeGamma) {
    System::CalcEulerAngles(*this, m, positiveRangeAlpha, positiveRangeBeta,
                            positiveRangeGamma);
  }

  /** Constructs and initialize Euler angles from a rotation \p rot.
   *
   * \note All angles will be in the range [-PI, PI], unless \p rot is an
   * EulerAngles. If rot is an EulerAngles, expected EulerAngles range is
   * __undefined__. (Use other functions here for enforcing range if this effect
   * is desired)
   */
  template <typename Derived>
   ...
   */
  template <typename Derived>
  EulerAngles(const RotationBase<Derived, 3>& rot, bool positiveRangeAlpha,
              bool positiveRangeBeta, bool positiveRangeGamma) {
    System::CalcEulerAngles(*this, rot.toRotationMatrix(), positiveRangeAlpha,
                            positiveRangeBeta, positiveRangeGamma);
  }

  /** \returns The angle values stored in a vector (alpha, beta, gamma). */
  const Vector3& angles() const { return m_angles; }
  /** \returns A read-write reference to the angle values stored in a vector
   * (alpha, beta, gamma). */
  ...
  /** \returns The Euler angles rotation inverse (which is as same as the
   * negative),
   *  (-alpha, -beta, -gamma).
   */
  ...
  /** \returns The Euler angles rotation negative (which is as same as the
   * inverse),
   *  (-alpha, -beta, -gamma).
   */
  ...
  /** Constructs and initialize Euler angles from a 3x3 rotation matrix \p m,
   *  with options to choose for each angle the requested range (__only in
   * compile time__).
   *
   * If positive range is true, then the specified angle will be in the range
   * [0, +2*PI]. Otherwise, the specified angle will be in the range [-PI, +PI].
   ...
   * \param m The 3x3 rotation matrix to convert
   * \tparam positiveRangeAlpha If true, alpha will be in [0, 2*PI]. Otherwise,
   * in [-PI, +PI]. \tparam positiveRangeBeta If true, beta will be in [0,
   * 2*PI]. Otherwise, in [-PI, +PI]. \tparam positiveRangeGamma If true, gamma
   * will be in [0, 2*PI]. Otherwise, in [-PI, +PI].
   */
  template <bool PositiveRangeAlpha, bool PositiveRangeBeta,
            bool PositiveRangeGamma, typename Derived>
  static EulerAngles FromRotation(const MatrixBase<Derived>& m) {
    EIGEN_STATIC_ASSERT_MATRIX_SPECIFIC_SIZE(Derived, 3, 3)

    EulerAngles e;
    System::template CalcEulerAngles<PositiveRangeAlpha, PositiveRangeBeta,
                                     PositiveRangeGamma, _Scalar>(e, m);
    return e;
  }
  ...
  static EulerAngles FromRotation(const RotationBase<Derived, 3>& rot) {
    return FromRotation<PositiveRangeAlpha, PositiveRangeBeta,
                        PositiveRangeGamma>(rot.toRotationMatrix());
  }
  ...
  http://www.euclideanspace.com/maths/geometry/rotations/conversions/quaternionToEuler/
    //  we can compute only the needed matrix cells and then convert to euler
  angles. (see ZYX example below)
    // Currently we compute all matrix cells from quaternion.

    // Special case only for ZYX
    //Scalar y2 = q.y() * q.y();
    //m_angles[0] = std::atan2(2*(q.w()*q.z() + q.x()*q.y()), (1 - 2*(y2 +
  q.z()*q.z())));
    //m_angles[1] = std::asin( 2*(q.w()*q.y() - q.z()*q.x()));
    //m_angles[2] = std::atan2(2*(q.w()*q.x() + q.y()*q.z()), (1 -
  2*(q.x()*q.x() + y2)));
  }*/

  /** Set \c *this from a rotation matrix(i.e. pure orthogonal matrix with
   * determinant of +1). */
  template <typename Derived>
  ...
  friend std::ostream& operator<<(
      std::ostream& s, const EulerAngles<Scalar, System>& eulerAngles) {
...
#define EIGEN_EULER_ANGLES_SINGLE_TYPEDEF(AXES, SCALAR_TYPE, SCALAR_POSTFIX) \
  /** \ingroup EulerAngles_Module */                                         \
  typedef EulerAngles<SCALAR_TYPE, EulerSystem##AXES>                        \
      EulerAngles##AXES##SCALAR_POSTFIX;
