 ...
 * \param perm gives the sorted sequence on output. Must be initialized with
 * 0..n-1 \param ncut Put  the ncut smallest elements at the end of the vector
...
void sortWithPermutation(VectorType& vec, IndexType& perm,
 ...
 * \endcode
 *
 * DGMRES can also be used in a matrix-free context, see the following \link
...
class DGMRES
 ...
 protected:
  // DGMRES algorithm
  template <typename Rhs, typename Dest>
  void dgmres(const MatrixType& mat, const Rhs& rhs, Dest& x,
              const Preconditioner& precond) const;
  // Perform one cycle of GMRES
  template <typename Dest>
  Index dgmresCycle(const MatrixType& mat, const Preconditioner& precond,
                    Dest& x, DenseVector& r0, RealScalar& beta,
  ...
  mutable DenseMatrix
      m_Hes;  // Initial hessenberg matrix wihout Givens rotations applied
  mutable Index m_restart;  // Maximum size of the Krylov subspace
  mutable DenseMatrix
      m_U;  // Vectors that form the basis of the invariant subspace
  mutable DenseMatrix m_MU;  // matrix operator applied to m_U (for next cycles)
  mutable DenseMatrix m_T;   /* T=U^T*M^{-1}*A*U */
  mutable PartialPivLU<DenseMatrix> m_luT;  // LU factorization of m_T
  mutable StorageIndex
      m_neig;         // Number of eigenvalues to extract at each restart
  ...
  // Adaptive strategy
  mutable RealScalar
      m_smv;  // Smaller multiple of the remaining number of steps allowed
...
template <typename _MatrixType, typename _Preconditioner>
template <typename Rhs, typename Dest>
void DGMRES<_MatrixType, _Preconditioner>::dgmres(
...
template <typename _MatrixType, typename _Preconditioner>
template <typename Dest>
Index DGMRES<_MatrixType, _Preconditioner>::dgmresCycle(
    const MatrixType& mat, const Preconditioner& precond, Dest& x,
  ...
  while (m_info == NoConvergence && it < m_restart && nbIts < m_iterations) {
    ...
    // Orthogonalize it with the previous basis in the basis using modified
    // Gram-Schmidt
    Scalar coef;
  ...
  nrs = m_H.topLeftCorner(it, it).template triangularView<Upper>().solve(
      g.head(it));
  ...
  if (nbIts < m_iterations && m_info == NoConvergence && m_neig > 0 &&
      (m_r + m_neig) < m_maxNeig)
    dgmresComputeDeflationData(mat, precond, it, m_neig);
  return 0;
}

template <typename _MatrixType, typename _Preconditioner>
void DGMRES<_MatrixType, _Preconditioner>::dgmresInitDeflation(
    Index& rows) const {
...
template <typename _MatrixType, typename _Preconditioner>
Index DGMRES<_MatrixType, _Preconditioner>::dgmresComputeDeflationData(
    const MatrixType& mat, const Preconditioner& precond, const Index& it,
    StorageIndex& neig) const {
  // First, find the Schur form of the Hessenberg matrix H
  typename internal::conditional<NumTraits<Scalar>::IsComplex,
                                 ComplexSchur<DenseMatrix>,
                                 RealSchur<DenseMatrix> >::type schurofH;
  bool computeU = true;
...
template <typename _MatrixType, typename _Preconditioner>
template <typename RhsType, typename DestType>
Index DGMRES<_MatrixType, _Preconditioner>::dgmresApplyDeflation(
    const RhsType& x, DestType& y) const {
