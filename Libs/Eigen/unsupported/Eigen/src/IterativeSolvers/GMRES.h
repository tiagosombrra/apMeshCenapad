 ...
 * Linear Systems. SIAM J.Sci.Stat.Comp. 7, 1986, pp. 856 - 869.
 *
 * Saad, Y.
...
bool gmres(const MatrixType& mat, const Rhs& rhs, Dest& x,
           const Preconditioner& precond, Index& iters, const Index& restart,
  ...
  for (Index k = 1; k <= restart; ++k) {
    ...
    for (Index i = k - 1; i >= 0; --i) {
      v.tail(m - i).applyHouseholderOnTheLeft(
    ...
    if (v.tail(m - k).norm() != 0.0) {
      if (k <= restart) {
        ...
        v.tail(m - k).applyHouseholderOnTheLeft(Hk_tail, tau.coeffRef(k),
                                                workspace.data());
      }
    }
 ...
 * generalized minimal residual method. The vectors x and b can be either dense
 * or sparse.
 *
 ...
 *
 * GMRES can also be used in a matrix-free context, see the following \link
 ...
 public:
  ...
  void _solve_with_guess_impl(const Rhs& b, Dest& x) const {
    ...
    for (Index j = 0; j < b.cols(); ++j) {
      ...
      if (!internal::gmres(matrix(), b.col(j), xj, Base::m_preconditioner,
                           m_iterations, m_restart, m_error))
        failed = true;
    }
