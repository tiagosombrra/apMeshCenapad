 ...
 * \brief iterative scaling algorithm to equilibrate rows and column norms in
 * matrices
 *
 * This class can be used as a preprocessing tool to accelerate the convergence
 * of iterative methods
 *
 * This feature is  useful to limit the pivoting amount during LU/ILU
 * factorization The  scaling strategy as presented here preserves the symmetry
 * of the problem NOTE It is assumed that the matrix does not have empty row or
 * column,
 *
 ...
 * // Compute the left and right scaling vectors. The matrix is equilibrated at
 * output scal.computeRef(A);
 ...
 * \endcode
 *
 * \tparam _MatrixType the type of the matrix. It should be a real square
 * sparsematrix
 *
 * References : D. Ruiz and B. Ucar, A Symmetry Preserving Algorithm for Matrix
 * Scaling, INRIA Research report RR-7552
 *
...
class IterScaling {
 ...
 public:
  ...
  /**
   * Compute the left and right diagonal matrices to scale the input matrix @p
   * mat
   *
   * FIXME This algorithm will be modified such that the diagonal elements are
   * permuted on the diagonal.
   *
  ...
  void compute(const MatrixType& mat) {
    ...
    int its = 0;
    do {  // Iterate until the infinite norm of each row and column is
          // approximately 1
