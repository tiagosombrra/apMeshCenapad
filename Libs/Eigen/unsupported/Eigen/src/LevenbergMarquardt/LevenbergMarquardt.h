...
//
// The algorithm of this class initially comes from MINPACK whose original
// authors are: Copyright Jorge More - Argonne National Laboratory Copyright
// Burt Garbow - Argonne National Laboratory Copyright Ken Hillstrom - Argonne
// National Laboratory
//
...
class LevenbergMarquardt : internal::no_assignment_operator {
 ...
 public:
  ...
  static LevenbergMarquardtSpace::Status lmdif1(
      FunctorType &functor, FVectorType &x, Index *nfev,
      const Scalar tol = std::sqrt(NumTraits<Scalar>::epsilon()));
  ...
  /** \returns a reference to the matrix where the current Jacobian matrix is
   * stored
   */
  JacobianType &jacobian() { return m_fjac; }

  /** \returns a reference to the triangular matrix R from the QR of the
   * jacobian matrix. \sa jacobian()
   */
 ...
 private:
  JacobianType m_fjac;
  JacobianType m_rfactor;  // The triangular matrix R from the QR of the
                           // jacobian matrix m_fjac
...
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType>::minimize(
    FVectorType &x) {
...
template <typename FunctorType>
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType>::minimizeInit(
    FVectorType &x) {
  n = x.size();
  m = m_functor.values();
  ...
  m_fjac.resize(m, n);
  //     m_fjac.reserve(VectorXi::Constant(n,5)); // FIXME Find a better
  //     alternative
  if (!m_useExternalScaling) m_diag.resize(n);
  eigen_assert((!m_useExternalScaling || m_diag.size() == n) &&
               "When m_useExternalScaling is set, the caller must provide a "
               "valid 'm_diag'");
  m_qtf.resize(n);
  ...
  if (n <= 0 || m < n || m_ftol < 0. || m_xtol < 0. || m_gtol < 0. ||
      m_maxfev <= 0 || m_factor <= 0.) {
    m_info = InvalidInput;
...
template <typename FunctorType>
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType>::lmder1(
    FVectorType &x, const Scalar tol) {
  n = x.size();
  m = m_functor.values();
...
template <typename FunctorType>
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType>::lmdif1(
    FunctorType &functor, FVectorType &x, Index *nfev, const Scalar tol) {
  Index n = x.size();
  ...
  LevenbergMarquardtSpace::Status info =
      LevenbergMarquardtSpace::Status(lm.minimize(x));
