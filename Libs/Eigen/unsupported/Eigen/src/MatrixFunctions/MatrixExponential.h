...
void matrix_exp_pade5(const MatA& A, MatU& U, MatV& V) {
  typedef typename MatA::PlainObject MatrixType;
  typedef
      typename NumTraits<typename traits<MatrixType>::Scalar>::Real RealScalar;
 ...
 *  approximant of \f$ \exp(A) \f$ around \f$ A = 0 \f$.
 *
 *  This function activates only if your long double is double-double or
 * quadruple.
 */
#if LDBL_MANT_DIG > 64
template <typename MatA, typename MatU, typename MatV>
void matrix_exp_pade17(const MatA& A, MatU& U, MatV& V) {
  ...
  V = b[17] * A8 + b[15] * A6 + b[13] * A4 +
      b[11] * A2;  // used for temporary storage
  MatrixType tmp = A8 * V;
...
template <typename MatrixType,
          typename RealScalar =
              typename NumTraits<typename traits<MatrixType>::Scalar>::Real>
...
struct matrix_exp_computeUV<MatrixType, float> {
  ...
  static void run(const ArgType& arg, MatrixType& U, MatrixType& V,
                  int& squarings) {
    using std::frexp;
    using std::pow;
...
void matrix_exp_compute(const ArgType& arg, ResultType& result,
                        true_type)  // natively supported scalar type
{
  typedef typename ArgType::PlainObject MatrixType;
  MatrixType U, V;
  int squarings;
  matrix_exp_computeUV<MatrixType>::run(
      arg, U, V, squarings);  // Pade approximant is (U+V) / (-U+V)
 ...
 *
 * This class holds the argument to the matrix exponential until it is assigned
 * or evaluated for some other reason (so the argument should not be changed in
 * the meantime). It is the return type of MatrixBase::exp() and most of the
...
struct MatrixExponentialReturnValue
 ...
 public:
  /** \brief Constructor.
   *
   * \param src %Matrix (expression) forming the argument of the matrix
   * exponential.
   */
  ...
  inline void evalTo(ResultType& result) const {
    ...
    internal::matrix_exp_compute(
        tmp, result,
        internal::is_exp_known_type<typename Derived::RealScalar>());
  }
