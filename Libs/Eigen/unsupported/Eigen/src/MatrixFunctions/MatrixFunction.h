...
/** \brief Maximum distance allowed between eigenvalues to be considered
 * "close". */
 ...
 *
 * Here, an atomic matrix is a triangular matrix whose diagonal entries are
 * close to each other.
 */
    ...
    if (Fincr_norm < NumTraits<Scalar>::epsilon() * F_norm) {
      ...
      for (Index r = 0; r < rows; r++) {
        ...
        for (Index i = 0; i < rows; i++)
          mx = (std::max)(mx, std::abs(m_f(Ashifted(i, i) + avgEival,
                                           static_cast<int>(s + r))));
 ...
 * \returns Iterator to cluster containing \p key, or \c clusters.end() if no
 * cluster in \p m_clusters contains \p key.
 */
template <typename Index, typename ListOfClusters>
typename ListOfClusters::iterator matrix_function_find_cluster(
    Index key, ListOfClusters& clusters) {
  typename std::list<Index>::iterator j;
  for (typename ListOfClusters::iterator i = clusters.begin();
       i != clusters.end(); ++i) {
 ...
 * # Any eigenvalue in a certain cluster is at most matrix_function_separation()
 * away from another eigenvalue in the same cluster. # The distance between two
 * eigenvalues in different clusters is more than matrix_function_separation().
 * The implementation follows Algorithm 4.1 in the paper of Davies and Higham.
 */
template <typename EivalsType, typename Cluster>
void matrix_function_partition_eigenvalues(const EivalsType& eivals,
                                           std::list<Cluster>& clusters) {
  typedef typename EivalsType::Index Index;
  typedef typename EivalsType::RealScalar RealScalar;
  for (Index i = 0; i < eivals.rows(); ++i) {
    // Find cluster containing i-th ei'val, adding a new cluster if necessary
    typename std::list<Cluster>::iterator qi =
        matrix_function_find_cluster(i, clusters);
    if (qi == clusters.end()) {
      Cluster l;
      l.push_back(i);
    ...
    for (Index j = i + 1; j < eivals.rows(); ++j) {
      if (abs(eivals(j) - eivals(i)) <=
              RealScalar(matrix_function_separation) &&
          std::find(qi->begin(), qi->end(), j) == qi->end()) {
        typename std::list<Cluster>::iterator qj =
            matrix_function_find_cluster(j, clusters);
        if (qj == clusters.end()) {
          qi->push_back(j);
        } else {
...
void matrix_function_compute_cluster_size(
...
void matrix_function_compute_block_start(const VectorType& clusterSize,
...
void matrix_function_compute_map(const EivalsType& eivals,
...
void matrix_function_permute_schur(VectorType& permutation, MatrixType& U,
                                   MatrixType& T) {
  typedef typename VectorType::Index Index;
 ...
 *
 * This routine computes the matrix function applied to the block diagonal part
 * of \p T (which should be upper triangular), with the blocking given by \p
 * blockStart and \p clusterSize. The matrix function of each diagonal block is
 * computed by \p atomic. The off-diagonal parts of \p fT are set to zero.
 */
...
void matrix_function_compute_block_atomic(const MatrixType& T,
  ...
  for (typename VectorType::Index i = 0; i < clusterSize.rows(); ++i) {
    fT.block(blockStart(i), blockStart(i), clusterSize(i), clusterSize(i)) =
        atomic.compute(T.block(blockStart(i), blockStart(i), clusterSize(i),
                               clusterSize(i)));
  }
}
 ...
 *
 * If A is m-by-m and B is n-by-n, then both C and X are m-by-n.  The (i,j)-th
 * component of the Sylvester equation is \f[ \sum_{k=i}^m A_{ik} X_{kj} +
 * \sum_{k=1}^j X_{ik} B_{kj} = C_{ij}. \f] This can be re-arranged to yield:
 * \f[
 *     X_{ij} = \frac{1}{A_{ii} + B_{jj}} \Bigl( C_{ij}
 *     - \sum_{k=i+1}^m A_{ik} X_{kj} - \sum_{k=1}^{j-1} X_{ik} B_{kj} \Bigr).
 * \f]
 * It is assumed that A and B are such that the numerator is never zero
 * (otherwise the Sylvester equation does not have a unique solution). In that
 * case, these equations can be evaluated in the order \f$ i=m,\ldots,1 \f$ and
 * \f$ j=1,\ldots,n \f$.
 */
template <typename MatrixType>
MatrixType matrix_function_solve_triangular_sylvester(const MatrixType& A,
                                                      const MatrixType& B,
                                                      const MatrixType& C) {
 ...
 *
 * This routine completes the computation of \p fT, denoting a matrix function
 * applied to the triangular matrix \p T. It assumes that the block diagonal
 * part of \p fT has already been computed. The part below the diagonal is zero,
 * because \p T is upper triangular.
 */
...
void matrix_function_compute_above_diagonal(const MatrixType& T,
  ...
  typedef Matrix<Scalar, Dynamic, Dynamic, Options, RowsAtCompileTime,
                 ColsAtCompileTime>
      DynMatrixType;
  ...
  for (Index k = 1; k < clusterSize.rows(); k++) {
    for (Index i = 0; i < clusterSize.rows() - k; i++) {
      // compute (i, i+k) block
      DynMatrixType A =
          T.block(blockStart(i), blockStart(i), clusterSize(i), clusterSize(i));
      DynMatrixType B = -T.block(blockStart(i + k), blockStart(i + k),
                                 clusterSize(i + k), clusterSize(i + k));
      DynMatrixType C = fT.block(blockStart(i), blockStart(i), clusterSize(i),
                                 clusterSize(i)) *
                        T.block(blockStart(i), blockStart(i + k),
                                clusterSize(i), clusterSize(i + k));
      C -= T.block(blockStart(i), blockStart(i + k), clusterSize(i),
                   clusterSize(i + k)) *
           fT.block(blockStart(i + k), blockStart(i + k), clusterSize(i + k),
                    clusterSize(i + k));
      for (Index m = i + 1; m < i + k; m++) {
        C += fT.block(blockStart(i), blockStart(m), clusterSize(i),
                      clusterSize(m)) *
             T.block(blockStart(m), blockStart(i + k), clusterSize(m),
                     clusterSize(i + k));
        C -= T.block(blockStart(i), blockStart(m), clusterSize(i),
                     clusterSize(m)) *
             fT.block(blockStart(m), blockStart(i + k), clusterSize(m),
                      clusterSize(i + k));
      }
      fT.block(blockStart(i), blockStart(i + k), clusterSize(i),
               clusterSize(i + k)) =
 ...
 *                      expected to be an instantiation of the Matrix class
 * template. \tparam  AtomicType  type for computing matrix function of atomic
 * blocks. \tparam  IsComplex   used internally to select correct
 * specialization.
 *
 * This class implements the Schur-Parlett algorithm for computing matrix
 * functions. The spectrum of the matrix is divided in clustered of eigenvalues
 * that lies close together. This class delegates the computation of the matrix
 * function on every block corresponding to these clusters to an object of type
 * \p AtomicType and uses these results to compute the matrix function of the
 * whole matrix. The class \p AtomicType should have a \p compute() member
 * function for computing the matrix function of a block.
 *
 ...
 * function of that matrix, and then converts the result back to a real matrix.
 */
template <typename MatrixType>
...
struct matrix_function_compute<MatrixType, 1> {
  ...
  static void run(const MatA& A, AtomicType& atomic, ResultType& result) {
    ...
    // blockStart[i] is row index at which block corresponding to i-th cluster
    // starts
    Matrix<Index, Dynamic, 1> blockStart;
    matrix_function_compute_block_start(clusterSize, blockStart);

    // compute map so that eivalToCluster[i] = j means that i-th ei'val is in
    // j-th cluster
    ...
    matrix_function_compute_permutation(blockStart, eivalToCluster,
                                        permutation);
    ...
    matrix_function_compute_block_atomic(T, atomic, blockStart, clusterSize,
                                         fT);
 ...
 *
 * This class holds the argument to the matrix function until it is assigned or
 * evaluated for some other reason (so the argument should not be changed in the
 * meantime). It is the return type of matrixBase::matrixFunction() and related
 * functions and most of the time this is the only way it is used.
 */
...
class MatrixFunctionReturnValue
 ...
 public:
  /** \brief Constructor.
   *
   * \param[in] A  %Matrix (expression) forming the argument of the matrix
   * function. \param[in] f  Stem function for matrix function under
   * consideration.
   */
  ...
  /** \brief Compute the matrix function.
   *
   * \param[out] result \p f applied to \p A, where \p f and \p A are as in the
   * constructor.
   */
  ...
  inline void evalTo(ResultType& result) const {
    ...
    typedef
        typename internal::remove_all<NestedEvalType>::type NestedEvalTypeClean;
    typedef internal::traits<NestedEvalTypeClean> Traits;
    static const int RowsAtCompileTime = Traits::RowsAtCompileTime;
    static const int ColsAtCompileTime = Traits::ColsAtCompileTime;
    typedef std::complex<typename NumTraits<Scalar>::Real> ComplexScalar;
    typedef Matrix<ComplexScalar, Dynamic, Dynamic, 0, RowsAtCompileTime,
                   ColsAtCompileTime>
        DynMatrixType;
    ...
    internal::matrix_function_compute<
        typename NestedEvalTypeClean::PlainObject>::run(m_A, atomic, result);
  }
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::matrixFunction(
    typename internal::stem_function<
        typename internal::traits<Derived>::Scalar>::type f) const {
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::sin() const {
  ...
  return MatrixFunctionReturnValue<Derived>(
      derived(), internal::stem_function_sin<ComplexScalar>);
}
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::cos() const {
  ...
  typedef typename internal::stem_function<Scalar>::ComplexScalar ComplexScalar;
  return MatrixFunctionReturnValue<Derived>(
      derived(), internal::stem_function_cos<ComplexScalar>);
}
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::sinh() const {
  ...
  typedef typename internal::stem_function<Scalar>::ComplexScalar ComplexScalar;
  return MatrixFunctionReturnValue<Derived>(
      derived(), internal::stem_function_sinh<ComplexScalar>);
}
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::cosh() const {
  ...
  typedef typename internal::stem_function<Scalar>::ComplexScalar ComplexScalar;
  return MatrixFunctionReturnValue<Derived>(
      derived(), internal::stem_function_cosh<ComplexScalar>);
}
