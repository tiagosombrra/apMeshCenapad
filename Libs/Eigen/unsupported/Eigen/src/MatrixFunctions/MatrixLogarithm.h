...
struct matrix_log_max_pade_degree {
  ...
  static const int value =
      std::numeric_limits<RealScalar>::digits <= 24 ? 5 :  // single precision
...
void matrix_log_compute_2x2(const MatrixType& A, MatrixType& result) {
  ...
  } else if ((abs(A(0, 0)) < RealScalar(0.5) * abs(A(1, 1))) ||
             (abs(A(0, 0)) > 2 * abs(A(1, 1)))) {
    result(0, 1) = A(0, 1) * (logA11 - logA00) / y;
  } else {
    // computation in previous branch is inaccurate if A(1,1) \approx A(0,0)
    int unwindingNumber =
        static_cast<int>(ceil((imag(logA11 - logA00) - RealScalar(EIGEN_PI)) /
                              RealScalar(2 * EIGEN_PI)));
    result(0, 1) = A(0, 1) *
                   (numext::log1p(y / A(0, 0)) +
                    Scalar(0, 2 * EIGEN_PI * unwindingNumber)) /
                   y;
  }
}

/* \brief Get suitable degree for Pade approximation. (specialized for
 * RealScalar = float) */
inline int matrix_log_get_pade_degree(float normTminusI) {
  const float maxNormForPade[] = {2.5111573934555054e-1 /* degree = 3 */,
                                  4.0535837411880493e-1, 5.3149729967117310e-1};
...
inline int matrix_log_get_pade_degree(double normTminusI) {
  const double maxNormForPade[] = {1.6206284795015624e-2 /* degree = 3 */,
                                   5.3873532631381171e-2, 1.1352802267628681e-1,
...
inline int matrix_log_get_pade_degree(long double normTminusI) {
  ...
  const long double maxNormForPade[] = {
      8.58970550342939562202529664318890e-5L /* degree = 3 */,
  ...
  const long double maxNormForPade[] = {
      4.7419931187193005048501568167858103e-5L /* degree = 3 */,
...
template <typename MatrixType>
void matrix_log_compute_pade(MatrixType& result, const MatrixType& T,
                             int degree) {
  ...
  for (int k = 0; k < degree; ++k) {
    ...
    result +=
        weight * (MatrixType::Identity(T.rows(), T.rows()) + node * TminusI)
...
void matrix_log_compute_big(const MatrixType& A, MatrixType& result) {
  ...
  while (true) {
    RealScalar normTminusI = (T - MatrixType::Identity(T.rows(), T.rows()))
                                 .cwiseAbs()
                                 .colwise()
                                 .sum()
                                 .maxCoeff();
...
class MatrixLogarithmAtomic {
 public:
   ...
   * \param[in]  A  argument of matrix logarithm, should be upper triangular and
   * atomic \returns  The logarithm of \p A.
   */
...
class MatrixLogarithmReturnValue
 ...
 public:
   ...
   *
   * \param[out]  result  Logarithm of \c A, where \c A is as specified in the
   * constructor.
   */
  ...
  inline void evalTo(ResultType& result) const {
    ...
    internal::matrix_function_compute<
        typename DerivedEvalTypeClean::PlainObject>::run(m_A, atomic, result);
  }
