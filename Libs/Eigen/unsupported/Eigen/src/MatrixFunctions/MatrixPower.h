...
class MatrixPowerParenthesesReturnValue
    : public ReturnByValue<MatrixPowerParenthesesReturnValue<MatrixType> > {
 public:
...
class MatrixPowerAtomic : internal::noncopyable {
 private:
  ...
  static ComplexScalar computeSuperDiag(const ComplexScalar&,
...
template <typename MatrixType>
MatrixPowerAtomic<MatrixType>::MatrixPowerAtomic(const MatrixType& T,
                                                 RealScalar p)
    : m_A(T), m_p(p) {
...
template <typename MatrixType>
void MatrixPowerAtomic<MatrixType>::computePade(int degree,
...
template <typename MatrixType>
void MatrixPowerAtomic<MatrixType>::compute2x2(ResultType& res,
                                               RealScalar p) const {
  using std::abs;
  using std::pow;
  ...
  for (Index i = 1; i < m_A.cols(); ++i) {
    ...
    if (m_A.coeff(i - 1, i - 1) == m_A.coeff(i, i))
      ...
      res.coeffRef(i - 1, i) = (res.coeff(i, i) - res.coeff(i - 1, i - 1)) /
                               (m_A.coeff(i, i) - m_A.coeff(i - 1, i - 1));
    else
      res.coeffRef(i - 1, i) =
          computeSuperDiag(m_A.coeff(i, i), m_A.coeff(i - 1, i - 1), p);
...
inline int MatrixPowerAtomic<MatrixType>::getPadeDegree(float normIminusT) {
  const float maxNormForPade[] = {2.8064004e-1f /* degree = 3 */,
                                  4.3386528e-1f};
  int degree = 3;
  for (; degree <= 4; ++degree)
...
inline int MatrixPowerAtomic<MatrixType>::getPadeDegree(double normIminusT) {
  const double maxNormForPade[] = {1.884160592658218e-2 /* degree = 3 */,
                                   6.038881904059573e-2, 1.239917516308172e-1,
...
inline int MatrixPowerAtomic<MatrixType>::getPadeDegree(
  ...
  const double maxNormForPade[] = {
      1.0007161601787493236741409687186e-4L /* degree = 3 */,
  ...
  const int maxPadeDegree = 10;
  const double maxNormForPade[] = {
      5.524506147036624377378713555116378e-5L /* degree = 3 */,
...
MatrixPowerAtomic<MatrixType>::computeSuperDiag(const ComplexScalar& curr,
  ...
  int unwindingNumber =
      ceil((numext::imag(logCurr - logPrev) - RealScalar(EIGEN_PI)) /
           RealScalar(2 * EIGEN_PI));
  ComplexScalar w = numext::log1p((curr - prev) / prev) / RealScalar(2) +
                    ComplexScalar(0, EIGEN_PI * unwindingNumber);
  return RealScalar(2) * exp(RealScalar(0.5) * p * (logCurr + logPrev)) *
         sinh(p * w) / (curr - prev);
}

template <typename MatrixType>
inline typename MatrixPowerAtomic<MatrixType>::RealScalar
MatrixPowerAtomic<MatrixType>::computeSuperDiag(RealScalar curr,
                                                RealScalar prev, RealScalar p) {
  using std::exp;
  using std::log;
  using std::sinh;
...
void MatrixPower<MatrixType>::initialize() {
  ...
  m_conditionNumber = m_T.diagonal().array().abs().maxCoeff() /
                      m_T.diagonal().array().abs().minCoeff();
  ...
  if (m_nulls) {
    eigen_assert(m_T.bottomRightCorner(m_nulls, m_nulls).isZero() &&
                 "Base of matrix power should be invertible or with a "
                 "semisimple zero eigenvalue.");
...
void MatrixPower<MatrixType>::computeFracPower(ResultType& res, RealScalar p) {
  ...
  MatrixPowerAtomic<ComplexMatrix>(m_T.topLeftCorner(m_rank, m_rank), p)
      .compute(blockTp);
  if (m_nulls) {
    m_fT.topRightCorner(m_rank, m_nulls) =
        m_T.topLeftCorner(m_rank, m_rank)
            .template triangularView<Upper>()
...
inline void MatrixPower<MatrixType>::revertSchur(
    ...
    const ComplexMatrix& T, const ComplexMatrix& U) {
  res.noalias() =
      (U * (T.template triangularView<Upper>() * U.adjoint())).real();
}
...
class MatrixPowerReturnValue
    : public ReturnByValue<MatrixPowerReturnValue<Derived> > {
 public:
...
template <typename Derived>
const MatrixPowerReturnValue<Derived> MatrixBase<Derived>::pow(
    const RealScalar& p) const {
  return MatrixPowerReturnValue<Derived>(derived(), p);
}

template <typename Derived>
const MatrixComplexPowerReturnValue<Derived> MatrixBase<Derived>::pow(
