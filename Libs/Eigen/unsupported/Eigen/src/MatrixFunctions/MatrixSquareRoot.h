...
void matrix_sqrt_quasi_triangular_2x2_diagonal_block(
    const MatrixType& T, typename MatrixType::Index i, ResultType& sqrtT) {
  // TODO: This case (2-by-2 blocks with complex conjugate eigenvalues) is
...
template <typename MatrixType, typename ResultType>
void matrix_sqrt_quasi_triangular_1x1_off_diagonal_block(
    const MatrixType& T, typename MatrixType::Index i,
    typename MatrixType::Index j, ResultType& sqrtT) {
  typedef typename traits<MatrixType>::Scalar Scalar;
  Scalar tmp = (sqrtT.row(i).segment(i + 1, j - i - 1) *
                sqrtT.col(j).segment(i + 1, j - i - 1))
                   .value();
  sqrtT.coeffRef(i, j) =
      (T.coeff(i, j) - tmp) / (sqrtT.coeff(i, i) + sqrtT.coeff(j, j));
}

// similar to compute1x1offDiagonalBlock()
template <typename MatrixType, typename ResultType>
void matrix_sqrt_quasi_triangular_1x2_off_diagonal_block(
    const MatrixType& T, typename MatrixType::Index i,
    typename MatrixType::Index j, ResultType& sqrtT) {
  typedef typename traits<MatrixType>::Scalar Scalar;
  Matrix<Scalar, 1, 2> rhs = T.template block<1, 2>(i, j);
  if (j - i > 1)
    rhs -= sqrtT.block(i, i + 1, 1, j - i - 1) *
           sqrtT.block(i + 1, j, j - i - 1, 2);
  Matrix<Scalar, 2, 2> A = sqrtT.coeff(i, i) * Matrix<Scalar, 2, 2>::Identity();
  A += sqrtT.template block<2, 2>(j, j).transpose();
  sqrtT.template block<1, 2>(i, j).transpose() =
      A.fullPivLu().solve(rhs.transpose());
}

// similar to compute1x1offDiagonalBlock()
template <typename MatrixType, typename ResultType>
void matrix_sqrt_quasi_triangular_2x1_off_diagonal_block(
    const MatrixType& T, typename MatrixType::Index i,
    typename MatrixType::Index j, ResultType& sqrtT) {
  typedef typename traits<MatrixType>::Scalar Scalar;
  Matrix<Scalar, 2, 1> rhs = T.template block<2, 1>(i, j);
  if (j - i > 2)
    rhs -= sqrtT.block(i, i + 2, 2, j - i - 2) *
           sqrtT.block(i + 2, j, j - i - 2, 1);
...
void matrix_sqrt_quasi_triangular_solve_auxiliary_equation(
    MatrixType& X, const MatrixType& A, const MatrixType& B,
    const MatrixType& C) {
  typedef typename traits<MatrixType>::Scalar Scalar;
...
// similar to compute1x1offDiagonalBlock()
template <typename MatrixType, typename ResultType>
void matrix_sqrt_quasi_triangular_2x2_off_diagonal_block(
    const MatrixType& T, typename MatrixType::Index i,
    typename MatrixType::Index j, ResultType& sqrtT) {
  typedef typename traits<MatrixType>::Scalar Scalar;
  ...
  if (j - i > 2)
    C -= sqrtT.block(i, i + 2, 2, j - i - 2) *
         sqrtT.block(i + 2, j, j - i - 2, 2);
  Matrix<Scalar, 2, 2> X;
...
// post: the diagonal blocks of sqrtT are the square roots of the diagonal
// blocks of T
template <typename MatrixType, typename ResultType>
void matrix_sqrt_quasi_triangular_diagonal(const MatrixType& T,
                                           ResultType& sqrtT) {
  using std::sqrt;
...
// pre:  T is quasi-upper-triangular and diagonal blocks of sqrtT are square
// root of diagonal blocks of T. post: sqrtT is the square root of T.
template <typename MatrixType, typename ResultType>
void matrix_sqrt_quasi_triangular_off_diagonal(const MatrixType& T,
                                               ResultType& sqrtT) {
  const Index size = T.rows();
  for (Index j = 1; j < size; j++) {
    ...
    for (Index i = j - 1; i >= 0; i--) {
      if (i > 0 &&
          T.coeff(i, i - 1) != 0)  // if T(i-1:i, i-1:i) is a 2-by-2 block
        continue;
 ...
 * template. \tparam  ResultType  type of \p result, where result is to be
 * stored. \param[in]  arg      argument of matrix square root. \param[out]
 * result   matrix square root of upper Hessenberg part of \p arg.
 *
 * This function computes the square root of the upper quasi-triangular matrix
 * stored in the upper Hessenberg part of \p arg.  Only the upper Hessenberg
 * part of \p result is updated, the rest is not touched.  See
 * MatrixBase::sqrt() for details on how this computation is implemented.
 *
 ...
 * template. \tparam  ResultType  type of \p result, where result is to be
 * stored. \param[in]  arg      argument of matrix square root. \param[out]
 * result   matrix square root of upper triangular part of \p arg.
 *
 * Only the upper triangular part (including the diagonal) of \p result is
 * updated, the rest is not touched.  See MatrixBase::sqrt() for details on how
...
void matrix_sqrt_triangular(const MatrixType& arg, ResultType& result) {
  ...
  // This uses that the square root of triangular matrices can be computed
  // directly.
  result.resize(arg.rows(), arg.cols());
...
class MatrixSquareRootReturnValue
 ...
 public:
  ...
  inline void evalTo(ResultType& result) const {
    ...
    typedef typename internal::remove_all<DerivedEvalType>::type
