...
class companion {
 ...
 public:
  DenseCompanionMatrixType denseMatrix() const {
    ...
    companMat << (LeftBlock(deg, deg_1) << LeftBlockFirstRow::Zero(1, deg_1),
                  BottomLeftBlock::Identity(deg - 1, deg - 1) *
 ...
 protected:
   ...
   * */
  bool balanced(RealScalar colNorm, RealScalar rowNorm, bool& isBalanced,
   ...
   * the column and the row in order to balance them.
   * */
  bool balancedR(RealScalar colNorm, RealScalar rowNorm, bool& isBalanced,
                 RealScalar& colB, RealScalar& rowB);
...
template <typename _Scalar, int _Deg>
inline bool companion<_Scalar, _Deg>::balanced(RealScalar colNorm,
                                               RealScalar rowNorm,
                                               bool& isBalanced,
                                               RealScalar& colB,
                                               RealScalar& rowB) {
...
template <typename _Scalar, int _Deg>
inline bool companion<_Scalar, _Deg>::balancedR(RealScalar colNorm,
                                                RealScalar rowNorm,
                                                bool& isBalanced,
                                                RealScalar& colB,
                                                RealScalar& rowB) {
...
void companion<_Scalar, _Deg>::balance() {
  using std::abs;
  EIGEN_STATIC_ASSERT(Deg == Dynamic || 1 < Deg,
