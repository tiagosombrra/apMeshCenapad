 ...
 * the only possibility is RowMajor. The default is 0 which means column-major.
 *
 *
 */
namespace internal {
...
class SkylineMatrix
    ...
    if (IsRowMajor) {
      if (inner > outer)  // upper matrix
      {
        ...
        if (outer >= minOuterIndex)
          return this->m_data.upper(m_colStartIndex[inner] + outer -
                                    (inner - m_data.upperProfile(inner)));
        else
          return Scalar(0);
      }
      if (inner < outer)  // lower matrix
      {
        ...
        if (inner >= minInnerIndex)
          return this->m_data.lower(m_rowStartIndex[outer] + inner -
                                    (outer - m_data.lowerProfile(outer)));
        else
          return Scalar(0);
      }
 ...
 public:
  ...
  inline void reserve(Index reserveSize, Index reserveUpperSize,
    ...
    if (IsRowMajor) {
      if (outer < inner)  // upper matrix
      {
        ...
        if (outer < minOuterIndex)  // The value does not yet exist
        {
          ...
          // zeros new data
          memset(this->_upperPtr() + start, 0,
                 (bandIncrement - 1) * sizeof(Scalar));

          return m_data.upper(m_colStartIndex[inner]);
        } else {
          return m_data.upper(m_colStartIndex[inner] + outer -
                              (inner - m_data.upperProfile(inner)));
        }
      }
      ...
      if (outer > inner)  // lower matrix
      {
        ...
        if (inner < minInnerIndex)  // The value does not yet exist
        {
          ...
          // zeros new data
          memset(this->_lowerPtr() + start, 0,
                 (bandIncrement - 1) * sizeof(Scalar));
          return m_data.lower(m_rowStartIndex[outer]);
        } else {
          return m_data.lower(m_rowStartIndex[outer] + inner -
                              (outer - m_data.lowerProfile(outer)));
        }
      }
    } else {
      if (outer > inner)  // upper matrix
      {
        ...
        if (outer > maxOuterIndex)  // The value does not yet exist
        {
          ...
          for (Index innerIdx = inner + 1; innerIdx < outerSize() + 1;
               innerIdx++) {
            m_rowStartIndex[innerIdx] += bandIncrement;
          }
          memset(
              this->_upperPtr() + m_rowStartIndex[inner] + previousProfile + 1,
              0, (bandIncrement - 1) * sizeof(Scalar));
          return m_data.upper(m_rowStartIndex[inner] +
      ...
      if (outer < inner)  // lower matrix
      {
        ...
        if (inner > maxInnerIndex)  // The value does not yet exist
        {
          ...
          for (Index innerIdx = outer + 1; innerIdx < outerSize() + 1;
               innerIdx++) {
            m_colStartIndex[innerIdx] += bandIncrement;
          }
          memset(
              this->_lowerPtr() + m_colStartIndex[outer] + previousProfile + 1,
              0, (bandIncrement - 1) * sizeof(Scalar));
          return m_data.lower(m_colStartIndex[outer] +
  ...
  inline void finalize() {
    if (IsRowMajor) {
      if (rows() > cols())
        m_data.resize(cols(), cols(), rows(), m_colStartIndex[cols()] + 1,
                      m_rowStartIndex[rows()] + 1);
      else
        m_data.resize(rows(), cols(), rows(), m_colStartIndex[cols()] + 1,
                      m_rowStartIndex[rows()] + 1);

      //            eigen_assert(rows() == cols() && "memory reorganisatrion
      //            only works with suare matrix");
      //
      //            Scalar* newArray = new Scalar[m_colStartIndex[cols()] + 1 +
      //            m_rowStartIndex[rows()] + 1]; Index dataIdx = 0; for (Index
      //            row = 0; row < rows(); row++) {
      //
      //                const Index nbLowerElts = m_rowStartIndex[row + 1] -
      //                m_rowStartIndex[row];
      //                //                std::cout << "nbLowerElts" <<
      //                nbLowerElts << std::endl; memcpy(newArray + dataIdx,
      //                m_data.m_lower + m_rowStartIndex[row], nbLowerElts *
      //                sizeof (Scalar)); m_rowStartIndex[row] = dataIdx;
      //                dataIdx += nbLowerElts;
      //
      //                const Index nbUpperElts = m_colStartIndex[row + 1] -
      //                m_colStartIndex[row]; memcpy(newArray + dataIdx,
      //                m_data.m_upper + m_colStartIndex[row], nbUpperElts *
      //                sizeof (Scalar)); m_colStartIndex[row] = dataIdx;
      //                dataIdx += nbUpperElts;
      //
      //
      //            }
      //            //todo : don't access m_data profile directly : add an
      //            accessor from SkylineMatrix m_rowStartIndex[rows()] =
      //            m_rowStartIndex[rows()-1] + m_data.lowerProfile(rows()-1);
      //            m_colStartIndex[cols()] = m_colStartIndex[cols()-1] +
      //            m_data.upperProfile(cols()-1);
      //
      ...
      if (rows() > cols())
        m_data.resize(cols(), rows(), cols(), m_rowStartIndex[cols()] + 1,
                      m_colStartIndex[cols()] + 1);
      else
        m_data.resize(rows(), rows(), cols(), m_rowStartIndex[rows()] + 1,
                      m_colStartIndex[rows()] + 1);
    }
  }
   ...
   * zero \sa resizeNonZeros(Index), reserve(), setZero()
   */
  void resize(size_t rows, size_t cols) {
    ...
    if (diagSize % 2) {  // diagSize is odd
      ...
      m_data.resize(diagSize, IsRowMajor ? cols : rows,
                    IsRowMajor ? rows : cols, 2 * k * k + k + 1,
                    2 * k * k + k + 1);
  ...
  inline SkylineMatrix& operator=(const SkylineMatrix& other) {
    ...
    if (other.isRValue()) {
      ...
      memcpy(m_colStartIndex, other.m_colStartIndex,
             (m_outerSize + 1) * sizeof(Index));
      memcpy(m_rowStartIndex, other.m_rowStartIndex,
             (m_outerSize + 1) * sizeof(Index));
      m_data = other.m_data;
    }
    return *this;
  ...
  friend std::ostream& operator<<(std::ostream& s, const SkylineMatrix& m) {
    EIGEN_DBG_SKYLINE(
        ...
        for (Index i = 0; i < m.m_data.upperSize(); i++) std::cout
        << m.m_data.upper(i) << "\t";
        std::cout << std::endl; std::cout << "upper profile : " << std::endl;
        ...
        for (Index i = 0; i < m.m_data.lowerSize(); i++) std::cout
        << m.m_data.lower(i) << "\t";
        std::cout << std::endl; std::cout << "lower profile : " << std::endl;
...
class SkylineMatrix<Scalar, _Options>::InnerUpperIterator {
 public:
  InnerUpperIterator(const SkylineMatrix& mat, Index outer)
      : m_matrix(mat),
        m_outer(outer),
        m_id(_Options == RowMajor ? mat.m_colStartIndex[outer]
                                  : mat.m_rowStartIndex[outer] + 1),
        m_start(m_id),
        m_end(_Options == RowMajor ? mat.m_colStartIndex[outer + 1]
                                   : mat.m_rowStartIndex[outer + 1] + 1) {}
  ...
  inline Index index() const {
    return IsRowMajor ? m_outer - m_matrix.m_data.upperProfile(m_outer) +
                            (m_id - m_start)
...
class SkylineMatrix<Scalar, _Options>::InnerLowerIterator {
 public:
  InnerLowerIterator(const SkylineMatrix& mat, Index outer)
      : m_matrix(mat),
        m_outer(outer),
        m_id(_Options == RowMajor ? mat.m_rowStartIndex[outer]
                                  : mat.m_colStartIndex[outer] + 1),
        m_start(m_id),
        m_end(_Options == RowMajor ? mat.m_rowStartIndex[outer + 1]
                                   : mat.m_colStartIndex[outer + 1] + 1) {}
  ...
  inline Index index() const {
    return IsRowMajor ? m_outer - m_matrix.m_data.lowerProfile(m_outer) +
                            (m_id - m_start)
                      : m_outer + (m_id - m_start) + 1;
    ;
  }
