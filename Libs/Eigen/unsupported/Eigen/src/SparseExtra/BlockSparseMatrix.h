 ...
 * Here each block has its own dimensions. The only restriction is that all the
 * blocks in a row (resp. a column) should have the same number of rows (resp.
 * of columns). It is thus required in this case to describe the layout of the
 * matrix by calling setBlockLayout(rowBlocks, colBlocks).
 *
 * In any of the previous case, the matrix can be filled by calling
 * setFromTriplets(). A regular sparse matrix can be converted to a block sparse
 * matrix and vice versa. It is obviously required to describe the block layout
 * beforehand by calling either setBlockSize() for fixed-size blocks or
 * setBlockLayout for variable-size blocks.
 *
 * \tparam _Scalar The Scalar type
 * \tparam _BlockAtCompileTime The block layout option. It takes the following
 * values Dynamic : block size known at runtime a numeric number : fixed-size
 * block known at compile time
 */
template <typename _Scalar, int _BlockAtCompileTime = Dynamic,
          int _Options = ColMajor, typename _StorageIndex = int>
class BlockSparseMatrix;
...
namespace internal {
template <typename _Scalar, int _BlockAtCompileTime, int _Options,
          typename _Index>
struct traits<
    BlockSparseMatrix<_Scalar, _BlockAtCompileTime, _Options, _Index> > {
...
struct traits<BlockSparseMatrixView<BlockSparseMatrixT> > {
  typedef Ref<
      Matrix<typename BlockSparseMatrixT::Scalar, BlockSparseMatrixT::BlockSize,
             BlockSparseMatrixT::BlockSize> >
      Scalar;
  typedef Ref<
      Matrix<typename BlockSparseMatrixT::RealScalar,
             BlockSparseMatrixT::BlockSize, BlockSparseMatrixT::BlockSize> >
      RealScalar;
};
...
class BlockVectorReturn {
 public:
  ...
  typedef Ref<Matrix<typename VectorType::Scalar, RowsAtCompileTime,
                     ColsAtCompileTime> >
      Scalar;
  typedef typename BlockSparseMatrixT::Index Index;
 ...
 public:
  ...
  void scaleAndAddTo(Dest& dest, const typename Rhs::Scalar& alpha) const {
    ...
    internal::sparse_time_dense_product(BlockSparseMatrixView<Lhs>(m_lhs),
                                        BlockVectorView<Lhs, Rhs>(m_lhs, m_rhs),
                                        tmpDest, alpha);
  }
...
class BlockSparseMatrix
    : public SparseMatrixBase<BlockSparseMatrix<_Scalar, _BlockAtCompileTime,
                                                _Options, _StorageIndex> > {
 public:
  typedef _Scalar Scalar;
  typedef typename NumTraits<Scalar>::Real RealScalar;
  typedef _StorageIndex StorageIndex;
  typedef typename internal::ref_selector<BlockSparseMatrix<
      _Scalar, _BlockAtCompileTime, _Options, _StorageIndex> >::type Nested;
  ...
  typedef Matrix<Scalar, _BlockAtCompileTime, _BlockAtCompileTime,
                 IsColMajor ? ColMajor : RowMajor>
      BlockScalar;
  typedef Matrix<RealScalar, _BlockAtCompileTime, _BlockAtCompileTime,
                 IsColMajor ? ColMajor : RowMajor>
      BlockRealScalar;
  typedef
      typename internal::conditional<_BlockAtCompileTime == Dynamic, Scalar,
                                     BlockScalar>::type BlockScalarReturnType;
  typedef BlockSparseMatrix<Scalar, BlockSize, IsColMajor ? ColMajor : RowMajor,
                            StorageIndex>
      PlainObject;
 ...
 public:
  ...
  BlockSparseMatrix(const BlockSparseMatrix& other)
    ...
    // should we allow copying between variable-size blocks and fixed-size
    // blocks ??
    eigen_assert(m_blockSize == BlockSize &&
                 " CAN NOT COPY BETWEEN FIXED-SIZE AND VARIABLE-SIZE BLOCKS");

    std::copy(other.m_innerOffset, other.m_innerOffset + m_innerBSize + 1,
              m_innerOffset);
    std::copy(other.m_outerOffset, other.m_outerOffset + m_outerBSize + 1,
              m_outerOffset);
    std::copy(other.m_values, other.m_values + m_nonzeros, m_values);

    if (m_blockSize != Dynamic)
      std::copy(other.m_blockPtr, other.m_blockPtr + m_nonzerosblocks,
                m_blockPtr);

    std::copy(other.m_indices, other.m_indices + m_nonzerosblocks, m_indices);
    std::copy(other.m_outerIndex, other.m_outerIndex + m_outerBSize,
              m_outerIndex);
  }
  ...
  inline BlockSparseMatrix(const MatrixType& spmat) : m_blockSize(BlockSize) {
    EIGEN_STATIC_ASSERT((m_blockSize != Dynamic),
  ...
  inline BlockSparseMatrix& operator=(const MatrixType& spmat) {
    ...
    eigen_assert(((MatrixType::Options & RowMajorBit) != IsColMajor) &&
                 "Wrong storage order");
    typedef SparseMatrix<bool, MatrixType::Options, typename MatrixType::Index>
        MatrixPatternType;
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      std::vector<bool> nzblocksFlag(m_innerBSize,
                                     false);  // Record the existing blocks
      blockPattern.startVec(bj);
      for (StorageIndex j = blockOuterIndex(bj); j < blockOuterIndex(bj + 1);
           ++j) {
        typename MatrixType::InnerIterator it_spmat(spmat, j);
        for (; it_spmat; ++it_spmat) {
          StorageIndex bi = innerToBlock(
              it_spmat.index());  // Index of the current nonzero block
          if (!nzblocksFlag[bi]) {
            // Save the index of this nonzero block
            nzblocksFlag[bi] = true;
            blockPattern.insertBackByOuterInnerUnordered(bj, bi) = true;
            // Compute the total number of nonzeros (including explicit zeros in
            // blocks)
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      for (StorageIndex j = blockOuterIndex(bj); j < blockOuterIndex(bj + 1);
        ...
        for (; it_spmat; ++it_spmat) {
          ...
          StorageIndex idxVal;  // Get the right position in the array of values
                                // for this element
          if (m_blockSize == Dynamic) {
            // Offset from all blocks before ...
            idxVal = m_blockPtr[m_outerIndex[bj] + idx];
            // ... and offset inside the block
            idxVal += (j - blockOuterIndex(bj)) * blockOuterSize(bj) +
                      it_spmat.index() - m_innerOffset[bi];
          } else {
            // All blocks before
            idxVal = (m_outerIndex[bj] + idx) * m_blockSize * m_blockSize;
            // inside the block
            idxVal += (j - blockOuterIndex(bj)) * m_blockSize +
                      (it_spmat.index() % m_blockSize);
          }
          // Insert the value
   ...
   * or setBlockSize() (for fixed-size blocks).
   *
   * \param blockPattern Sparse matrix of boolean elements describing the block
   * structure
   *
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      for (StorageIndex idx = 0; idx < nzBlockIdx.size(); ++idx) {
        ...
        if (m_blockSize == Dynamic)
          m_blockPtr[offset] =
              m_blockPtr[offset - 1] +
              blockInnerSize(nzBlockIdx[idx]) * blockOuterSize(bj);
   ...
   */
  inline void setBlockLayout(const VectorXi& rowBlocks,
    ...
    eigen_assert(m_innerBSize == innerBlocks.size() &&
                 "CHECK THE NUMBER OF ROW OR COLUMN BLOCKS");
    eigen_assert(m_outerBSize == outerBlocks.size() &&
                 "CHECK THE NUMBER OF ROW OR COLUMN BLOCKS");
    ...
    m_innerOffset[0] = 0;
    m_outerOffset[0] = 0;
    std::partial_sum(&innerBlocks[0], &innerBlocks[m_innerBSize - 1] + 1,
                     &m_innerOffset[1]);
    std::partial_sum(&outerBlocks[0], &outerBlocks[m_outerBSize - 1] + 1,
                     &m_outerOffset[1]);
  ...
  /**
   * \brief Allocate the internal array of pointers to blocks and their inner
   * indices
   *
   * \note For fixed-size blocks, call setBlockSize() to set the block.
   * And For variable-size blocks, call setBlockLayout() before using this
   * function
   *
   * \param nonzerosblocks Number of nonzero blocks. The total number of
   * nonzeros is is computed in setBlockLayout() for variable-size blocks \sa
   * setBlockSize()
   */
   ...
   * The InputIterator class should provide the functions row(), col() and
   * value()
   *
  ...
  void setFromTriplets(const InputIterator& begin, const InputIterator& end) {
     ...
     * FIXME This can be unnecessarily expensive since only the inner indices
     * have to be sorted The best approach is like in
    ...
    VectorXi nzblock_outer(
        m_outerBSize);                // Number of nz blocks per outer vector
    VectorXi nz_outer(m_outerBSize);  // Number of nz per outer vector...for
                                      // variable-size blocks
    nzblock_outer.setZero();
    nz_outer.setZero();
    for (InputIterator it(begin); it != end; ++it) {
      eigen_assert(it->row() >= 0 && it->row() < this->blockRows() &&
                   it->col() >= 0 && it->col() < this->blockCols());
      eigen_assert((it->value().rows() == it->value().cols() &&
                    (it->value().rows() == m_blockSize)) ||
                   (m_blockSize == Dynamic));

      if (m_blockSize == Dynamic) {
        eigen_assert((rowBlocks[it->row()] == 0 ||
                      rowBlocks[it->row()] == it->value().rows()) &&
                     "NON CORRESPONDING SIZES FOR ROW BLOCKS");
        eigen_assert((colBlocks[it->col()] == 0 ||
                      colBlocks[it->col()] == it->value().cols()) &&
                     "NON CORRESPONDING SIZES FOR COLUMN BLOCKS");
        rowBlocks[it->row()] = it->value().rows();
        colBlocks[it->col()] = it->value().cols();
      }
      nz_outer(IsColMajor ? it->col() : it->row()) +=
          it->value().rows() * it->value().cols();
    ...
    // Temporary markers
    VectorXi block_id(
        m_outerBSize);  // To be used as a block marker during insertion
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      if (m_blockSize == Dynamic) {
        m_blockPtr[m_outerIndex[bj + 1]] =
            m_blockPtr[m_outerIndex[bj]] + nz_outer(bj);
      }
    }
    ...
    for (InputIterator it(begin); it != end; ++it) {
      ...
      memcpy(&(m_values[nz_marker]), it->value().data(),
             block_size * sizeof(Scalar));
      if (m_blockSize == Dynamic) {
        m_blockPtr[block_id(outer) + 1] =
            m_blockPtr[block_id(outer)] + block_size;
      }
      block_id(outer)++;
    }

    // An alternative when the outer indices are sorted...no need to use an
    // array of markers
    ...
    //      for(InputIterator it(begin); it!=end; ++it)
    //      {
    //        while (id<bcol) // one pass should do the job unless there are
    //        empty columns
    //        {
    //          id++;
    ...
    //        id_nzblock++;
    //        memcpy(&(m_values[id_nz]),it->value().data(),
    //        block_size*sizeof(Scalar)); id_nz += block_size;
    //      }
  ...
  Ref<BlockScalar> coeffRef(Index brow, Index bcol) {
    ...
    eigen_assert(bcol < blockCols() &&
                 "BLOCK nzblocksFlagCOLUMN OUT OF BOUNDS");

    StorageIndex rsize =
        IsColMajor ? blockInnerSize(brow) : blockOuterSize(bcol);
    StorageIndex csize =
        IsColMajor ? blockOuterSize(bcol) : blockInnerSize(brow);
  ...
  Map<const BlockScalar> coeff(Index brow, Index bcol) const {
    ...
    while (offset < m_outerIndex[outer + 1] && m_indices[offset] != inner)
      offset++;
    if (m_indices[offset] == inner) {
      return Map<const BlockScalar>(&(m_values[blockPtr(offset)]), rsize,
                                    csize);
    } else
  ...
  BlockSparseTimeDenseProduct<BlockSparseMatrix, VecType> operator*(
      const VecType& lhs) const {
  ...
  /** \returns the total number of nonzero elements, including eventual explicit
   * zeros in blocks */
  ...
  inline Index blockInnerSize(Index bi) const {
    return (m_blockSize == Dynamic)
               ? (m_innerOffset[bi + 1] - m_innerOffset[bi])
               : m_blockSize;
  }
  inline Index blockOuterSize(Index bj) const {
    return (m_blockSize == Dynamic)
               ? (m_outerOffset[bj + 1] - m_outerOffset[bj])
               : m_blockSize;
  }
  ...
  Index blockPtr(Index id) const {
    ...
    // return blockDynIdx(id, typename
    // internal::conditional<(BlockSize==Dynamic), internal::true_type,
 ...
 protected:
  ...
  StorageIndex* m_innerOffset;  // Starting index of each inner block (size
                                // m_innerBSize+1)
  StorageIndex* m_outerOffset;  // Starting index of each outer block (size
                                // m_outerBSize+1)
  Index m_nonzerosblocks;  // Total nonzeros blocks (lower than  m_innerBSize x
                           // m_outerBSize)
  ...
  StorageIndex* m_outerIndex;  // Starting pointer of each block column in
                               // m_indices (size m_outerBSize)... OK
  Index m_blockSize;  // Size of a block for fixed-size blocks, otherwise -1
};

template <typename _Scalar, int _BlockAtCompileTime, int _Options,
          typename _StorageIndex>
class BlockSparseMatrix<_Scalar, _BlockAtCompileTime, _Options,
                        _StorageIndex>::BlockInnerIterator {
 public:
  enum { Flags = _Options };
  ...
  inline Map<BlockScalar> valueRef() {
    return Map<BlockScalar>(&(m_mat.m_values[m_mat.blockPtr(m_id)]), rows(),
                            cols());
  }
  // Block inner index
  ...
  inline Index rows() const {
    return (m_mat.m_blockSize == Dynamic) ? (m_mat.m_innerOffset[index() + 1] -
                                             m_mat.m_innerOffset[index()])
                                          : m_mat.m_blockSize;
  }
  // Number of columns in the current block ...
  inline Index cols() const {
    return (m_mat.m_blockSize == Dynamic) ? (m_mat.m_outerOffset[m_outer + 1] -
                                             m_mat.m_outerOffset[m_outer])
                                          : m_mat.m_blockSize;
  }
  inline operator bool() const { return (m_id < m_end); }

 protected:
  const BlockSparseMatrix<_Scalar, _BlockAtCompileTime, _Options, StorageIndex>&
      m_mat;
  const Index m_outer;
  Index m_id;
  Index m_end;
