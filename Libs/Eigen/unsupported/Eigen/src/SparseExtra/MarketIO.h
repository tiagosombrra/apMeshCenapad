...
inline bool GetMarketLine(std::stringstream& line, IndexType& M, IndexType& N,
                          IndexType& i, IndexType& j, Scalar& value) {
  line >> i >> j >> value;
  i--;
  j--;
  if (i >= 0 && j >= 0 && i < M && j < N) {
...
template <typename RealScalar>
inline void GetVectorElt(const std::string& line,
...
template <typename Scalar>
inline void PutMatrixElt(std::complex<Scalar> value, int row, int col,
                         std::ofstream& out) {
...
inline bool getMarketHeader(const std::string& filename, int& sym,
                            bool& iscomplex, bool& isvector) {
  sym = 0;
  iscomplex = false;
  isvector = false;
