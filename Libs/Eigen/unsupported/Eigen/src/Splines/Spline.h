...
class Spline {
 public:
  ...
  typedef
      typename SplineTraits<Spline>::ParameterVectorType ParameterVectorType;
   ...
   * derivatives. */
  typedef
      typename SplineTraits<Spline>::BasisDerivativeType BasisDerivativeType;

  /** \brief The data type representing the spline's control points. */
  typedef typename SplineTraits<Spline>::ControlPointVectorType
  ...
  Spline()
      : m_knots(1, (Degree == Dynamic ? 2 : 2 * Degree + 2)),
        m_ctrls(ControlPointVectorType::Zero(
            Dimension, (Degree == Dynamic ? 1 : Degree + 1))) {
    ...
    m_knots.template segment<MinDegree + 1>(0) =
        Array<Scalar, 1, MinDegree + 1>::Zero();
    m_knots.template segment<MinDegree + 1>(MinDegree + 1) =
        Array<Scalar, 1, MinDegree + 1>::Ones();
  }
   ...
   **/
  typename SplineTraits<Spline>::DerivativeType derivatives(
   ...
   *computes.
   **/
  typename SplineTraits<Spline>::BasisDerivativeType basisFunctionDerivatives(
      Scalar u, DenseIndex order) const;
  ...
  /**
   * \brief Computes the span within the provided knot vector in which u is
   *falling.
   **/
  static DenseIndex Span(
   ...
   **/
  static BasisVectorType BasisFunctions(Scalar u, DenseIndex degree,
  ...
  static BasisDerivativeType BasisFunctionDerivatives(
      const Scalar u, const DenseIndex order, const DenseIndex degree,
      const KnotVectorType& knots);
...
DenseIndex Spline<_Scalar, _Dim, _Degree>::Span(
    ...
    const typename SplineTraits<
        Spline<_Scalar, _Dim, _Degree> >::KnotVectorType& knots) {
...
Spline<_Scalar, _Dim, _Degree>::BasisFunctions(
  ...
  VectorBlock<BasisVectorType, Degree>(right, 1, p) =
      VectorBlock<const KnotVectorType, Degree>(U, i + 1, p) - u;
...
void derivativesImpl(const SplineType& spline, typename SplineType::Scalar u,
  ...
  for (DenseIndex der_order = 0; der_order < n + 1; ++der_order) {
    const Replicate<BasisDerivativeRowXpr, Dimension, 1> ctrl_weights(
...
Spline<_Scalar, _Dim, _Degree>::BasisFunctionDerivatives(
    const typename Spline<_Scalar, _Dim, _Degree>::Scalar u,
    const DenseIndex order, const DenseIndex degree,
    const typename Spline<_Scalar, _Dim, _Degree>::KnotVectorType& knots) {
