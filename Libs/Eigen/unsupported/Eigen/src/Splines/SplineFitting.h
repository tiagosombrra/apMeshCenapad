...
void KnotAveraging(const KnotVectorType& parameters, DenseIndex degree,
    ...
    for (int j = 0; j <= temporaryParameterIndex - 2; ++j) {
      for (DenseIndex k = 0; k < derivativeIndices.size(); ++k) {
        if (parameterIndices[j + 1] == derivativeIndices[k] &&
          ...
          derivativeKnots[++newKnotIndex] =
              temporaryParameters.segment(j, 3).mean();
          break;
        }
      }
    }
...
void ChordLengths(const PointArrayType& pts, KnotVectorType& chord_lengths) {
  ...
  chord_lengths.rightCols(n - 1) =
      (pts.array().leftCols(n - 1) - pts.array().rightCols(n - 1))
          .matrix()
          .colwise()
          .norm();
...
struct SplineFitting {
  ...
  static SplineType InterpolateWithDerivatives(
      const PointArrayType& points, const PointArrayType& derivatives,
...
template <typename SplineType>
template <typename PointArrayType>
SplineType SplineFitting<SplineType>::Interpolate(
  ...
  for (DenseIndex i = 1; i < n - 1; ++i) {
    ...
    A.row(i).segment(span - degree, degree + 1) =
        SplineType::BasisFunctions(knot_parameters[i], degree, knots);
  }
  A(0, 0) = 1.0;
  A(n - 1, n - 1) = 1.0;
...
template <typename SplineType>
template <typename PointArrayType, typename IndexArray>
SplineType SplineFitting<SplineType>::InterpolateWithDerivatives(
    const PointArrayType& points, const PointArrayType& derivatives,
    const IndexArray& derivativeIndices, const unsigned int degree,
    const ParameterVectorType& parameters) {
  typedef typename SplineType::KnotVectorType::Scalar Scalar;
  ...
  if (derivativeIndices[derivatives.cols() - 1] == points.cols() - 1) {
    ...
    Scalar y =
        (knots(knots.size() - 1) - knots(knots.size() - (degree + 2))) / degree;
  ...
  ControlPointVectorType controlPoints =
...
SplineType SplineFitting<SplineType>::InterpolateWithDerivatives(
  ...
  return InterpolateWithDerivatives(points, derivatives, derivativeIndices,
                                    degree, parameters);
}
}  // namespace Eigen
