...
AlignedBox<double, Dim> bounding_box(const Ball<Dim> &b) {
  return AlignedBox<double, Dim>(b.center.array() - b.radius,
                                 b.center.array() + b.radius);
}

inline double SQR(double x) { return x * x; }

template <int Dim>
struct BallPointStuff  // this class provides functions to be both an
                       // intersector and a minimizer, both for a ball and a
...
struct TreeTest {
  ...
  void testMinimize2() {
    ...
    for (int i = 0; i < 50; ++i) {
      b.push_back(BallType(VectorType::Random(),
                           1e-7 + 1e-6 * internal::random(0., 1.)));
      for (int j = 0; j < 3; ++j) v.push_back(VectorType::Random());
    }
