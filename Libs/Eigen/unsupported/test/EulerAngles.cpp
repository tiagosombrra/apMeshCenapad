...
void verify_euler_ranged(const Matrix<Scalar, 3, 1>& ea,
  ...
  Matrix3 m(e);
  Vector3 eabis = EulerAnglesType(m, positiveRangeAlpha, positiveRangeBeta,
                                  positiveRangeGamma)
                      .angles();
  ...
  VERIFY_IS_APPROX(
      eabis,
      eabis2);  // Verify that our estimation is the same as m.eulerAngles() is

  Matrix3 mbis(AngleAxisType(eabis[0], I) * AngleAxisType(eabis[1], J) *
               AngleAxisType(eabis[2], K));
  ...
  if (!(positiveRangeAlpha || positiveRangeBeta || positiveRangeGamma)) {
    ...
    if ((i != k || ea[1] != 0) &&
        (i == k || !internal::isApprox(abs(ea[1]), Scalar(EIGEN_PI / 2),
  ...
  // Quaternions
  QuaternionType q(e);
  eabis = EulerAnglesType(q, positiveRangeAlpha, positiveRangeBeta,
                          positiveRangeGamma)
              .angles();
  VERIFY_IS_APPROX(eabis,
                   eabis2);  // Verify that the euler angles are still the same
}
