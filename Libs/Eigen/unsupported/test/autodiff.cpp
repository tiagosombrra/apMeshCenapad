...
EIGEN_DONT_INLINE Scalar foo(const Scalar& x, const Scalar& y) {
  ...
  return x * 2 - 1 + static_cast<Scalar>(pow(1 + x, 2)) + 2 * sqrt(y * y + 0) -
         4 * sin(0 + x) + 2 * cos(y + 0) - exp(Scalar(-0.5) * x * x + 0);
...
struct TestFunc1 {
  ...
  void operator()(const Matrix<T, InputsAtCompileTime, 1>& x,
                  Matrix<T, ValuesAtCompileTime, 1>* _v) const {
...
struct integratorFunctor {
  ...
  void operator()(const T1& input, T2* output, T3* jacobian,
...
void forward_jacobian(const Func& f) {
  ...
  typename Func::JacobianType j(f.values(), f.inputs()),
...
void test_autodiff_hessian() {
  ...
  VectorADD x(2);
  double s1 = internal::random<double>(), s2 = internal::random<double>(),
         s3 = internal::random<double>(), s4 = internal::random<double>();
  x(0).value() = s1;
  x(1).value() = s2;

  // set unit vectors for the derivative directions (partial derivatives of the
  // input vector)
  ...
  VERIFY_IS_APPROX(y.value().derivatives()(0),
                   s3 * std::cos(s1 * s3 + s2 * s4));
  VERIFY_IS_APPROX(y.value().derivatives()(1),
                   s4 * std::cos(s1 * s3 + s2 * s4));
  VERIFY_IS_APPROX(y.derivatives()(0).derivatives(),
                   -std::sin(s1 * s3 + s2 * s4) * Vector2d(s3 * s3, s4 * s3));
  VERIFY_IS_APPROX(y.derivatives()(1).derivatives(),
...
double bug_1222() {
  ...
  const AD chi_3 = 1.0;
  // this line did not work, because operator+ returns ADS<DerType&>, which then
  // cannot be converted to ADS<DerType>
...
// failed because implementation of min attempts to construct ADS<DerType&> via
// constructor AutoDiffScalar(const Real& value) without initializing
// m_derivatives (which is a reference in this case)
...
// regression test for some compilation issues with specializations of
