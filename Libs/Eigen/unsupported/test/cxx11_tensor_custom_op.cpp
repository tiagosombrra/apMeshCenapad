...
struct InsertZeros {
  ...
  void eval(const Tensor<float, 2>& input, Output& output,
    ...
    Eigen::DSizes<DenseIndex, 2> extents(output.dimension(0) - 1,
                                         output.dimension(1) - 1);
    output.slice(offsets, extents).stride(strides).device(device) =
        input.constant(0.0f);
  }
};
...
struct BatchMatMul {
  DSizes<DenseIndex, 3> dimensions(const Tensor<float, 3>& input1,
                                   const Tensor<float, 3>& input2) const {
  ...
  void eval(const Tensor<float, 3>& input1, const Tensor<float, 3>& input2,
    ...
    for (int i = 0; i < output.dimension(2); ++i) {
      output.template chip<2>(i).device(device) =
          input1.chip<2>(i).contract(input2.chip<2>(i), dims);
    }
  }
};
...
static void test_custom_binary_op() {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    array<DimPair, 1> dims;
    dims[0] = DimPair(1, 0);
    Tensor<float, 2> reference =
        tensor1.chip<2>(i).contract(tensor2.chip<2>(i), dims);
