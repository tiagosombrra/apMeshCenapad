        ...
        for (int c = 0; c < 5; ++c) {
          for (int d = 0; d < 2; ++d) {
            for (int b = 0; b < 7; ++b) {
              ...
              if (r - 1 + i >= 0 && c - 2 + j >= 0 && r - 1 + i < 3 &&
                  c - 2 + j < 5) {
                expected = tensor(d, r - 1 + i, c - 2 + j, b);
                expected_row_major =
                    tensor_row_major(b, c - 2 + j, r - 1 + i, d);
              }
              // ColMajor
              ...
              VERIFY_IS_EQUAL(entire_image_patch(d, r, c, patchId, b),
                              expected);
              // RowMajor
        ...
        for (int c = 0; c < 2; ++c) {
          for (int d = 0; d < 2; ++d) {
            for (int b = 0; b < 7; ++b) {
              ...
              // RowMajor
              if (row_offset >= 0 && col_offset >= 0 &&
                  row_offset < tensor_row_major.dimension(2) &&
                  col_offset < tensor_row_major.dimension(1)) {
                expected_row_major =
                    tensor_row_major(b, col_offset, row_offset, d);
              }
              if (twod_patch_row_major(b, patchId, c, r, d) !=
                  expected_row_major) {
                std::cout << "Mismatch detected at index i=" << i << " j=" << j
                          << " r=" << r << " c=" << c << " d=" << d
                          << " b=" << b << std::endl;
              }
              VERIFY_IS_EQUAL(twod_patch_row_major(b, patchId, c, r, d),
                              expected_row_major);
              // Check that ColMajor and RowMajor agree.
              VERIFY_IS_EQUAL(expected, expected_row_major);
            }
...
void test_patch_padding_valid() {
  ...
  int input_cols = 3;
  int input_batches = 1;
  int ksize = 2;   // Corresponds to the Rows and Cols for
  ...
  Tensor<float, 5> result = tensor.extract_image_patches(
      ksize, ksize, stride, stride, 1, 1, PADDING_VALID);
  ...
  Tensor<float, 5, RowMajor> result_row_major =
      tensor_row_major.extract_image_patches(ksize, ksize, stride, stride, 1, 1,
                                             PADDING_VALID);
  ...
  for (int i = 0; (i + stride + ksize - 1) < input_rows;
       i += stride) {  // input rows
    for (int j = 0; (j + stride + ksize - 1) < input_cols;
         j += stride) {  // input cols
        ...
        for (int c = 0; c < ksize; ++c) {              // patch cols
          for (int d = 0; d < input_depth; ++d) {      // depth
            for (int b = 0; b < input_batches; ++b) {  // batch
              ...
              if (row_offset >= 0 && col_offset >= 0 &&
                  row_offset < input_rows && col_offset < input_cols) {
                expected = tensor(d, row_offset, col_offset, b);
                expected_row_major =
                    tensor_row_major(b, col_offset, row_offset, d);
              }
              ...
              VERIFY_IS_EQUAL(result_row_major(b, patchId, c, r, d),
                              expected_row_major);
              // Check that ColMajor and RowMajor agree.
              VERIFY_IS_EQUAL(expected, expected_row_major);
            }
...
void test_patch_padding_valid_same_value() {
  ...
  int input_cols = 5;
  int input_batches = 2;
  int ksize = 3;   // Corresponds to the Rows and Cols for
                   // tensor.extract_image_patches<>.
  int stride = 2;  // Only same stride is supported.
  // ColMajor
  Tensor<float, 4> tensor(input_depth, input_rows, input_cols, input_batches);
...
void test_patch_padding_same() {
  ...
  Tensor<float, 5> result =
      tensor.extract_image_patches(ksize, ksize, stride, stride, PADDING_SAME);
  ...
  Tensor<float, 5, RowMajor> result_row_major =
      tensor_row_major.extract_image_patches(ksize, ksize, stride, stride,
                                             PADDING_SAME);
  VERIFY_IS_EQUAL(result.dimension(0), result_row_major.dimension(4));
  VERIFY_IS_EQUAL(result.dimension(1), result_row_major.dimension(3));
  VERIFY_IS_EQUAL(result.dimension(2), result_row_major.dimension(2));
      ...
      for (int r = 0; r < 2; ++r) {
        for (int c = 0; c < 2; ++c) {
          for (int d = 0; d < 2; ++d) {
            ...
            // RowMajor
            if (row_offset >= 0 && col_offset >= 0 &&
                row_offset < tensor_row_major.dimension(1) &&
            ...
            VERIFY_IS_EQUAL(twod_patch_row_major(patchId, c, r, d),
                            expected_row_major);
            // Check that ColMajor and RowMajor agree.
            VERIFY_IS_EQUAL(expected, expected_row_major);
          }
...
void test_imagenet_patches() {
  // Test the code on typical configurations used by the 'imagenet' benchmarks
  // at https://github.com/soumith/convnet-benchmarks ColMajor
  ...
  // RowMajor
  Tensor<float, 5, RowMajor> l_out_row_major =
      l_in.swap_layout().extract_image_patches(11, 11);
