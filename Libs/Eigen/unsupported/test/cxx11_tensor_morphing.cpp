...
static void test_reshape_in_expr() {
  ...
  tensor3 = tensor1.reshape(newDims1).contract(tensor2.reshape(newDims2),
                                               contract_along);
...
static void test_simple_slice() {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < 3; ++k) {
        VERIFY_IS_EQUAL(slice2(0, 0, i, j, k),
                        tensor(1, 1, 3 + i, 4 + j, 5 + k));
      }
    }
  }
}
...
static void test_slice_in_expr() {
  ...
  tensor3 = tensor1.slice(indices1, sizes1)
                .contract(tensor2.slice(indices2, sizes2), contract_along);
  ...
  Tensor<float, 1, DataLayout> tensor6 =
      tensor4.reshape(DSizes<ptrdiff_t, 1>(7 * 7))
          .exp()
          .slice(DSizes<ptrdiff_t, 1>(0), DSizes<ptrdiff_t, 1>(35));
  for (int i = 0; i < 35; ++i) {
...
static void test_slice_raw_data() {
  ...
  typedef TensorEvaluator<decltype(tensor.slice(offsets, extents)),
                          DefaultDevice>
      SliceEvaluator;
  ...
  if (DataLayout == ColMajor) {
    ...
    auto slice2 =
        SliceEvaluator(tensor.slice(offsets, extents), DefaultDevice());
...
static void test_strided_slice() {
  ...
  if (true) {
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        for (int k = 0; k < 3; ++k) {
          VERIFY_IS_EQUAL(slice(0, 0, i, j, k),
                          tensor(1, 1, 3 + i, 4 + j, 5 + k));
        }
      }
    }
...
static void test_composition() {
  ...
  Eigen::Tensor<float, 3, DataLayout> tensor =
      matrix.slice(DSizes<ptrdiff_t, 2>(2, 0), DSizes<ptrdiff_t, 2>(1, 11))
          .reshape(newDims);
