...
void test_sycl_cpu(const Eigen::SyclDevice &sycl_device) {
  ...
  float *gpu_in3_data = static_cast<float *>(
      sycl_device.allocate(in3.dimensions().TotalSize() * sizeof(float)));
  ...
  sycl_device.memcpyDeviceToHost(
      in1.data(), gpu_in1_data, (in1.dimensions().TotalSize()) * sizeof(float));
  ...
  sycl_device.memcpyHostToDevice(
      gpu_in2_data, in2.data(), (in2.dimensions().TotalSize()) * sizeof(float));
  ...
  // a*3.14f + b*2.7f
  gpu_out.device(sycl_device) =
      gpu_in1 * gpu_in1.constant(3.14f) + gpu_in2 * gpu_in2.constant(2.7f);
  sycl_device.memcpyDeviceToHost(
      out.data(), gpu_out_data, (out.dimensions().TotalSize()) * sizeof(float));
  for (int i = 0; i < sizeDim1; ++i) {
    for (int j = 0; j < sizeDim2; ++j) {
      for (int k = 0; k < sizeDim3; ++k) {
        VERIFY_IS_APPROX(out(i, j, k),
                         in1(i, j, k) * 3.14f + in2(i, j, k) * 2.7f);
      }
    }
  }
  ...
  /// d= (a>0.5? b:c)
  sycl_device.memcpyHostToDevice(
      gpu_in3_data, in3.data(), (in3.dimensions().TotalSize()) * sizeof(float));
  gpu_out.device(sycl_device) =
      (gpu_in1 > gpu_in1.constant(0.5f)).select(gpu_in2, gpu_in3);
  sycl_device.memcpyDeviceToHost(
      out.data(), gpu_out_data, (out.dimensions().TotalSize()) * sizeof(float));
  for (int i = 0; i < sizeDim1; ++i) {
    for (int j = 0; j < sizeDim2; ++j) {
      for (int k = 0; k < sizeDim3; ++k) {
        VERIFY_IS_APPROX(out(i, j, k),
                         (in1(i, j, k) > 0.5f) ? in2(i, j, k) : in3(i, j, k));
      }
    }
  }
