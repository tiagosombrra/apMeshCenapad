...
void VERIFY_EQUAL(TensorUInt128<uint64_t, uint64_t> actual,
                  uint128_t expected) {
...
void test_add() {
  ...
  for (uint64_t i1 = 0; i1 < 100; ++i1) {
    for (uint64_t i2 = 1; i2 < 100 * incr; i2 += incr) {
      ...
      uint128_t a =
          (static_cast<uint128_t>(i1) << 64) + static_cast<uint128_t>(i2);
      for (uint64_t j1 = 0; j1 < 100; ++j1) {
        for (uint64_t j2 = 1; j2 < 100 * incr; j2 += incr) {
          TensorUInt128<uint64_t, uint64_t> j(j1, j2);
          uint128_t b =
              (static_cast<uint128_t>(j1) << 64) + static_cast<uint128_t>(j2);
...
void test_misc2() {
  ...
  for (int64_t log_div = 0; log_div < 63; ++log_div) {
    for (int64_t divider = 1; divider <= 1000000 * incr; divider += incr) {
      uint64_t expected = (static_cast<uint128_t>(1) << (64 + log_div)) /
                              static_cast<uint128_t>(divider) -
