...
void randomTest(const MatrixType& m, double tol) {
  ...
  */
  ...
  MatrixType m1(rows, cols), m2(rows, cols),
      identity = MatrixType::Identity(rows, cols);

  typedef
      typename NumTraits<typename internal::traits<MatrixType>::Scalar>::Real
          RealScalar;
