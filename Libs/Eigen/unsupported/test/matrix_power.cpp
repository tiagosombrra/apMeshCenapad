...
void test2dRotation(const T& tol) {
  ...
  for (int i = 0; i <= 20; ++i) {
    ...
    std::cout << "test2dRotation: i = " << i
              << "   error powerm = " << relerr(C, B) << '\n';
...
void test2dHyperbolicRotation(const T& tol) {
  ...
  for (int i = 0; i <= 20; ++i) {
    ...
    C = Apow(angle);
    std::cout << "test2dHyperbolicRotation: i = " << i
              << "   error powerm = " << relerr(C, B) << '\n';
    VERIFY(C.isApprox(B, tol));
  }
}
...
void test3dRotation(const T& tol) {
  ...
  for (int i = 0; i <= 20; ++i) {
    ...
    v.normalize();
    angle = std::pow(T(10), (i - 10) / T(5.));
    VERIFY(AngleAxis<T>(angle, v).matrix().isApprox(
        AngleAxis<T>(1, v).matrix().pow(angle), tol));
  }
}
...
void testSingular(const MatrixType& m_const,
  ...
  const int IsComplex =
      NumTraits<typename internal::traits<MatrixType>::Scalar>::IsComplex;
  typedef
      typename internal::conditional<IsComplex,
                                     TriangularView<MatrixType, Upper>,
                                     const MatrixType&>::type TriangularType;
  typename internal::conditional<IsComplex, ComplexSchur<MatrixType>,
                                 RealSchur<MatrixType> >::type schur;
  MatrixType T;
...
void test_matrix_power() {
  ...
  CALL_SUBTEST_1(test2dRotation<float>(
      2e-5));  // was 1e-5, relaxed for clang 2.8 / linux / x86-64
