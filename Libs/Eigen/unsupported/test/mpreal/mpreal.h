...
#define MPREAL_HAVE_DYNAMIC_STD_NUMERIC_LIMITS  // Enable extended
                                                ...
                                                // similar members are defined
                                                // as functions, not constants.
                                                // See
                                                // std::numeric_limits<mpfr::mpreal>
...
// Detect support for explicit converters.
     ...
     (defined(__GXX_EXPERIMENTAL_CXX0X__) && __GNUC_MINOR__ >= 5) || \
...
// (a) uses low-level data members, might not be compatible with new versions of
// MPFR (b) sign is not set, add (x)->_mpfr_sign = 1;
...
// MPFR 2.4.0 Specifics
  ...
  friend const mpreal remainder(const mpreal& x, const mpreal& y,
                                mp_rnd_t rnd_mode);
  friend const mpreal remquo(long* q, const mpreal& x, const mpreal& y,
                             mp_rnd_t rnd_mode);
...
inline mpreal::mpreal(mpreal&& other) {
  mpfr_set_uninitialized(
      mpfr_ptr());  // make sure "other" holds no pointer to actual data
...
inline const typename internal::result_type<Rhs>::type operator+(
    const mpreal& lhs, const Rhs& rhs) {
  return mpreal(lhs) += rhs;
}

template <typename Lhs>
inline const typename internal::result_type<Lhs>::type operator+(
    const Lhs& lhs, const mpreal& rhs) {
...
inline bool operator>(const mpreal& a, const long double b) {
  return !isnan EIGEN_NOT_A_MACRO(a) && (b == b) &&
...
// inline bool operator >= (const mpreal& a, const unsigned int b      ){ return
// !isnan EIGEN_NOT_A_MACRO (isnan()a) && (mpfr_cmp_ui(a.mpfr_srcptr(),b) >= 0
// );                }
inline bool operator>=(const mpreal& a, const long int b) {
  return !isnan EIGEN_NOT_A_MACRO(a) && (mpfr_cmp_si(a.mpfr_srcptr(), b) >= 0);
}
...
class numeric_limits<mpfr::mpreal> {
 public:
  ...
  inline static mpfr::mpreal round_error(
      mp_prec_t precision = mpfr::mpreal::get_default_prec()) {
