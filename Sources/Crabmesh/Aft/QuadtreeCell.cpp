...
void QuadtreeCell::subdivide(bool setChildrenEdges)
{
    ...
    */
    ...
    children[QUAD_BOTTOM_LEFT] = new QuadtreeCell(tree->getCellId(), /*mainDrive, */tree, min, mids[QUAD_MID], this, level + 1);
    children[QUAD_BOTTOM_RIGHT] = new QuadtreeCell(tree->getCellId(), /*mainDrive, */tree, mids[QUAD_BOTTOM], mids[QUAD_RIGHT], this, level + 1);
    children[QUAD_TOP_RIGHT] = new QuadtreeCell(tree->getCellId(), /*mainDrive, */tree, mids[QUAD_MID], max, this, level + 1);
    children[QUAD_TOP_LEFT] = new QuadtreeCell(tree->getCellId(), /*mainDrive, */tree, mids[QUAD_LEFT], mids[QUAD_TOP], this, level + 1);
...
{
    children[QUAD_BOTTOM_LEFT]->neighbors[QUAD_RIGHT].push_back(children[QUAD_BOTTOM_RIGHT]);
    children[QUAD_BOTTOM_LEFT]->neighbors[QUAD_TOP].push_back(children[QUAD_TOP_LEFT]);

    children[QUAD_BOTTOM_RIGHT]->neighbors[QUAD_TOP].push_back(children[QUAD_TOP_RIGHT]);
    children[QUAD_BOTTOM_RIGHT]->neighbors[QUAD_LEFT].push_back(children[QUAD_BOTTOM_LEFT]);

    children[QUAD_TOP_RIGHT]->neighbors[QUAD_BOTTOM].push_back(children[QUAD_BOTTOM_RIGHT]);
    children[QUAD_TOP_RIGHT]->neighbors[QUAD_LEFT].push_back(children[QUAD_TOP_LEFT]);

    children[QUAD_TOP_LEFT]->neighbors[QUAD_BOTTOM].push_back(children[QUAD_BOTTOM_LEFT]);
    children[QUAD_TOP_LEFT]->neighbors[QUAD_RIGHT].push_back(children[QUAD_TOP_RIGHT]);
...
Edge *QuadtreeCell::makeE(int i, int j)
{
...
void QuadtreeCell::makeMeshType2(int c)
{
    ...
    faces[0] = new Face(v[(base+7)%8], v[base], v[(base+5)%8], tree->faceId());
    faces[1] = new Face(v[(base+5)%8], v[base], v[(base+3)%8], tree->faceId());
    faces[2] = new Face(v[(base+3)%8], v[base], v[(base+1)%8], tree->faceId());
...
void QuadtreeCell::makeMeshType3(int c)
{
    ...
    vertices[0] = new Vertex(0.5*(min->getX() + max->getX()), 0.5*(min->getY() + max->getY()), tree->vertexId());
    ...
    faces[1] = new Face(v[(base+1)%8], v[(base+3)%8], vertices[0], tree->faceId());
    faces[2] = new Face(v[(base+3)%8], v[(base+4)%8], vertices[0], tree->faceId());
    faces[3] = new Face(v[(base+4)%8], v[(base+5)%8], vertices[0], tree->faceId());
    faces[4] = new Face(v[(base+5)%8], v[(base+7)%8], vertices[0], tree->faceId());
...
void QuadtreeCell::makeMeshType4(int c)
{
    ...
    faces[0] = new Face(v[base], v[(base+1)%8], v[(base+2)%8], tree->faceId());
    faces[1] = new Face(v[base], v[(base+2)%8], vertices[0], tree->faceId());
    faces[2] = new Face(v[(base+2)%8], v[(base+3)%8], vertices[0], tree->faceId());
    faces[3] = new Face(v[(base+3)%8], v[(base+5)%8], vertices[0], tree->faceId());
    faces[4] = new Face(v[(base+5)%8], v[(base+7)%8], vertices[0], tree->faceId());
    faces[5] = new Face(v[(base+7)%8], v[base], vertices[0], tree->faceId());
...
void QuadtreeCell::makeMeshType5(int c)
{
    ...
    faces[0] = new Face(v[(base+2)%8], v[base], v[(base+1)%8], tree->faceId());
    faces[1] = new Face(v[(base+4)%8], v[base], v[(base+2)%8], tree->faceId());
    faces[2] = new Face(v[(base+5)%8], v[base], v[(base+4)%8], tree->faceId());
    faces[3] = new Face(v[(base+7)%8], v[base], v[(base+5)%8], tree->faceId());
    faces[4] = new Face(v[(base+3)%8], v[(base+4)%8], v[(base+2)%8], tree->faceId());
...
void QuadtreeCell::setQuadtree(Quadtree *tree)
{
    this->tree = tree;
}

void QuadtreeCell::setLevel(long int level)
{
    this->level = level;
}

long int QuadtreeCell::getLevel()
{
    return level;
}

double QuadtreeCell::getSize()
{
    return size;
}

void QuadtreeCell::setParent(QuadtreeCell *parent)
{
    this->parent = parent;
}

QuadtreeCell *QuadtreeCell::getParent()
{
    return parent;
}
...
VertexList QuadtreeCell::getVertices()
{
         ...
         iter != edges.end(); iter++)
    {
        ...
        for (VertexList::iterator iter2 = vertices.begin();
...
EdgeList QuadtreeCell::getEdges()
{
    return edges;
}
...
double QuadtreeCell::surface()
{
    return size*size;
}

Vertex *QuadtreeCell::getMin()
{
    return min;
}

Vertex *QuadtreeCell::getMax()
{
    return max;
}
...
void QuadtreeCell::subdivideToLevel(unsigned long int level)
{
    if ((parent) && (this->level <= level) && (edges.empty()))
    {
        ...
        for (EdgeList::iterator iter = pEdges.begin();
             iter != pEdges.end(); iter++)
        {
            tree->findCell((*iter));
        }
    }
...
bool QuadtreeCell::subdivideAccordingToNeighbors()
{
    if ((neighbors[QUAD_BOTTOM].size() > 2) ||
            (neighbors[QUAD_RIGHT].size() > 2) ||
            (neighbors[QUAD_TOP].size() > 2) ||
...
{
    return ((bool)children[QUAD_BOTTOM_LEFT]);
}
...
{
    return (neighbors[QUAD_B].empty() ||
            neighbors[QUAD_R].empty() ||
            neighbors[QUAD_T].empty() ||
            neighbors[QUAD_L].empty());
}
...
void QuadtreeCell::makeOuterVertices()
{
    ...
    {
        v[1] = new Vertex(0.5*(min->getX() + max->getX()), min->getY(), tree->vertexId());
    ...
    {
        v[3] = new Vertex(max->getX(), 0.5*(min->getY() + max->getY()), tree->vertexId());
    ...
    {
        v[5] = new Vertex(0.5*(min->getX() + max->getX()), max->getY(), tree->vertexId());
    ...
    {
        v[7] = new Vertex(min->getX(), 0.5*(min->getY() + max->getY()), tree->vertexId());
...
void QuadtreeCell::makeTemplateBasedMesh()
{
    ...
    else if (total == 6)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if ((numNeighs[i] == numNeighs[(i+1)%4]) &&
                        (numNeighs[i] == 2))
                {
                    makeMeshType4(i);
...
//void QuadtreeCell::highlight(bool highlightEdges, bool dontHighlightNeighbors)
//{
//    highlighted = true;
...
//void QuadtreeCell::unhighlight(bool highlightEdges, bool dontHighlightNeighbors)
//{
//    highlighted = false;
