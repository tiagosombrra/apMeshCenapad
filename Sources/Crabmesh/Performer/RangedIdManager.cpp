...
void Performer::RangedIdManager::setMin(ULInt min)
{
    this->setMin(0, min);
}

ULInt Performer::RangedIdManager::getMin(UInt i) const
{
    return this->mins[i];
}

ULInt Performer::RangedIdManager::getMin() const
{
...
ULInt Performer::RangedIdManager::getOffset() const
{
    return this->offset;
}
...
ULInt Performer::RangedIdManager::getRange() const
{
    return this->range;
}
...
//        //std::cout<<"i:"<<i<<" this->ids[i]:"<<this->ids[i]<<" thread:"<<omp_get_thread_num()<<endl;
//        ofs<<"i:"<<i<<" this->ids[i]:"<<this->ids[i]<<" thread:"<<omp_get_thread_num()<<endl;
//    }
    return this->ids[i]++;
}

ULInt Performer::RangedIdManager::next()
{
    return this->next(0);
}
...
void Performer::RangedIdManager::adjust(UInt i)
{
    ...
    {
        ULInt numOffsets = (this->offset > 0) ? (this->ids[i] - this->mins[i])/this->offset : 0;
