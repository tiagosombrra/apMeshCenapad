...
Vetor CurvParamHermite::getDP0() const { return this->DP0; }

Vetor CurvParamHermite::getDP1() const { return this->DP1; }

void CurvParamHermite::setDP0(const Vetor &v) { this->DP0 = v; }

void CurvParamHermite::setDP1(const Vetor &v) { this->DP1 = v; }
...
double CurvParamHermite::calcularCurvatura(double t) {
  ...
  double v0 = 3 * pow((1 - t), 2) * (DP0.x - P0.x) +
              6 * t * (1 - t) * (DP1.x - DP0.x) +
              3 * pow(t, 2) * (P1.x - DP1.x);
  double v1 = 3 * pow((1 - t), 2) * (DP0.y - P0.y) +
              6 * t * (1 - t) * (DP1.y - DP0.y) +
              3 * pow(t, 2) * (P1.y - DP1.y);
  double v2 = 3 * pow((1 - t), 2) * (P1.z - P0.z) +
              6 * t * (1 - t) * (DP1.z - DP0.z) +
              3 * pow(t, 2) * (P1.z - DP1.z);

  // B''(t)= 6(1-t)(P2-2P1+P0)+6t(P3-2P2+P1)
  double v00 = 6 * (1 - t) * (DP1.x - 2 * DP0.x + P0.x) +
               6 * t * (P1.x - 2 * DP1.x + DP0.x);
  double v10 = 6 * (1 - t) * (DP1.y - 2 * DP0.y + P0.y) +
               6 * t * (P1.y - 2 * DP1.y + DP0.y);
  double v20 = 6 * (1 - t) * (DP1.z - 2 * DP0.z + P0.z) +
               6 * t * (P1.z - 2 * DP1.z + DP0.z);
