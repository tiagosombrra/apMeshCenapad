...
{
	return Data::time(static_cast<UInt>(hours), static_cast<UInt>(mins), static_cast<UInt>(secs));
}
...
#if defined(__linux__)
//code for trim() functions from http://stackoverflow.com/questions/216823/whats-the-best-way-to-trim-stdstring
...
bool Data::isStepByStep()
{
	return Data::state.isStepByStep();
}
...
void Data::glColor(Real r, Real g, Real b)
{
	...
	GLfloat col[] = {static_cast<GLfloat>(r), static_cast<GLfloat>(g), static_cast<GLfloat>(b), 1.0};
...
void Data::glColor(Real r, Real g, Real b, Real a)
{
	...
	GLfloat col[] = {static_cast<GLfloat>(r), static_cast<GLfloat>(g), static_cast<GLfloat>(b), static_cast<GLfloat>(a)};
...
void Data::glTranslate(Real x, Real y)
{
	glTranslate(x, y, 0.0);
}
...
void Data::glScale(Real x, Real y)
{
	glScale(x, y, 1.0);
}
...
void Data::glTextId(Real x, Real y, ULInt id)
{
	glTextId(x, y, 0.0, id);
}
