...
#if USE_OPENMP
  this->erro =
      this->erroGlobalOmp(malha, timer, this->comm->rank(), sizeThread);
#else
#if USE_MPI
...
int GeradorAdaptativoPorCurvatura::generatorSeq(Modelo& modelo, Timer* timer,
                                                int idrange) {
#if !USE_MPI
  // this->comm = new Parallel::TMCommunicator(false);
#endif
...
Malha* GeradorAdaptativoPorCurvatura::generatorInitialMeshOmp(
...
void GeradorAdaptativoPorCurvatura::escreveMalha(Malha* malha, int passo) {
  ...
  arq << "%HEADER" << endl
      << "Arquivo gerado pelo gerador de malhas de superficie do Daniel "
         "Siqueira"
      << endl
      << endl;
...
void GeradorAdaptativoPorCurvatura::escreveMalha(Malha* malha, int passo,
  ...
  cout << "#elementos_" << nameModel << "_n.process_" << numberProcess
       << "_passo_" << passo << "_rank_" << rank << endl;
  cout << Nt << endl;
...
#if USE_MPI
int GeradorAdaptativoPorCurvatura::generatorMpi(double listOfPatches[],
                                                int sizeOfListPatches,
                                                Timer* timer) {
  ...
  while (this->erro > EPSYLON) {
...
#ifdef __linux
      novosPontos[i] = AdaptadorPorCurvatura::adaptaCurvaByCurva(
          geo->getCurva(i), mapaPontos, this->idManager, 1);
      geo->getCurva(i)->setPontos(novosPontos[i]);
      novosPontos[i] = AdaptadorPorCurvatura::adaptaCurvaBySuperficie(
          geo->getCurva(i), mapaPontos, this->idManager, 1);
      geo->getCurva(i)->setPontos(novosPontos[i]);
#elif __APPLE__
      novosPontos.push_back(AdaptadorPorCurvatura::adaptaCurva(
          geo->getCurva(i), /*this->passo,*/ mapaPontos));
#elif WIN64
      novosPontos.push_back(AdaptadorPorCurvatura::adaptaCurva(
          geo->getCurva(i), /*this->passo,*/ mapaPontos));
#endif
    ...
    for (unsigned int i = 0; i < geo->getNumDePatches(); ++i) {
      CoonsPatch* p = static_cast<CoonsPatch*>(geo->getPatch(i));
      SubMalha* sub =
          AdaptadorPorCurvatura::adaptaDominio(p, this->idManager, 1);
      sub->setPatch(p);
      malha->insereSubMalha(sub, i);
    }
  ...
  timer->endTimerParallel(0, 0, 10);  // Full
  timer->printTime();  // escreve o arquivo com resumo de tempo para o process 0
                       // e n
  ...
  // 4.9. Escreve o(s) arquivo(s) com suas respectivos erros entre Ga e Gd da
  // malhas em cada passo
  //        for (MeshVector::iterator it = saveMesh.begin(); it !=
  //        saveMesh.end(); it++) {
  //            escreveErroMalha((*it).second, (*it).first, RANK_MPI);
  //        }
  //  }
...
void GeradorAdaptativoPorCurvatura::generatorMeshInitial() {
  for (unsigned int i = 0; i < this->geo->getNumDePatches(); ++i) {
    ...
    SubMalha* sub =
        this->malhaInicial(static_cast<CoonsPatch*>(patch), this->idManager);
