...
void Arquivo::lerCurvas ( const string leitura )
{
    ...
    if ( leitura[0] == 'P' or leitura[0] == 'D' )
        this->curvas.push_back( leitura );
}
...
{	
    // "D" de DEFINE_PATCH
    if ( leitura[0] == 'D' )
        this->patches.push_back( leitura );
}
...
char* Arquivo::convertaString( string fonte )
{
    ...
    for (unsigned int i = 0; i < fonte.length(); ++i)
        destino[i] = fonte[i];
...
void Arquivo::criarCurvasPara(/*Modelo& modelo*/)
{
    ...
    while ( itr != fim )
    {
        ...
        cout 	<< "\nContrui uma curva com ponto inicial ( "
                << pt0[0] << ", " << pt0[1] << ", " << pt0[2] <<")\n"
                << "ponto final: ("
                << pt1[0] << ", " << pt1[1] << ", " << pt1[2] << ")\n"
                << "Derivada no ponto inicial: ("
                << dpt0[0] << ", " << dpt0[1] << ", " << dpt0[2] << ")\n"
                << "Derivada no ponto final: ("
                << dpt1[0] << ", " << dpt1[1] << ", " << dpt1[2] << ")" << endl;
...
void Arquivo::lerArquivoPara ()
{
    ...
    while ( entrada.good() )
    {
        ...
        {
            leiaCurvas = true; // você pode ler as linhas e definir os pontos e suas derivadas das curvas
            continue;
        }
        ...
        else if ( linha == inicioDosPatches ) // começa a definição dos patches no arquivo
        {
